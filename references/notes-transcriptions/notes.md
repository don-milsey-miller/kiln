...
| 71 | Who writes the consumer's `.gitignore` | **One shared setup/initializer helper (#49)** owns every Kiln edit. Fresh projects receive one marked block containing `.planning/`, `.pi/sessions/`, and `.pi/runtime/`; `.pi/settings.json` is deliberately not ignored. The action is recorded so a deliberately removed block is never silently restored. **Amended 2026-09-02:** an existing exact legacy block containing only `.planning/` may be migrated by that same owner, under the same lock and atomic-write discipline, while preserving every byte outside the block; a removed or edited block requires an explicit user choice. Not a git repo ⇒ notice and continue, don't abort. | Nobody owned this originally: the `.planning/` line existed only in *this* repo's `.gitignore`, where it was inert. The record-and-never-re-add half is #67's principle applied again — a PM who deletes the line has made a decision, and setup must not silently override it. The agent-delivery design later added local Pi transcripts and runtime state, which must not be committed and must not be handled by a second appender with different idempotency rules. Idempotency keys on **"did Kiln add or migrate its block"**, not merely on whether one ignored path is present. |
...
| 29 | Session persistence | ~~**To disk under `.planning/`.**~~ **Amended 2026-09-02: the persistent user-facing Pi session lives under the consumer project's `.pi/sessions/`, with local launch/session-selection state under `.pi/runtime/`; both are gitignored. `.pi/settings.json` remains non-secret, project-scoped configuration and is not ignored. Specialist sessions remain disposable (`--no-session`).** Documents stay the real state. | **The old location would dirty the `.planning/` clone that #50 updates with `git pull`; tool state is not permission to write runtime data into the tool's source checkout.** The outer `.pi/` directory is Pi's project-local runtime/configuration boundary and keeps the clone clean. Losing a session must still never lose a decision: if a decision existed only in the transcript, it was never really recorded. This amendment changes storage location, not authority — `planning-content/` remains the committed project truth. |
...
| 123 | **The ownership boundary** — evidence owns observations, meaning is derived per claim | **`Evidence stores observations and provenance. What those observations mean for a claim is derived relative to that claim.`** Consequently: **evidence does not own polarity, relevance, applicability, verdict or confidence**; the **assertion's link** (`supportedBy` / `refutedBy`) expresses **bearing**; the **effective-assertion view** derives **applicability, verdict and confidence**; and **matrix summaries and environment-wide labels are projections, never substitutes for observations** (#113). The sandbox therefore records an omission state **only when it can distinguish that state operationally** — otherwise the honest representation is plain absence: `unknown`, still applicable, and capped below `environment-matched` (#122). | **This is the general rule that four separate corrections were each a special case of.** #105 (excluded ≠ discredited), #112 (a `contested` verdict on an under-scoped claim), #113 (`"both"` as an environment value), and #122's `not relevant` were all the same mistake: **a claim-relative judgement written into a place that has only one copy.** Evidence has one record and many claims, so any judgement stored on it silently asserts itself for every claim that ever links to it. **The rule is what makes evidence reusable at all** — a record scoped to the claim that first cited it can only ever serve that one. ✅ **The current model already conforms**: `evidence` declares none of the five, and `outcome` (`success` \| `failure`) already documents itself as *"NOT a verdict on the assertion"*. So this row governs **what may be added**, and the field #122 defers to `QST-0006` is the first thing it governs. ⚠️ **Guarded by `test/derived-not-stored.test.mjs`, and the guard is a name check, not a proof** — falsified 2026-08-18 by injecting `evidence.polarity` and `assertion.confidence` (both caught). It stops the obvious drift and **cannot stop the same concept arriving under a name nobody forbade**; the reviewable rule is this row. _(Recorded 2026-08-18.)_
| 124 | **QST-0005 answered** — research capability is a typed interface the extension owns, never a vendor | **The shipped Pi extension owns `research_search`, `research_fetch` and `research_capability`, and the specialist depends on THOSE.** An MCP server may sit behind the extension as an **adapter**; it must not become the contract. **Boundary:** public, read-only research when a delegated question turns on a current external fact. **Authenticated sources, purchases and submissions are unavailable** until separately activated. **Credentials live host-side, outside the repo and the planning content** — consumed by the extension, never placed in a child prompt or returned in tool output. **Every delegation probes tool registration AND backend health/auth: installed-but-unusable is unavailable.** **Search results are discovery observations, not evidence** — a fetched source backing a recorded claim must become `evidence(kind: source)` with URL, retrieval time, source metadata and a retained citation. **If retrieval is unavailable the specialist returns a structured capability refusal and records the gap; it must NOT answer from model memory.** → `DEC-0004`, `QST-0005` answered. | **Naming three typed tools rather than a backend is what makes the capability probeable, which is the half of #121 that does the work** — "do you have research" now has a probe result for an answer instead of an assumption. **Pi has no built-in MCP and its extension API registers custom tools**, so a vendor belongs in the adapter layer; a contract written against vendor tool names would change whenever the backend did — **#67's toolless child arriving through the supply chain rather than through trust.** ⚠️ **The refusal clause is the load-bearing one.** The intake's problem 2 was *"educated guessing"*, and a research specialist that falls back to model memory when retrieval is down **reproduces exactly that failure while looking like success** — the same shape as #67's trust-denied session that completed normally with the tools absent. **Discovery-is-not-evidence keeps #123 intact at the point new observations enter**: a search hit is an observation about what EXISTS, not an observation about the claim. ✅ **The backend stays replaceable because changing it cannot alter the contract** — which is what makes replaceability a property rather than an intention, so the first adapter is work, not a blocking design question. _(Decided by the PM 2026-08-22.)_ |
| 125 | **QST-0006 answered** — tier 1 first, behind a typed validation controller with a **stated** boundary | **Tier 1 first (#77 already makes it the default), through a typed controller owning `provision → execute → observe → destroy`. The specialist never receives unrestricted shell execution.** A job DECLARES: required tier and capabilities · inputs or hashes · **commands as an argument array, never an interpolated shell string** · a capture plan · timeout and output limits · expected outputs · any network, credential or cost requirement. **The controller refuses a job that exceeds the approved ceiling BEFORE provisioning.** Tier 1 = fresh temp workspace + Python venv, with the boundary stated rather than implied: **"a virtual environment isolates Python dependencies; it is not an OS security sandbox."** It therefore **claims no containment of hostile code, no host-filesystem denial and no network isolation**, supplies no credentials, uses an allowlisted process environment, and suits **trusted, non-destructive validation only**. Containment, OS/package fidelity, credentials or external infrastructure ⇒ **higher tier**. Every run yields an observation record: commands, inputs, outputs, exit status, timing, **versions**, environment facts, permission failures, cleanup outcome. **The capture plan makes omission states DETECTABLE** — `not captured` (collection deliberately disabled) · `not observable` (no collector exists) · `unavailable` (collection ran, fact absent) · `redacted` (obtained, policy suppressed) — **each with a reason**; unrequested facts stay simply absent. **Cleanup failure is a first-class run result: preserve the evidence, report the retained path, never present destruction as successful.** → `DEC-0005`, `QST-0006` answered. | ⚠️ **The stated boundary is the whole decision.** A venv called a sandbox would let every claim validated in it **inherit containment it never had** — and #42's rungs would report a strength nothing earned, invisibly, which is #122's failure mode with a bigger blast radius. **Naming what tier 1 does NOT do is what keeps the ceiling truthful.** **Argument arrays remove an interpolation defect class at the type level rather than by review** (#58's floor, same move). **The four omission states are exactly what #122 said was owed and #123 said must not be exceeded**: each is a fact about the OBSERVER, none is a judgement about a claim, and **each is declared only because a capture plan can actually distinguish it** — a state nothing can detect would repeat #121's error one layer down, in the data model. **Refusal before provisioning is what makes #77's authorisation a control rather than a report**; a ceiling checked after spending is a receipt. ⚠️ **Still owed:** the `evidence` field carrying the omission states is decided but unbuilt — #122's deferred consequence lands here, and it is now work rather than an open question. ⚠️ **Implementation note, 2026-08-22 — four research-CLI defects live under this row rather than getting one of their own.** The PM's call, and the reasoning is worth keeping because it is about what a decision IS: **(a)** resolving the content root *before* fetching is an **application of this row's precondition-before-side-effect rule**, not a new rule; **(b)** `process.exit()` crashing after `fetch` on this Node/Windows build is a **measured platform behaviour** — the regression test is the durable control, and if it is ever promoted further it belongs as **assertion + evidence, never as a decision**; **(c)** the dangling timer is an **implementation defect**, fixed by the shared timeout helper; **(d)** joining multi-token flag values is **defensive hardening against a failure never observed**. ⚠️ **Bundling four things of four different kinds into one numbered row would blur decisions, observations and defect fixes — the exact separation the evidence model exists to preserve** (#123). A decision register that accepts "things we learned this week" stops being a decision register. _(Decided by the PM 2026-08-22.)_ |
| 126 | `resolveQuestion` — the fifth mutation, and the **third** recurrence of one defect | **Settling a question now has a typed operation.** `resolveQuestion(id, "answered"\|"deferred"\|"moot", {answer, answeredBy})`: refuses `answered` carrying neither an answer nor a link, refuses a dangling or non-ID `answeredBy` (writing nothing when it refuses), merges rather than replaces existing links, marks an approved question `amended` (#16), and **refuses reopening deliberately** — nothing authorises it, and doing it silently would leave a stale answer on a live question. | **Stage 4's entire purpose is producing questions and settling them, and until now nothing could write `resolution`.** `reviseArtifact` could reach `resolution` and `answer` but correctly refuses `answeredBy` as a trace field (#101) — so **recording WHAT settled a question had no typed path at all**, and #88 forbids the hand edit. **That is the same defect as #83's missing lifecycle setter and #101's missing unlink: the workflow authorises a state transition and supplies no operation for it.** ⚠️ **Three recurrences is a pattern, and its provenance is worth naming honestly: each was found by NEEDING it, never by auditing for it** — `QST-0010` is the one instance found by looking, and it is still open. **A worklist derived from "which authorised transitions have no operation" would have found all four at once.** ✅ Six tests, and **the two guards were falsified** 2026-08-22 by disabling each in turn — both failed as they should. _(Recorded 2026-08-22.)_ |
| 127 | **Step 7's internal order** — capabilities first, contracts only after they pass | **Three slices, in this order. (1) RESEARCH VERTICAL SLICE:** register the three typed tools · implement the capability signature **and a live backend probe** · add the first adapter · **run one real external question end to end — search → fetch → `evidence(kind: source)`** · and **prove that unavailable search produces a refusal rather than model-memory prose**. **(2) TIER-1 VALIDATION SLICE:** build the controller **and the evidence omission field together** — *the collector's fixtures must justify every schema state* · exercise `provision → execute → observe → destroy` · include **timeout, unavailable fact, redaction, cleanup failure**, and the explicit *"a venv is not containment"* boundary. **(3) SPECIALIST CONTRACTS, only after their capabilities pass:** each contract describes the **measured** tool signatures and limitations, and **#81 rejects any child that cannot prove those signatures.** Then, and only then, step 6 — driven by real stage 5–8 work. | **Each slice ends in a demonstration, not a merge, and that is what makes the order enforceable rather than aspirational.** The two negative proofs are the load-bearing halves: **a research slice that never runs with search switched off has not tested the only clause that prevents #67's failure**, and a controller that never fails cleanup has not tested the result that keeps a destroyed-environment claim honest. ⚠️ **"Build the controller and the omission field together" is the sharpest constraint here**, because it inverts the usual order and fixes #122's original defect at the source: **a schema state written before its collector exists is a state nobody has shown is distinguishable** — exactly #121's error, and the reason #122 deferred the field's shape rather than guessing it. **The fixtures are the justification**; a state no fixture can produce does not go in the schema. ⚠️ **Contracts last is DEC-0003 executed rather than restated.** A contract written against a *planned* signature promises what the host might supply; one written against a **measured** signature promises what it does — and #81's rejection has something real to compare against only in the second case. ⚠️ **`QST-0010` stays legitimate work and does not jump the queue.** The transition audit is a **worklist, not a priority queue** — the same distinction #94 drew for the capability gate, and worth restating because a freshly-written worklist reads like a plan. _(Decided by the PM 2026-08-22.)_ |
| 128 | **The first adapter is Tavily** — and the credential boundary is the one code can hold | **Tavily Search API. Basic depth, `auto_parameters: false`, `include_answer: false`, `include_raw_content: false` — discovery results only. `GET /usage` is the probe: it validates authentication and reports remaining quota WITHOUT performing a search.** **Credential contract:** the PM creates the account and supplies `TAVILY_API_KEY` host-side · injected **only into the research child**, never into planning or validation children · the specialist has **no shell and no generic environment-reading tool** · the extension reads the key but **never places it in prompts, logs, errors or tool results** · and **missing key · failed auth · exhausted quota · unreachable backend each produce a DISTINCT unavailable reason.** ⚠️ **Wording corrected on the way in:** a credential cannot be *"outside anything this project can read"* — **the extension must read it.** The enforceable boundary is that it stays outside project **files**, planning **content**, and **every model-visible interface**. → `DEC-0006`, `QST-0011` answered. | **The plan's behaviour at exhaustion is why this backend and not a bigger one: 1,000 monthly credits, no payment card, and it STOPS.** That puts REQ-0012's ceiling inside the mechanism instead of leaving it to the PM watching a bill — the same move as #125's refusal *before* provisioning. **`GET /usage` is what makes DEC-0003's detectability affordable**: a backend testable only by searching would spend a credit to answer *"are you there"*, so the probe would get skipped, and a probe that gets skipped is #67 again. **The spend-relevant parameters are fixed in the adapter rather than exposed**, because a caller who can set search depth can spend more than was authorised — a control that callers can widen is a suggestion. ⚠️ **The corrected wording is not pedantry; the original was unenforceable.** "Outside anything the project can read" describes no boundary any code could hold, so it would have been satisfied by nothing and checked by nothing. **"Outside files, content, and every model-visible surface" is testable** — and it is now tested: the key is asserted absent from every result and every error on five failure paths. **The distinction is #121's, applied to a secret rather than a capability: state the boundary the host can actually enforce.** _(Decided by the PM 2026-08-22.)_ |
| 129 | **7a's public-web boundary** — `research_fetch` is not "plain HTTP" | **A fetch tool that reaches anything reachable is not a research capability. `research_fetch` rejects non-HTTP(S) schemes, URLs carrying credentials, and loopback / private / link-local / unique-local / CGNAT / multicast / reserved destinations (including IPv4-mapped IPv6), refuses oversized bodies and unsupported media types, and REVALIDATES EVERY REDIRECT.** A name resolving to *any* non-public address is refused, not just one resolving to only private ones. | ⚠️ **I described `research_fetch` as "plain HTTP, needs no credential" and the PM corrected it.** Both halves of that sentence were true and the conclusion was wrong: needing no credential is exactly what makes it dangerous, because **the thing it reaches for free is the host's own network.** `http://169.254.169.254/` is cloud metadata; `http://localhost:5432` is whatever the PM is running. **DEC-0004 scoped the capability to public, read-only research, and a scope nothing enforces is a sentence in a document.** ⚠️ **Revalidating every redirect is the half that is easy to skip and useless to omit**: `redirect: "follow"` validates the URL the caller supplied and then lets the SERVER pick the next one, so the boundary inspects the doormat. ✅ **Found by falsification, worth recording as a method result:** the redirect test passed with the guard removed, because **a stub cannot auto-follow the way the platform does** — so the test proved the loop revalidated and could not see that the loop would never run. The fix was to assert on the **request** (`redirect: "manual"`), which the stub can prove. **A green test that survives the removal of what it tests is the exact failure this project keeps finding**, and it is why the mutation run happens at all. ⚠️ **Residual risk stated, not closed: DNS rebinding.** Addresses are checked, then `fetch` resolves the name again; a hostile authority can answer differently the second time. Closing it needs connect-to-IP with an explicit Host header and SNI, which Node's fetch does not expose. **The boundary raises the cost of an attack rather than eliminating it** — the same shape of admission as #125's *"a venv is not containment"*. _(Recorded 2026-08-22.)_ |
| 130 | **7a is complete** — the research loop ran end to end and produced real evidence | **`AST-0014` ← `EVD-0016`, derived `supported` / `source-supported`.** The whole path ran: `research_search` (discovery, one credit) → **PM chose the authoritative source among five results, two of which were third-party blogs** → `research_fetch` through the public-web boundary → **quote verified present in the retrieved text** → `evidence(kind: source)` written through the typed path with locator and retrieval date → linked with polarity → **verdict and confidence DERIVED, never stored** (#96). The intake's problem 2 — *"the agent cannot research"* — has a working mechanism against it for the first time. | ⚠️ **What it does NOT prove, said plainly.** It proves the loop runs and that a citation can be mechanically tied to text that was really on the page. **It proves nothing about the QUALITY of a judgement** — which source is authoritative, and whether the quote actually bears on the claim, were both the PM's calls, and #123 says bearing is claim-relative and therefore not something the pipeline can settle. ✅ **A flagged risk did not materialise and is recorded as such:** I predicted the Mintlify-style docs might be client-rendered and defeat the quote check. They were not. **Recording a prediction that failed is the same discipline as recording one that held** — the alternative is a memory that only keeps its hits. ⚠️ **The sharp result is the asymmetry it exposed.** `AST-0014` (*what the docs say* — 1,000 credits) is now `source-supported`. `AST-0013` (*what this account measured* — 1500 credits) is still `unresolved`/`unverified`, **not because the observation is weaker but because `QST-0012` makes it unrecordable**: an experiment needs an environment, an environment needs a sandbox tier, and the probe ran under none. **A documented figure is currently better evidenced than a measured one, purely because of a recording gap** — which is the strongest argument yet for settling QST-0012 inside 7b rather than after it. ⚠️ **One defect found by the first real record:** `extractTitle` did not decode entities, so the citation stored `Credits &amp; Pricing`. Fixed and the record corrected — `amends: true`, because `sources` is semantic (#103) and a citation's title is part of what the record says. _(Recorded 2026-08-22.)_ |
| 131 | **The environment model gains two axes** — and there is deliberately no tier 0 | **`evidence.environment` is reshaped: every experiment declares `execution` (`host` \| `controller`) and `facts`; **`sandboxTier` is required ONLY when a controller supplied the isolation, and is FORBIDDEN on a host run**; a controller run must also carry an `isolationBoundary` (`isolates` / `doesNotClaim`) so the boundary travels WITH the record. `omissions[]` carries the four states from DEC-0005 — `not-captured` · `not-observable` · `unavailable` · `redacted` — **each requiring a reason**; unrequested facts stay simply absent. **Shape settled by two fixtures written BEFORE the schema changed**, on the PM's instruction: a direct host run and a controller-managed tier-1 run. | **A "tier 0" was the obvious fix and it was the wrong one.** Putting host execution at the bottom of the isolation ladder reads as *"the weakest isolation"*, when the truth is *"isolation is not a question this run has an answer to"* — **a rung implies a comparison, and there is none.** Two axes say it exactly: `execution` records who ran it, `sandboxTier` records what isolation was supplied, and **the absence of a tier is meaningful because `execution: "host"` explains it.** ⚠️ **The load-bearing constraint is the prohibition, not the permission.** Letting a host run omit the tier would merely PERMIT honesty; forbidding it from carrying one makes *"no isolation claim"* structural rather than a convention someone remembers. ✅ **Both fixtures were required, and each justified states the other could not:** the host run produces `not-captured` (no collector existed); the controller run produces `unavailable`, `not-observable` and `redacted`. **A state no fixture can produce does not go in the schema** (#127), and one fixture would have justified only a quarter of it. ⚠️ **The controller fixture is provisional until the controller emits it** — it is a claim about what will be produced, and the controller's own tests are what turn it into a fact. _(Recorded 2026-08-22, on the PM's instruction to pressure the model before settling it.)_ |
| 132 | **Migration 2, and the fourth missing operation** | **The reshape broke 12 stored records, all carrying `tier: 1`. `bin/migrate-content.mjs` migrates them to `execution: "host"` with no tier, bumps `schemaVersion` to 2 (#50), and **appends to each record what it used to carry and why it no longer does**. Dry run by default; `--apply` writes, under the lock, atomically, and **validates everything before writing anything**. It **REFUSES** a tier 2 or 3 record rather than guessing.** | **Dropping the tier removes a value those records carried, and that needs a justification rather than a shrug: their `tier: 1` was never an observation.** No validation controller has ever existed on this project, so every one of those runs executed directly on the PM's machine — **tier 1 was simply the only value the old schema would accept.** Keeping it would preserve a claim of isolation no run ever had. ⚠️ **But "preserve the original observation exactly" cuts both ways**, so the dropped value is **recorded in each record's notes, not erased**: a migration that silently rewrote history is the same defect as one that invented data. ⚠️ **Validate-all-then-write matters more than it looks:** a migration that writes six records and then finds the seventh invalid leaves the content in a state no schema describes — worse than not running. ⚠️ **And this is the FOURTH missing typed operation**, after #83's lifecycle setter, #101's unlink and #126's question resolver. #50 assumed migration existed; `reviseArtifact` correctly refuses `schemaVersion` as identity; so a schema change had **no legitimate route to existing records at all**. ⚠️ **Found by needing it. Again.** Three of the four were. _(Recorded 2026-08-22.)_ |
| 133 | **`AST-0013` recorded from the original observation, and the asymmetry closes** | **`AST-0013` ← `EVD-0017`, deriving `supported` / `environment-matched`. Recorded from the ORIGINAL probe output, four days of decisions later, and deliberately NOT re-run.** Facts: exactly the five the output carried. Everything else — Node version, OS build, plan name, timing — is an `omission` with `not-captured` and a reason. | ⚠️ **The PM's constraint is the whole point: a fresh probe would observe a fresh moment.** By now at least one search credit has been spent, so a re-run reporting different numbers **could not be evidence of the 1500/1500, 0-used event** — it would be a later observation wearing an earlier date. **Later understanding must not retroactively enrich what an earlier observation recorded**, which is the same rule that governed QST-0001's `EPERM` finding. ✅ **The asymmetry #130 identified is now closed**: what the docs SAY is `source-supported`, what the account MEASURED is `environment-matched`. **7a proved the observation pipeline works for sources; 7b's first job was making experimental observations equally representable without granting them containment they never had** — and the reshape did exactly that: `EVD-0017` reaches the top rung while carrying **no isolation claim at all**. ⚠️ **Which immediately exposed `QST-0013`:** nothing caps the rung by isolation level. #56 and DEC-0005 both say a tier CAPS the reachable rung, and the derivation never implemented it — `environment.tier` was carried and never read. **Pre-existing, not introduced here; #131 made it visible by removing the value that looked like it was doing the work.** _(Recorded 2026-08-22.)_ |
| 134 | **Tier is a permission, not a rank** — isolation becomes claim-relative, and the old ceiling is amended away | **There is NO global isolation-to-confidence cap.** Sandbox tier describes **how execution was provisioned and what boundary it supplied**; confidence describes **how well applicable evidence matches a particular claim**. `isolationBoundary` enters the same claim-relative comparison as every other environment fact, through the assertion's new `requiresIsolation`: **required and matched** → may reach `environment-matched` · **required but absent** → `unknown`, applicable and capped · **required and incompatible** → the evidence **does not apply** to that claim · **not required** → **no penalty at all**. → `DEC-0007`, `QST-0013` answered. **The old "tier determines the confidence ceiling" language is AMENDED, not implemented** — in #77's third consequence and in *"Which tier can reach which rung"*. | **`QST-0013` looked like a missing implementation and was a category error preserved in the wording.** Building the cap would have been wrong in **both** directions: it would have penalised `AST-0013` — a claim about the PM's host, evidenced by a run on that host, where the absence of isolation is not a shortfall but an **irrelevance** — and it would have credited a tier-1 run for containment tier 1 **explicitly states it does not provide.** ⚠️ **The neutral state is the part most easily got wrong.** `not-required` must not collapse into `unknown`: doing so **reintroduces the global penalty in different words**, capping every claim that never asked about isolation. It is a separate value, and the mutation test that proves it fails two assertions when it is removed. ✅ **DEC-0005's `doesNotClaim` list becomes load-bearing rather than decorative:** an explicit denial is a **contradiction** of a requirement, where silence is only `unknown` — which is what turns the rung table's NVIDIA-driver example from advice into an exclusion the code performs. ⚠️ **And the machine-dependence #77 warned about survives in a narrower form:** it bites only on claims that actually turn on isolation the machine cannot supply. **The tier-2 recommendation should say so** — not *"without Docker nothing exceeds rung 2"*, but *"without Docker, these specific claims cannot be settled here."* _(Decided by the PM 2026-08-22.)_ |
| 135 | **The tier-1 controller** — and the destroy path as the design rather than the epilogue | **`provision → execute → observe → destroy`, built against the PM's seven acceptance conditions. Every path — refused, provision failure, execution failure, success — reaches a **recorded destroy attempt**. `destroy.outcome` is `destroyed` only when the workspace is **verifiably absent afterwards**; otherwise `retained`, with the path that still exists and `evidencePreserved: true`. Commands run as argument arrays with `shell: false`, under an **allowlisted** environment rather than an inherited one. Every record carries tier 1's `isolationBoundary`, so *"a venv is dependency isolation, not containment"* travels WITH the evidence.** ✅ **Cleanup failure is FORCED, not simulated:** a live process whose CWD is the workspace, which Windows genuinely refuses to delete — `EPERM`, retained, and it destroys cleanly once the holder exits. ✅ **The real path emits all four omission states from four different causes:** `timing` disabled by the plan → `not-captured` · `cpu-model` with no collector → `not-observable` · `git-commit` in a non-repo → `unavailable` · a credential obtained then suppressed → `redacted`. | ⚠️ **The sharpest result came from falsification, and it exposed a hole in the tests rather than in the code.** Replacing the entire destroy call in `runJob` with a hardcoded `{outcome: "destroyed"}` **passed every test in the file.** They all read the RECORD — which is exactly what a lying implementation controls. **A suite that only inspects the report cannot distinguish a destroyed environment from a claimed one**, which is the precise failure the acceptance condition was written against, reproduced inside the thing meant to prevent it. The fix is the only assertion a false report cannot satisfy: **run the job into a known base directory and check the disk is empty afterwards.** ⚠️ **`rmSync` reporting success is not evidence of removal** — with `force: true` it can return without throwing and leave the directory behind — so the outcome is decided by `existsSync`, never by the absence of an exception. ✅ **Fixture B is now AUTHORITATIVE, rewritten from a real run, and reality corrected it twice:** the redacted fact is `env:TAVILY_API_KEY` (the fixture invented a name without the `env:` prefix), and there was a **fourth** omission the draft missed — `timing`, `not-captured`, the state that comes from the capture plan itself. **Neither correction was large, and that is the argument for the condition:** a fixture written from a design is plausible everywhere and wrong in the details, and the details are what a schema is made of. _(Recorded 2026-08-22. 189 tests; seven guards mutation-tested, one of which found the missing test.)_ |
| 136 | **Stage 3, reconstructed** — a provenance record that deliberately does not close its own gate | **`planning-content/stages/03-discovery.md` exists, labelled RECONSTRUCTED and NOT gate-complete.** Stage 3 genuinely ran **twice** — experimentally for `QST-0001`, and through source research plus a probe matrix for `QST-0004` — and until now that work was visible only by traversing questions, assertions and evidence. The document inventories both runs, traces every artifact, explains that **the apparent source contradiction was resolved by ENVIRONMENT SCOPING rather than by choosing a side**, and records that **no `research-finding` was needed**. **Neither exit criterion is attested**, and the gate stays `ready: false`. | ⚠️ **The refusals in it are what make it worth having.** It does **not** manufacture a `research-finding` to satisfy `produces[]` — the `gate/type-not-implemented` finding is accurate and should stand, because a stage definition is **authoritative (#90), not correct (#91)**, and creating an artifact to silence a gate inverts #106. It does **not** attest either criterion: **a criterion attested from a reconstruction is attested from a document rather than from a run**, and #93 gives its three verdicts to a PM who has LOOKED. **A gate that turned green because someone wrote a summary is the clearest possible form of the failure this project exists to prevent.** ✅ **`QST-0002` survives unresolved, which was the point of the exercise.** The contradiction stage 3 met was **apparent** — synchronous versus awaited, dev versus build — and scoping dissolved it. What would actually bear on `research-finding` is a **genuinely irreconcilable conflict between sources describing the SAME configuration**, and that has not been encountered. ⚠️ **One claim in the draft was corrected against the derivation before it was written down:** `AST-0007` is `contested`, not `refuted` — and contested **because it is under-scoped**, which is why the fix was to write `AST-0010` and `AST-0011` beside it rather than to repair it. Every one of the 24 artifact IDs the document cites was checked to resolve. _(Recorded 2026-08-22 on the PM's instruction.)_ |
| 137 | **`AST-0010` is deferred, not owed** — a registered validation task with a trigger | **`DEC-0001` makes it blocking only when a production build enters scope, and the product is local and runs `next dev`. Running it now would validate an UNDECLARED deployment mode.** Registered under #80 branch 3 with an explicit **reopening condition: *execute before adopting `next build` / `next start`, or when upgrading beyond the currently tested Next.js version (16.3.1 / React 19.2.8 / Node v24.18.0)*.** The condition lives on the assertion, so it is discoverable without the narrative. ⚠️ **The reconstructed stage 3 document said *"still owed an experiment"*, which was too strong; corrected to *"validation is deferred until its deployment mode enters scope"*.** | **The wording mattered because "owed" and "deferred" schedule differently.** *Owed* reads as debt and pulls work forward; *deferred with a trigger* is #80 branch 3 exactly — real, not blocking, and recorded with the condition that reopens it. **`AST-0010` currently advances no in-scope commitment**, and validating an undeclared deployment mode is demand-driven sequencing inverted: the experiment would arrive before the decision that needs it. ⚠️ **The trigger fires when the risk becomes POSSIBLE, not when it is observed** — #80's stated preference, because waiting for observation means shipping the failure once to learn it was real. ⚠️ **One consequence in the stage document had to be re-derived rather than left alone:** its `unknowns-resolved` row said the unknown was *"neither answered nor raised as a validation task"*, which registration made false. It now reads that the criterion **may be substantively satisfied** while still not being attested — because the disposition is real and the attestation would not be. _(Decided by the PM 2026-08-22.)_ |
| 138 | **7c — specialist contracts written against measured signatures**, and the credential boundary becomes a mechanism | **Three contracts (`research` · `planning` · `validation`) built by REFERENCE to the signature tables, never by copying them — a test asserts the contract holds the SAME OBJECT. `verifyChild` accepts output only after **stdin delivered → not timed out → capabilities demonstrated → signatures matched**, in that order, and every failure is a structured refusal that **does not pass the output through**. `childEnv(role)` is the injection point `DEC-0006` named and nothing implemented: `TAVILY_API_KEY` reaches the research child ONLY, proven by **spawning real children and asking them what they can read**. Write boundaries are per-role allowlists over the typed tools. Building it exposed and fixed one defect: the validation contract required `validation_controller`, **a name with no signature anywhere**, so #81's check would have compared the child against an empty object and passed — now `lib/validation/tools.mjs` publishes `validation_capability` and `validation_run`, and `contractFor` REPORTS any capability nothing defines.** | ⚠️ **"A verification that cannot fail is not a verification"** — the validation contract was in that state and nothing said so, because the signature loop silently skipped names it could not find. **Silently skipping is how a check becomes decoration**, and it is the same shape as #94's capability gap one level up. The fix reports `undefinedCapabilities` at build time rather than at use time. ⚠️ **The credential test spawns processes on purpose.** Asserting on `childEnv`'s return value would prove the function builds an object; **"the planning child cannot read the key" is a claim about a process**, and only a process can answer it. The same test catches inheritance: no child sees `AWS_SECRET_ACCESS_KEY`, and each has fewer than twenty variables. ⚠️ **A refusal must not hand back the output**, and that is tested. A refusal that still returns the text is an invitation to use it — the fallback-to-prose failure wearing a warning label. ✅ **Ten guards mutation-tested, all caught**, including dropping a `required` field while leaving properties intact — a signature change that a shallower comparison would have missed. ✅ **#79 does NOT reopen: 7c defines contracts and spawns nothing concurrently.** Checked rather than assumed — there is no orchestration in `lib/specialists/`. **The trigger to reopen it is the first time two specialists run concurrently over overlapping dependency sets**, which is #79's own condition and remains unbuilt. _(Recorded 2026-08-22. 199 tests.)_ |
| 139 | **Stage 5 run for real** — it demanded a type the catalogue lacks, and none of the three it declares | **Run 2026-08-22 against the 14 active requirements and the code that exists.** Twelve requirements trace to a component; **REQ-0010 traces to none and REQ-0011 to half**, both the handoff. Storage target and data model **attested `satisfied`** — they were already chosen and running. The third criterion, *all requirements trace to a component*, is **`not-satisfied`**. ✅ **Demand produced: `component`** — passes #41 with **four already-decided consumers**: #60's cascade (its worked example is `acceptance-criterion → component`), #19's role slicing, the stage-9 orphan check (*"a requirement no component satisfies"*), and **stage 5's own exit criterion, which is unevaluable without it**. ❌ **Demand produced for `schema`, `api-spec`, `wireframe`: none.** → `QST-0014`, **deliberately not built pending the PM's answer.** | ⚠️ **The strongest part of the case is that the gap is OLD.** `notes.md` names *"design component"* three times in the traceability chain — *requirement → design component → assertion + evidence → acceptance criterion → task → runbook step* — and **#38's sixteen do not include it.** So the chain that #16, #19, the stage-9 gate and #25 all rest on **has a missing link**, and nobody noticed **because nothing had run stage 5.** Running a stage found a hole that reading the catalogue could not. ⚠️ **The three declared types were each refused for a DIFFERENT reason, which is why this is evidence rather than a verdict on the table.** `schema`: the schemas are consumed directly by four modules, and an artifact wrapping them would add only trace edges — **which is the `component` edge again**. `api-spec`: the real interface contracts are the signature tables, already machine-readable and already traversed, and **#138's whole fix was to stop copying them into a second document** — modelling them as artifacts would recreate that defect by design. `wireframe`: **a design for a UI nobody has committed to**, which is #137's error in another costume. ⚠️ **Two stages have now disagreed with the draft table** (#136 was the first), and the honest reading is **not** that the table is wrong: `produces[]` describes **a project TYPE, not a stage**, and it was transcribed as though it described a stage. A consumer project building a real service would plausibly need all three. ⚠️ **And the run exposed something about this project rather than the methodology: stage 5 CONFIRMED a storage target and data model it was supposed to design**, because the substrate was built before the stage that designs it ever ran. _(Recorded 2026-08-22.)_ |
| 140 | **`component` joins the catalogue as `CMP`** — the first type added because work demanded it | **Approved by the PM and activated on this project (`DEC-0008`). Minimal shape: a named design unit, a `responsibility`, `satisfies` links to requirements, the standard envelope, plus advisory `implementedBy` paths. Further links wait for authentic stage 7–8 demand.** ✅ **And the semantic correction that came with it: a stage's `produces[]` is a CAPABILITY ENVELOPE across project types; the applicable set for a project is `produces[] ∩ activatedTypes`.** Activation describes the project type; the stage definition owns where an applicable type is produced. **`component` was added to stage 5; `schema`, `api-spec` and `wireframe` were RETAINED** — #139's run showed them inapplicable here, **not invalid globally**. #34 and #90 amended so `produces[]` no longer reads as *"every run emits all of these."* | **The reframing is wording catching up with behaviour, not a change of behaviour**: `lib/lint.mjs` already skipped unactivated types (*"a stage cannot demand an unactivated type"*), so the gate had been computing the intersection all along while the prose said something stronger. **Two stage runs found declared types legitimately unused** (#136, #139) and neither was evidence the table is wrong — it is evidence the table describes **a project type**. ⚠️ **Approving `component` promoted `QST-0010` from deferred to prerequisite**, and that promotion is the useful part: it was non-blocking only while no activation change was pending, and **hand-editing the manifest once more would have been the fifth instance of #126's pattern committed knowingly.** `lib/tools/activate-type.mjs` now validates **catalogue membership · authorability (schema AND typed tool) · stage reachability**, writes only the activated list, stamps who approved it, and **refuses to deactivate a type whose artifacts exist** so #107's silence cannot begin with a manifest edit. It never touches `stages/` or `schemas/` — tested. ✅ **The orphan check now runs**: ten components, and **exactly one orphan, REQ-0010**, matching what the prose claimed — *evaluable* where it was previously only assertable. ⚠️ **REQ-0011 showed the limit of the mechanism**: `CMP-0010` satisfies it so the check is silent, but it renders and nothing exports. **An orphan check finds absent edges, not partial ones**, and that belongs in the attestation rather than in the tool. ⚠️ **Two falsification results worth keeping.** Removing the typed-tool guard broke **no** test — every type exercised had both a schema and a tool — so a case was added using `api-spec`, which has a schema and no way to author one. And `loadStageDefinitions` takes the TOOL ROOT, not a stages directory: passing the wrong one returned `null`, which **crashed instead of refusing**. A missing definition set must refuse, because #90 makes `stages/` the authority and an absent authority cannot approve. _(Decided by the PM 2026-08-22. 209 tests, 79 artifacts.)_ |
| 141 | **The handoff design slice** — one slice, three separate decisions, and the planned component that exposed a hazard | **`QST-0007` → `DEC-0009`: the v1 export exists and is produced by `npm run handoff`, an explicit deterministic command. **The gate stays READ-ONLY**; generation is never a gate side effect and never a freeform agent task. `QST-0008` → `DEC-0010`: **the authoring skeleton is not the delivery surface.** Canonical JSON serves agents; generated Markdown and `PLAN.md` serve humans; an interactive recipient site needs separately demonstrated demand. `QST-0003` → `DEC-0011`: **`runbook` is not retained for v1** — the real handoff needs none of whole-run preconditions, abort criteria or cross-step rollback — so it is **deactivated here and KEPT in the catalogue** with a reopening condition. ✅ **`CMP-0011` designed BEFORE it exists**, and the orphan count reached **zero**. | **Separating the gate from the generator keeps two failure modes apart.** A gate that generated as a side effect **could not be run to ask a question** — every check would rewrite the repository, and #46's *"warn continuously"* becomes *"rewrite continuously"*. A freeform agent handoff fails oppositely: two runs differ, and **a package that cannot be reproduced cannot be reviewed.** ⚠️ **`QST-0008` was mis-framed and the decision says so** rather than answering it as asked: sufficiency should be judged of the GENERATED package, not of the authoring tool. Reading REQ-0011 as *"therefore build an app for the recipient"* inflates a requirement about legibility into a commitment about product — and a live app shows CURRENT state where a handoff must show APPROVED state. ⚠️ **`runbook` was decided against the real handoff, not against the idea of one:** `runbook-step` already carries `remediation`, `destructive` and `dependsOn`; membership reverses `partOf` (#84) and ordering is `ordinal` + `dependsOn`, so the aggregate is a **projection at handoff**. Cross-step rollback is the strongest of the three and genuinely has nowhere to live — **but nothing here needs one**, and #106 says built-on-demand, not built-on-plausible-use. **Catalogue removal was deliberately NOT performed**: that is #107's three-authority act, and one thin handoff shows the type inapplicable HERE, not wrong. ⚠️ **`CMP-0011` created a hazard and it is recorded as `QST-0015` rather than glossed:** the orphan check counts **edges**, so *"all requirements trace to a component"* is now true while *"the handoff exists"* is not. The attestation carries the distinction until something better does; a planned/built axis was **raised, not built**, because the PM's shape for `component` was explicitly minimal. ✅ **And the lint caught me the same minute**: `QST-0015.raisedBy` pointed at a component, which `question.raisedBy` did not permit. **The exclusion was staleness, not a rule** — the list predates the type — so it was widened where demand actually appeared, and `blocks` was left alone because nothing yet blocks a component. _(Decided by the PM 2026-08-22. 209 tests, 84 artifacts.)_ |
| 142 | **`npm run handoff` exists, and its first job is to refuse this project** | **`CMP-0011` implemented (`bin/handoff.mjs`, `lib/handoff/*`).** Publish order: **lock → re-evaluate completeness → snapshot input → render to temp → validate the WRITTEN files → atomic swap → release.** ⚠️ **Completeness is not a clean lint.** It also requires that every declared exit criterion is **attested**, and none attested `not-satisfied` — `n/a` passes, because #93 gives that verdict to a PM who LOOKED. **The real project is refused today: 11 blockers, and nothing is written.** Deterministic by construction — sorted keys, no wall-clock anywhere, identity is a **content hash of the package itself**. Contract conflicts amended: **#19 loses `site/` and the aggregate `runbook.json`**, role slices move to stage 8, and `docs/plan/README.md` moved out to `docs/handoff-contract.md`. | ⚠️ **The authored README was INSIDE the directory the generator replaces** — a contradiction that would have deleted itself on the first successful run. Found by asking what the swap actually does. ⚠️ **The gate and the generator are separate for a reason that survives restating: a gate that generated as a side effect could not be run to ASK a question.** Every check would rewrite the repository. ✅ **The negative control is the dogfood project and it is the decisive test** — it lints clean and must still be refused, because a publish gate only ever shown succeeding has not been shown to gate. ⚠️ **Falsification found FOUR holes in the tests, not the code, and every one was a test that could not fail:** (1) the swap's restore branch had never once restored anything — nothing could make the first rename succeed and the second fail, so it was extracted with an injectable `rename`; (2) nothing made the on-disk package differ from what was rendered, closed with a **real case-only path collision** rather than a stub, which is a genuine hazard on this filesystem; (3) the deactivated-type test had **no artifact of a deactivated type** in its fixture, so the guard could be deleted with everything green; (4) `canonicalJson({b, a})` **reverses into sorted order**, so an implementation that reversed instead of sorting passed — two keys cannot distinguish sorting from reversing. ✅ **And one defect in the code, found the same way:** the renderer emitted a file only for types that HAD artifacts, collapsing *"this type does not apply here"* into *"there are none"*. Every activated type now gets a file, empty if empty; a deactivated one gets none. ⚠️ **`QST-0015` stays open**: `implementedBy` pointing somewhere proves code exists, **not** that the component satisfies acceptance criteria nobody has written yet. _(Recorded 2026-08-22. 225 tests, 84 artifacts; eight guards mutation-tested.)_ |
| 143 | **Five stages run for real** — and the catalogue was judged five times by demand | **Stage 5 re-attested `satisfied` (every active requirement now traces to a component). Stages 3, 6, 7, 8 and 9 EVALUATED, not bulk-attested — each against real state, each producing a finding.** ✅ **`npm run handoff` now refuses on ONE blocker instead of eleven**, and the blocker names a missing capability rather than an unexamined project: stage 9's `role-slice-self-sufficient`, because `task` is unbuilt so no slices exist so no recipient can start. **That is REQ-0010 itself.** ⚠️ **Demand results: `acceptance-criterion` PASSES #41 (stage 7) · `task` PASSES (stage 8) · `role-assignment` FAILS · `risk` FAILS · `research-finding` still undecided.** None built — all are catalogue acts and stage 7 is `decidedBy: User`. | ⚠️ **`role-assignment` failing is the sharpest result, and it was already decided rather than newly argued.** #18 says roles are **discovered from task assignments** and there is **no roster to maintain** — so the assignment is a PROPERTY of a task (one `role` string) and the set of roles is derived by collecting them. **A `role-assignment` artifact would store what a traversal derives**, which is now the third refusal of that exact shape: #138 stopped copying tool signatures into a second document, #141 refused the runbook aggregate, and this refuses the roster. ⚠️ **`risk` failed for a different and weaker reason: it has exactly ONE consumer, and it is the stage-6 criterion demanding it.** A type whose only consumer is the criterion that asks for it is the catalogue justifying itself. Compare `component`, which had **four consumers decided before it was proposed**. The prose register stays, and the cost is stated in the attestation rather than hidden: that criterion is evaluable only by a human reading a table, where stage 5's is now mechanical. ⚠️ **Stage 7 answered `QST-0015` by dissolving it: planned-versus-built is THREE questions with three existing homes** — does code exist (`implementedBy`), does it work (acceptance criteria), does the plan claim it is finished (the attestation). **A `state` field would answer none of them well and would go stale**, since `built` is derivable and a stored derived value can lie (#96). ⚠️ **Two findings recorded rather than fixed.** Stage 6: `load-bearing-assertions-at-rung` **can pass vacuously** — `RBS-0001` rests on `AST-0002`, which is not flagged `loadBearing`, so a criterion whose scope is an author-supplied boolean is satisfiable by not setting the boolean. Stage 9: **"critical" is undefined**, the second instance of the drafting defect stage 4 hit with "blocking", which took `DEC-0002` to resolve. ⚠️ **And one asymmetry worth naming: `runbook` was deactivated and `research-finding` was not.** The difference is that a DECISION was reached about the first (`DEC-0011`) and `QST-0002` is still open about the second — so stage 3's `gate/type-not-implemented` finding **stands accurate**, and deactivating to silence it would tidy away a true report. _(Recorded 2026-08-22. 225 tests, 84 artifacts.)_ |
| 144 | **`acceptance-criterion` and `task` built; the handoff PUBLISHES** | **Both approved and activated through `setTypeActivation`, in the PM's order — criteria are upstream of tasks in the real chain. `acceptance-criterion`: statement · `evaluates`→component · `verifies`→requirement · `outcome` (`not-evaluated` \| `pass` \| `fail`) · optional `evidencedBy`. `task`: statement · **`role` as a STRING** · `implements`→component · `fulfils`→requirement · `acceptedBy`→criterion · `dependsOn`→task. **Role slices are generated as a QUERY** over `task.role`, pulling each task's requirements, criteria and components into the slice, plus `blockedByOtherRoles`. ✅ **`npm run handoff` published: 23 files, 99 artifacts, and the same snapshot twice.** ✅ **`QST-0015` closed by `DEC-0012`; `risk` stays inactive by `DEC-0013`; stage 9's `critical` defined by `DEC-0014` and the criterion text amended.** | ⚠️ **`outcome` is a RECORDED JUDGEMENT and not a derivation, which is a deliberate break from #96 rather than an oversight.** An assertion's verdict is derived because an evidence graph determines it; a criterion's outcome is determined by **somebody running it and looking**. Deriving it would need the polarity machinery assertions already have, and would make an acceptance criterion a **second, weaker assertion**. ⚠️ **`not-evaluated` is a real state and it earned itself immediately**: `ACC-0010` was written **before** the code and sat there, which is precisely the fact `QST-0015` wanted a boolean for — **the state existed as data before anyone added a field for it.** ⚠️ **The test that was this project's negative control PASSED, and it was rewritten rather than deleted.** It refused through three successive real blockers — eleven unattested criteria, then stage 5's traceability, then stage 9's slices — **each cleared by building the missing thing, never by relaxing the predicate.** It now asserts readiness AND that no criterion is merely pending, because *"ready because nobody looked"* reports the same boolean as *"ready because everything was checked"*. ⚠️ **Two honest limits on the publish.** This project has **exactly one role**, so slicing is exercised while the **cross-role case — the one the criterion is really about — is covered by a constructed fixture rather than by real content.** And `ACC-0001`…`ACC-0009` were written by the same agent that wrote the code they accept, which is weaker independence than a separate reviewer. **Both are in the attestation rather than in a footnote.** ⚠️ **`DEC-0013` names the rule the `risk` refusal turned on: a stage criterion demanding a type is CIRCULAR demand.** Not a count of consumers — whether any consumer exists **independently of the demand**. _(Decided by the PM 2026-08-22. 229 tests, 99 artifacts.)_ |
| 145 | **The first real package's three semantic gaps, fixed — and a fourth found while fixing them** | **All four PM findings closed. (1) `task` status is now DERIVED from `acceptedBy` — `accepted` \| `outstanding` \| `unaccountable` — and a slice separates work REMAINING from work already finished, so a recipient cannot redo it. **The publish gate REFUSES an active task with no acceptance criteria**, which is not the same as an outstanding one: an outstanding task has a finish line nobody has crossed, and that one has no finish line at all. (2) **Retired artifacts leave the current plan and get their own labelled section** in `PLAN.md` — dropped from the sections, never dropped from the document. (3) `CMP-0011`'s present-tense narrative converted to dated history plus a CURRENT STATE line. (4) **`DEC-0015`: stage attestations approve the PACKAGE, `reviewStatus` approves the ARTIFACT, and executable content — `runbook-step`, `task` — needs both.** The MANIFEST states the basis and carries the review counts. ✅ **Republished, snapshot stable across runs.** | ⚠️ **Every one of these was invisible to fixture testing and visible in the frozen output**, which is the strongest argument yet for #142's negative control being necessary but not sufficient: **the fixture had no tasks, no retired artifacts and no stale narrative, so three defects had nothing to fail against.** Real publication is a different test from a passing suite. ⚠️ **`DEC-0015` draws the line at EXECUTABILITY rather than importance**: a draft question is a note; a draft runbook step is an instruction someone follows. Requiring approval everywhere would demand sign-off on exploratory questions — the acknowledgement fatigue the risk register already names — and requiring it nowhere would hand a recipient an instruction nobody reviewed. **It bit immediately**, as predicted in its own notes: `RBS-0001`, `TSK-0001` and `TSK-0002` were all `draft`, so the handoff refused until they were reviewed. ⚠️ **Two more missing operations, found by needing them — the seventh and eighth.** `linkTrace`/`unlinkTrace`, because `acceptedBy` is a trace field and `reviseArtifact` refuses those by design (#101) while `linkEvidence` only knows two fields — **and it REFUSES the fields that have bespoke operations, which is what keeps #101 true**. And `setReviewStatus`, which is the only one that already had a **working implementation in the wrong layer**: it lived in `app/server.mjs`, so the only way to approve an artifact was through a running web server. **Moved, not rewritten** (#47). ⚠️ **And a fourth finding, from republishing rather than from review:** the snapshot **did not move** when `MANIFEST.json` changed materially. `README.md` and `MANIFEST.json` both EMBED the snapshot so neither can be inside it — that is now stated rather than left to be discovered — but the residual is real and is `QST-0016`: **`toolVersion` is `0.0.0` and nothing makes it move**, so a renderer change can produce a different package with the same identity. _(Recorded 2026-08-22. 234 tests, 103 artifacts.)_ |
| 146 | **The snapshot becomes a normalized whole-package hash; RBS-0001 retired; and a closure that had not closed** | **`DEC-0016`, to the PM's specification: render every file with a fixed placeholder where the snapshot goes, hash the ENTIRE tree — `MANIFEST.json` and `README.md` included — over canonical path names plus bytes, then substitute. Verification normalizes the same way before recomputing. **`toolVersion` becomes provenance and carries no part of identity.** ⚠️ **`RBS-0001` approval REVOKED and the step retired**: *"launch specialist children concurrently"* names no command and no remediation, no orchestration path exists, and real concurrent orchestration would **reopen #79**. Stage 9's runbook criterion re-attested **`n/a`** — not `satisfied`, because an empty set clears every bar vacuously and that reads as an assurance nobody earned. ✅ Published, verified from its own bytes (`recorded === recomputed`), stable across runs. | ⚠️ **The finding that matters most is that I reported a fix that had not happened.** `CMP-0011`'s notes still ended *"The command REFUSES this project today"*, in the frozen output, after I had reported the narrative corrected. **My check was `!notes.includes("PLANNED, NOT BUILT — 2026")` — the string I had just edited.** It passed because I edited it, and it could not have failed. **A verification that tests the edit instead of the claim is the same defect as a test that cannot fail**, and it is the fourth time this session that shape has appeared. The discipline that would have caught it is the one the fix script used: **assert the offending text exists BEFORE rewriting it**, so a missed occurrence is an error rather than a silent pass. ⚠️ **The hashing defect was found the same way the last one was — by republishing and watching a number that should have moved.** Excluding the two self-referential files was a defensible-looking way to avoid a circular reference, and it cost exactly what exclusions cost: two materially different packages under one identity. **Normalization removes the self-reference without excluding anything**, which is strictly better and was not obvious until the PM named it. ✅ **A generator fingerprint was considered and refused as unnecessary**: once the OUTPUT is fully hashed, a renderer change that alters nothing should not change the package's identity — and a fingerprint would say it did. ⚠️ **Verification must normalize or it can never agree**, and a test asserts the unnormalized recompute does NOT accidentally match. ⚠️ **Retiring `RBS-0001` immediately exercised the retired-section rendering** built one commit earlier, with a real artifact rather than a fixture — it is out of the runbook section and named under *Retired and superseded*. _(Recorded 2026-08-22. 237 tests, 104 artifacts, six guards mutation-tested.)_ |
| 147 | **`TSK-0002` implemented and accepted** — stage 6's scope now comes from the graph, and the vacuity MOVES rather than vanishing | **`lib/stage-scope.mjs`: the set stage 6 checks is DERIVED from `restsOn` on active instruction-bearing artifacts, and the module never mentions `loadBearing` — asserted by a name check, because a scope that quietly still read the flag would look identical from outside.** `ACC-0011` and `ACC-0012` both evaluated `pass` from the tests, so **`TSK-0002`'s status derives to `accepted` (2/2)** and the slice shows **no outstanding work**. Stage 6's criterion re-attested **`n/a`** on the measured live scope, which is **empty**. Republished: `5979e738b81e0e1b`, self-verifying, stable. | ⚠️ **The finding worth keeping is that fixing it did not remove the vacuity — it relocated it, and the new location is honest.** With `RBS-0001` retired nothing rests on anything, so the criterion is *again* satisfiable with nothing checked. But that is now **a fact about the plan**, visible in the artifact graph, where before it was **a fact about whether someone remembered to set a boolean**, visible nowhere. **A criterion that is vacuous for a reason you can read is a different object from one that is vacuous for a reason you cannot.** ⚠️ **`ACC-0012` had to be constructed, and the reason is almost funny: `RBS-0001` — the very step whose unflagged premise produced the finding — was retired two commits earlier, so the live graph no longer contains the case its own criterion is about** (#117 exactly: constructed fixtures prove branches, live state proves wiring). ⚠️ **The mutation run used the amended #80 discipline for the first time**, and reported both halves rather than a bare CAUGHT: each mutation was checked to have actually applied (an unmatched anchor now prints `INVALID`, not `MISSED`) and the failing test NAMES were printed to confirm the suite failed for the intended reason. All three guards caught, each on its own test. ✅ **And `TSK-0002` closes the loop the register opened**: stage 6 was run, found a criterion that could pass vacuously, recorded it rather than fixing it in place, carried it as a task with objective criteria written before the code, and the task's completion is now DERIVED rather than declared. _(Recorded 2026-08-22. 242 tests, 104 artifacts.)_ |
...
| 19 | Handoff package | `docs/plan/` committed into the real project repo: **`data/` + `docs/` + `site/` + `PLAN.md`**. Full package **plus generated per-role slices**. | MDX is an authoring format, not a delivery format. Slices are a *query* over the task graph, not separate authoring. See "The handoff package" below. ⚠️ **AMENDED 2026-08-22 by #142, and the amendment REMOVES two things this row promised.** **`site/` is gone**: `DEC-0010` decides the delivery surfaces are canonical JSON for agents and generated Markdown plus `PLAN.md` for humans, and an interactive site now requires **separately demonstrated demand** rather than arriving by default. **The aggregate `runbook.json` is gone**: `DEC-0011` does not retain the `runbook` type for v1, so the package renders runbook STEPS and no aggregate — membership reverses `partOf` and ordering is `ordinal` + `dependsOn`, which makes the aggregate a projection nothing needs to store. ⚠️ **And per-role slices move to stage 8**, where the task graph they query actually exists: `task` and `role-assignment` are unbuilt, so a slice today would filter an empty graph. **The slice-is-a-query claim survives intact** — it is the timing that was wrong, not the design. _(Amended 2026-08-22.)_ |
...
⚠️ **AMENDED 2026-08-22 by #134 — the mechanism is not a tier lookup.** *"The tier caps the confidence
rung"* was the original phrasing and it conflated two axes. **Sandbox tier describes how execution was
provisioned and what boundary it supplied; confidence describes how well applicable evidence matches a
PARTICULAR claim.** Tier is a **permission and an execution boundary, not an epistemic rank.** What
actually happens is that a claim declares `requiresIsolation`, and the run's `isolationBoundary` enters
the same claim-relative comparison as every other environment fact.

**The table below survives as a useful summary of what usually follows** — because the isolation an
OS-packages claim needs is exactly what tier 1 says it does not supply — **but it is a consequence, not
the rule.** Read it as "what tends to be true", never as a lookup the code performs:
...
⚠️ **The specific failure this table exists to prevent:** validating an NVIDIA driver procedure inside a container, seeing it exit zero, and recording rung 4. ✅ **#134 makes that mechanical rather than advisory:** the claim declares `requiresIsolation: ["kernel-isolation"]`, a container's `isolationBoundary.doesNotClaim` lists it, and the record is **excluded from that claim entirely** — not merely capped. An explicit "does not claim" is a **contradiction** of the requirement, where silence would only have been `unknown`. That is what DEC-0005's `doesNotClaim` list is FOR.
...
│   └── pi-package/          ← shipped Pi extensions, skills, prompts, and specialist definitions
├── .pi/
│   ├── settings.json        ← non-secret project Pi configuration; committed
│   ├── sessions/            ← user-facing Pi session state; gitignored (#29)
│   └── runtime/             ← local launch/session-selection state; gitignored (#29)
...
- Session persistence — `SessionManager.create()` to disk so a plan survives a restart, or in-memory and rely on the documents as the real state? **→ #29**
	- ~~_original leaning:_ persist to disk under `.planning/`.~~ **Amended by #29 on 2026-09-02:** persist the user-facing session under the consumer project's gitignored `.pi/sessions/`, never in the updateable tool clone. The documents stay the real state: **losing a session must never lose a decision.** If it can, something that should have been written to a document wasn't.
