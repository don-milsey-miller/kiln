...
| 67 | How specialist children get project trust | **The delegation extension passes `--approve` on every child spawn.** Not the setup script, and not a global `defaultProjectTrust` change. Ships as part of #65's extension, and the flag is unconditional — a child is always launched into a trusted project. | **Settled by running it, 2026-08-15** — the first row in this table established empirically rather than by argument. Without it a non-interactive child silently comes up with **no** project package and therefore none of our typed tools, and *does not error*: the session completes normally and the model answers in prose. The alternative was #49 writing a decision into `~/.pi/agent/trust.json` at install time. `--approve` beats it on every axis that matters here: it lives in code we already have to write, it needs no new setup step, it cannot be forgotten on a machine that clones the repo later, and it cannot drift out of sync with a trust file nobody looks at. ⚠️ It is a real grant, not a formality — see the risk row. |
...
| **Blanket trust grant** | #67 has the delegation extension pass `--approve` on **every** child spawn, unconditionally. That is our code deciding, on the PM's behalf and without asking, that the project directory is trusted — every time. Pi's trust prompt exists precisely so a repository cannot silently change what the agent loads before someone approves it, and #67 opts out of it permanently. | The grant is narrow in scope and defensible in context: it applies only to children *we* spawn, in a directory the PM already trusted by running the setup script and installing the package (#32). It grants nothing the parent session didn't already have. ⚠️ What it does cost is the case where `planning-content/` or `.pi/` arrives from somewhere else — a clone, a colleague, a merge — carrying an extension nobody read. #20 makes that path real: content is committed and travels. Unmitigated beyond "the tool half is gitignored and comes from one remote (#51)." If project-supplied extensions ever become a feature, this row is where the argument has to restart. |
