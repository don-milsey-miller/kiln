...
| 82 | The shared artifact primitives | **Settled once in `schemas/common.schema.json`, consumed by every type schema; re-inventing one inside a type schema is the failure the file exists to prevent.** Six of them. **(1) IDs** — `PREFIX-NNNN`, prefix per #38 type (`REQ`, `DEC`, `SCH`, `API`, …), allocated once and never reused, including after deletion. **(2) Trace links** — always an **array** of IDs, never a bare string, exposed as **named fields** (`derivedFrom`, `verifiedBy`, `evidencedBy`) rather than a generic list with a relation value, each annotated `x-traceTarget`. **(3) Materiality** — `x-materiality` on every property, values per #61. **(4) `n/a`** — #45's `{na: true, reason}` as one branch of a `oneOf`. **(5) Status** — `draft · in-review · approved · amended · superseded · retired`, on the artifact per #4. **(6) The envelope** — `id · type · schemaVersion · status · title` on every artifact, written once and pulled in by `allOf`. **Key casing is `camelCase` throughout.** | **These were undecided, and `requirement` was the first thing that could not be written without them** — which is why it went first rather than because it is alphabetically convenient. Four choices in it are load-bearing enough to state their reasons. **The ID prefix makes a trace reference self-describing**, so #75's unresolvable-link check runs at lint time from the reference alone, with no registry lookup and no need for the target to exist. **Named trace fields rather than `{rel, to}` objects** because #61 requires materiality *per field*, and a relation buried in a value cannot carry a per-field annotation — the generic shape would have quietly made #60's cascade unimplementable. **Arrays always**, because a field that is sometimes one and sometimes many is two shapes for every consumer downstream of it. **camelCase** because `project.yaml` already uses it; the one schema literal in this document (the assertion, imported verbatim from the source material) is `snake_case`, and that inconsistency is inherited, not chosen — it converts when `assertion` is written. ⚠️ **Three consequences found by writing and validating the first schema, all recorded rather than assumed:** (i) **the lint's materiality lookup must be `allOf`-aware** — a type schema that *narrows* an envelope field (`id`, `type`) splits the annotation from the constraint, so a naive read of local `properties` reports fields as unclassified; (ii) **`allOf` + `additionalProperties: false` does not see the referenced subschema's properties**, so every type schema must re-list the envelope keys locally — a JSON Schema trap that silently rejects valid documents if missed; (iii) **`status` cannot be one materiality class.** `superseded`/`retired` must flag everything tracing to the artifact, `draft → in-review → approved` must flag nothing. Classified `structural` with the wrinkle recorded: **#60's cascade has to filter on the transition, not the field.** _(Decided 2026-08-18, written and validated against 17 positive and negative cases before being recorded.)_ |
