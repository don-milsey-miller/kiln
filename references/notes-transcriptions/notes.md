...
⚠️ **One methodological note worth keeping.** The first attempt at run 4 produced an empty output file —
the process was killed by a harness timeout before pi wrote anything — and its "extension absent" would
have read as a clean negative. It was re-run. **An absence is only evidence when you can show the thing
that would have produced the presence actually ran**; every row above carries a same-size transcript for
exactly that reason.
