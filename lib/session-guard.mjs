...
export const GUARD_PROBLEM = Object.freeze({
  ...
  NO_RESULT: "the guard left no result",
});

/**
 * What the guard decided, in the result file.
 *
 * ⚠️ **CODES, NOT CIRCUMSTANCES.** Pi exits 1 for many reasons, so the supervisor needs to know that THIS run
 * ended because the guard refused. The answer says only which check failed. It never carries the session id,
 * the transcript's path, its digest, or anything the transcript said.
 */
export const GUARD_OUTCOME = Object.freeze({ ACCEPTED: "accepted", REFUSED: "refused" });
export const GUARD_CODE = Object.freeze({
  EXPECTATION_UNAVAILABLE: "expectation-unavailable",
  SESSION_ID_MISMATCH: "session-id-mismatch",
  SESSION_FILE_MISMATCH: "session-file-mismatch",
  TRANSCRIPT_CHANGED: "transcript-changed",
  SESSION_FILE_MISSING: "session-file-missing",
});
const CODES = new Set(Object.values(GUARD_CODE));

const NOFOLLOW = constants.O_NOFOLLOW ?? 0;
const sameFile = (a, b) => a.dev === b.dev && a.ino === b.ino;

/**
 * Write the expectation for one launch.
 *
 * Returns `{ok, path, resultPath, readResult, remove}`, or `{ok: false, problem}`. `remove()` clears BOTH files
 * and is what the launch calls on success, on refusal, on a spawn failure and after the child exits.
 *
...
export function createGuardFile({ runtimeDir, expected, randomBytes }) {
  ...
  const stem = join(runtimeDir, `session-guard-${randomBytes(16).toString("hex")}`);
  const path = `${stem}.json`;
  const resultPath = `${stem}.result.json`;
  const body = Buffer.from(
    JSON.stringify({
      ...
      // Where the guard writes what it decided. Named here so the guard invents no path of its own.
      result: resultPath,
    }),
    "utf-8"
  );
  ...
  /**
   * What the guard decided, or why there is no answer. Reading never follows a link and never trusts a file
   * larger than a guard file.
   */
  const readResult = () => {
    let seen;
    try {
      seen = lstatSync(resultPath);
    } catch {
      return { ok: false, problem: GUARD_PROBLEM.NO_RESULT };
    }
    if (seen.isSymbolicLink() || !seen.isFile() || seen.size > MAX_GUARD_BYTES) return { ok: false, problem: GUARD_PROBLEM.NOT_A_FILE };
    let doc;
    try {
      doc = JSON.parse(readFileSync(resultPath, "utf-8"));
    } catch {
      return { ok: false, problem: GUARD_PROBLEM.INVALID };
    }
    if (doc?.resultVersion !== GUARD_VERSION) return { ok: false, problem: GUARD_PROBLEM.INVALID };
    if (doc.outcome === GUARD_OUTCOME.ACCEPTED) return { ok: true, outcome: GUARD_OUTCOME.ACCEPTED, code: null };
    if (doc.outcome === GUARD_OUTCOME.REFUSED && CODES.has(doc.code)) return { ok: true, outcome: GUARD_OUTCOME.REFUSED, code: doc.code };
    return { ok: false, problem: GUARD_PROBLEM.INVALID };
  };

  const removeResult = () => {
    try {
      const now = lstatSync(resultPath);
      if (!now.isSymbolicLink() && now.isFile()) unlinkSync(resultPath);
    } catch {
      /* nothing to clear */
    }
  };
  ...
  return {
    ...
    resultPath,
    readResult,
    /** Clears the expectation and the answer, on every path a launch can take. */
    remove: () => {
      removeResult();
      return remove();
    },
...
/**
 * Inside Pi: say what the guard decided, once, in the file the expectation named.
 *
 * ⚠️ **EXCLUSIVE, LIKE THE EXPECTATION.** An answer already at that name is not overwritten: the supervisor
 * would then read something no guard of this launch wrote.
 */
export function writeGuardResult(resultPath, outcome, code = null) {
  const doc = { resultVersion: GUARD_VERSION, outcome, code: outcome === GUARD_OUTCOME.REFUSED ? code : null };
  if (outcome === GUARD_OUTCOME.REFUSED && !CODES.has(code)) return { ok: false, problem: GUARD_PROBLEM.INVALID };
  let fd;
  try {
    fd = openSync(resultPath, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY | NOFOLLOW, 0o600);
    writeSync(fd, Buffer.from(JSON.stringify(doc), "utf-8"));
    fsyncSync(fd);
  } catch (e) {
    return { ok: false, problem: e?.code === "EEXIST" ? GUARD_PROBLEM.EXISTS : GUARD_PROBLEM.WRITE_FAILED, code: e?.code ?? "unknown" };
  } finally {
...
export function takeGuardFile(env = process.env) {
  ...
  if (doc?.guardVersion !== GUARD_VERSION || !str(doc.sessionId) || !str(doc.file) || !str(doc.result) || !/^[0-9a-f]{64}$/.test(doc.digest ?? ""))
    return { ok: false, problem: GUARD_PROBLEM.INVALID };
  return { ok: true, expected: { sessionId: doc.sessionId, file: doc.file, digest: doc.digest, result: doc.result } };
}
