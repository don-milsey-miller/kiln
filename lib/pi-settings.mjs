...
/**
 * The entry for the DEFAULT layout, where the content root is `<project>/planning-content`. Relative to `.pi`,
 * as Pi resolves resource paths in project settings. Observed overriding in EVD-0081.
 *
 * ⚠️ **A DEFAULT, NOT A CONSTANT (R9).** The content root can be selected elsewhere — `PLANNING_CONTENT_DIR`
 * is the supported way — and an entry fixed at this spelling would then point Pi at a directory that does not
 * exist, silently losing every skill override the project has. `skillOverrideEntry` derives the entry that
 * actually reaches the selected content root, and setup writes that.
 */
export const SKILL_OVERRIDE_PATH = "../planning-content/skills-overrides";

/** Where overrides live inside a content root. The scaffold creates it; this module only points at it. */
export const SKILL_OVERRIDE_DIR = "skills-overrides";

/**
 * The committed entry that reaches the selected content root's `skills-overrides/` from `.pi/settings.json`.
 *
 * ⚠️ **PROVED BY RESOLVING IT BACK, NEVER CONSTRUCTED AND ASSUMED.** The entry is derived from the two paths,
 * then resolved again from `.pi` and compared with the directory it is supposed to reach, so a spelling that
 * does not arrive there cannot be committed.
 *
 * ⚠️ **AND IT MUST STAY INSIDE THE PROJECT, BECAUSE THIS FILE IS COMMITTED.** A content root outside the
 * project can only be reached by a path that is this machine's — `../../elsewhere`, or an absolute one — and
 * committing that hands every other clone a path that means something different or nothing at all. Setup
 * refuses and names both directories rather than writing an entry that only works here.
 *
 * @param {{projectRoot: string, contentRoot: string}} options
 * @returns {{skillsEntry: string, directory: string}}
 */
export function skillOverrideEntry({ projectRoot, contentRoot }) {
  const project = canonicalPath(projectRoot);
  const settingsDir = join(project, ".pi");
  const directory = canonicalPath(join(contentRoot, SKILL_OVERRIDE_DIR));

  if (!isAtOrInside(directory, project))
    throw new SettingsRefusal(
      SETTINGS_REFUSAL.SKILL_OVERRIDE_OUTSIDE,
      `The selected content root's ${SKILL_OVERRIDE_DIR}/ is outside the project, so no entry for it could be ` +
        `committed to ${SETTINGS_PATH} without writing this machine's own path into a file every clone reads.\n` +
        `  project:        ${project}\n  skill overrides: ${directory}\n` +
        `Select a content root inside the project. Nothing was written.`,
  ...
  if (stats === null || !stats.isDirectory())
    throw new SettingsRefusal(
      SETTINGS_REFUSAL.SKILL_OVERRIDE_MISSING,
      `The selected content root has no ${SKILL_OVERRIDE_DIR}/ directory, so there is nothing to register. ` +
        `Kiln's initializer creates it; a content root without one is not a Kiln content root. Nothing was written.`,
      { directory }
    );

  const spelled = relative(settingsDir, directory).split(sep).join("/");
  const skillsEntry = spelled.startsWith(".") ? spelled : `./${spelled}`;

  if (!isPortableRelativePath(skillsEntry) || pathIdentityKey(canonicalPath(join(settingsDir, skillsEntry))) !== pathIdentityKey(directory))
    throw new SettingsRefusal(
      SETTINGS_REFUSAL.SKILL_OVERRIDE_OUTSIDE,
      `The entry Kiln derived for the content root's ${SKILL_OVERRIDE_DIR}/ does not resolve back to it from ` +
        `${SETTINGS_PATH}, so it is not the path Pi would follow. Nothing was written.`,
      { directory }
    );

  return { skillsEntry, directory };
}
...
export const SETTINGS_REFUSAL = Object.freeze({
  ...
  SKILL_OVERRIDE_OUTSIDE: "skill-override-outside-project",
  SKILL_OVERRIDE_MISSING: "skill-override-missing",
});
...
export function validateDesired(desired) {
  ...
  const { stateMode, provider, model, thinkingLevel, packageEntry = null, packageEntryEquivalents = [], skillsEntry = SKILL_OVERRIDE_PATH } = desired;
  ...
  // ⚠️ THE SKILL ENTRY IS AN ARGUMENT SO THE TWO WRITERS AGREE (R9). Registration commits the entry that
  // reaches the SELECTED content root; if this writer then re-added a fixed spelling, a project whose content
  // root is elsewhere would end up with both — one of them pointing at nothing.
  if (!isPortableRelativePath(skillsEntry)) refuse("skillsEntry");
...
export function mergeSettingsText(currentText, desired) {
  ...
  next.skills = onceIn(current.skills ?? [], d.skillsEntry, []);
...
/**
 * The two entries setup registers, checked before anything touches the filesystem.
 *
 * ⚠️ **BOTH ARE PROVED ELSEWHERE AND ONLY APPLIED HERE.** `provePortableEntryTarget` shows the package entry
 * reaches this checkout's package directory and `skillOverrideEntry` shows the skills entry reaches the
 * selected content root; this checks that what arrives is a path a committed file may hold at all.
 */
export function validateRegistration(registration) {
  ...
  if (!isPlainObject(registration)) refuse("registration");
  const { packageEntry, packageEntryEquivalents = [], skillsEntry } = registration;

  if (!isPortableRelativePath(packageEntry)) refuse("packageEntry");
  if (!isPortableRelativePath(skillsEntry)) refuse("skillsEntry");
  if (!Array.isArray(packageEntryEquivalents) || packageEntryEquivalents.some((e) => typeof e !== "string" || e.length === 0))
    refuse("packageEntryEquivalents");

  return Object.freeze({ packageEntry, skillsEntry, packageEntryEquivalents: Object.freeze([...packageEntryEquivalents]) });
}

/**
 * The merged settings text for a registration, or `null` when both entries are already there.
 *
 * Pure: no filesystem, no environment. ⚠️ **IT TOUCHES TWO KEYS.** Not the provider, not the model, not the
 * thinking level, not `sessionDir`: a run that has not chosen a model must not commit one, and a rerun after a
 * selection exists must not disturb it.
 *
 * @param {string|null} currentText  the file as read under the transaction, or null when absent
 * @param {object} registration      see `validateRegistration`
 * @returns {string|null}
 */
export function mergeRegistrationText(currentText, registration) {
  const r = validateRegistration(registration);
  const current = currentText === null ? {} : readSettings(currentText);

  const next = { ...current };
  next.packages = onceIn(current.packages ?? [], r.packageEntry, r.packageEntryEquivalents);
  next.skills = onceIn(current.skills ?? [], r.skillsEntry, []);
 ...
 */
export async function applyKilnSettings({ transaction, desired, ...options } = {}) {
  // ⚠️ VALIDATED BEFORE ANY FILESYSTEM CHANGE, so an invalid request cannot leave even a directory behind.
  const d = validateDesired(desired);
  return writeSettingsFile({ transaction, name: "applyKilnSettings", produce: (current) => mergeSettingsText(current, d), ...options });
}

/**
 * Register Kiln's package and skill-override entries, and nothing else (D27).
 *
 * ⚠️ **SETUP REGISTERS THE PACKAGE BEFORE IT KNOWS THE MODEL.** `applyKilnSettings` writes a complete desired
 * state, which includes a provider, a model and a thinking level; registration happens in the phase before any
 * of those has been chosen, and assigning one to satisfy the writer would commit a selection nobody made. This
 * writes the two entries Kiln owns and leaves every other key — including a selection an earlier run committed
 * — exactly as it found it.
 *
 * @param {object} options
 * @param {object} options.transaction     a live transaction whose plan includes `settingsTarget()`
 * @param {object} options.registration    see `validateRegistration`
 * @param {object} [options.lockRetries]
 * @param {(info: {path: string}) => Promise<void>} [options.onLockAcquired]
 * @returns {Promise<{target: string, changed: boolean}>}
 */
export async function applyKilnRegistration({ transaction, registration, ...options } = {}) {
  const r = validateRegistration(registration);
  return writeSettingsFile({ transaction, name: "applyKilnRegistration", produce: (current) => mergeRegistrationText(current, r), ...options });
}

/**
 * The one write path: Pi's settings lock, held across the transaction's own compare-before-write.
 *
 * ⚠️ **ONE PATH FOR BOTH WRITERS, BECAUSE THE LOCK DISCIPLINE IS THE HARD PART.** Acquiring Pi's lock,
 * refusing a compromised one, releasing it, proving it released and removing a directory created only to hold
 * it are five things a second copy would get subtly wrong. What differs between the writers is which bytes
 * they produce, and that is the argument.
 */
async function writeSettingsFile({
  ...
  // ⚠️ THE PLAN IS READ FROM THE LEDGER, not from the transaction object's shape: `plannedFiles` is what the
  // transaction actually planned, and an object that merely looks like one cannot answer for it.
  const want = pathIdentityKey(canonicalPath(join(lease.roots.project, ".pi", "settings.json")));
  const plannedAbs = lease.plannedFiles.find((abs) => pathIdentityKey(abs) === want);
  if (!plannedAbs)
    throw new SettingsRefusal(
  ...
  try {
    ...
    const result = await runWithTransaction(transaction, name, () => transaction.merge(SETTINGS_KEY, produce));
