 ...
 * @param {(ctx: {selection: object, agentDir: string, declared: object}) => Promise<object>} [opts.canary]  runs the
...
export async function checkLaunch({ declared = {}, canary = null, ...opts }) {
  ...
  let result;
  try {
    result = await canary({ selection, agentDir: pf.agentDir, declared });
  } catch (e) {
