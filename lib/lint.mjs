...
export function lintProject(ctx) {
  ...
  const defs = ctx.stageDefinitions ?? loadStageDefinitions();
  for (const d of stageAnnotationDisagreements(ctx.schemas, defs))
    findings.push(
      finding("stage/x-stage-disagrees", SEVERITY.ERROR, {
        message:
...
export function evaluateStageGate(ctx, stageId, opts = {}) {
  ...
  const defs = opts.stageDefinitions ?? loadStageDefinitions();
  ...
  for (const c of criteria ?? [])
    if (typeof c.check === "function" && !c.check(ctx, { findings, records }))
      gateFindings.push(
