...
import { loadStageDefinitions, producedBy, stageAnnotationDisagreements, producesUnknownTypes } from "./stages.mjs";
...
function lifecycleConsistency(ctx, rec, byId) {
  ...
  for (const [field, prop] of Object.entries(eff.properties)) {
    ...
    // ⚠️ A DECISION'S `addresses` EDGE IS THE ONE EXEMPTION, because it is not dependence. This rule
    // is about an active artifact RESTING on something that no longer stands; a decision that
    // addresses a retired requirement, a closed question or a superseded criterion is the artifact
    // that explains why it no longer stands. Warning about it would make every properly-recorded
    // retirement produce a permanent finding — and a warning that can never be cleared is one
    // everybody learns to scroll past, which costs more than this rule is worth.
    if (doc.type === "decision" && field === "addresses") continue;
    for (const ref of doc[field] ?? []) {
...
export function lintProject(ctx) {
  ...
  findings.push(...contentVersionAgreement(ctx, records));
  ...
  // #107: a stage may not claim to produce a type the catalogue does not contain. Removing a
  // type from #38 without removing it here leaves the methodology asserting an output nothing
  // can make, and deactivation alone would hide it rather than surface it.
  for (const u of producesUnknownTypes(ctx.schemas, defs))
    findings.push(
      finding("stage/produces-unknown-type", SEVERITY.ERROR, {
        message:
          `Stage ${u.stageId} declares it produces "${u.type}", which is not in #38's catalogue. ` +
          `If the type was rejected, stages/ and #38 must move together with activation (#107).`,
        details: u,
      })
    );
...
/**
 * #50 — the project's content schema version, checked rather than merely declared.
 *
 * ⚠️ **The manifest's number was inert until 2026-08-23.** `project.yaml` declared `schemaVersion: 1`
 * while the authoring tool stamped 2 on every new artifact and the stored content sat evenly split
 * across both. Nothing read the manifest, so nothing could disagree with it — the field described a
 * contract no code was party to. A version nobody checks is a comment.
 *
 * ⚠️ **Three separate questions, three rules**, because they call for different actions:
 *   behind the tool   →  the content needs migrating (`npm run migrate:content -- --apply`);
 *   ahead of the tool →  the TOOL is old; `git pull` inside `.planning/` before touching anything;
 *   an artifact disagreeing with the manifest → that record was missed, and migrating is the fix.
 *
 * ⚠️ **An undeclared version is an ADVISORY, not an error.** Agreement needs two declarations, and a
 * content root that never made one cannot be checked against anything. Failing closed here would
 * block every gate on a project that predates the manifest contract while saying nothing true about
 * its content — the rule reports what it can see and does not pretend to a verdict it cannot reach.
 */
function contentVersionAgreement(ctx, records) {
  const out = [];
  const read = "manifestSchemaVersion" in ctx ? { manifest: true, version: ctx.manifestSchemaVersion } : manifestSchemaVersion(ctx.contentRoot);
  const declared = read.version;

  // NO MANIFEST, NO FINDING. Agreement needs two declarations and this content root has made none,
  // so there is nothing here to be right or wrong about. Reporting one would put a permanent note on
  // every content root that predates the manifest contract while saying nothing true about its
  // artifacts — and a finding that fires on everything stops being read.
  if (!read.manifest) return out;

  if (declared === null) {
    out.push(
      finding("content/schema-version-undeclared", SEVERITY.ADVISORY, {
        message:
          `project.yaml declares no \`schemaVersion\`, so stored content cannot be checked against a ` +
          `project version (#50). This tool writes ${SCHEMA_VERSION}.`,
        details: { toolVersion: SCHEMA_VERSION },
      })
    );
    return out;
  }
  ...
  if (declared < SCHEMA_VERSION)
    out.push(
      finding("content/schema-version-behind-tool", SEVERITY.ERROR, {
        message:
          `project.yaml declares schemaVersion ${declared}; this tool writes ${SCHEMA_VERSION}. The content ` +
          `was written against an older schema — migrate it forward rather than authoring against two (#50).`,
        details: { declared, toolVersion: SCHEMA_VERSION },
      })
    );
  else if (declared > SCHEMA_VERSION)
    out.push(
      finding("content/schema-version-ahead-of-tool", SEVERITY.ERROR, {
        message:
          `project.yaml declares schemaVersion ${declared} and this tool writes ${SCHEMA_VERSION}. The CONTENT is ` +
          `newer than the tool: update the tool (\`git pull\` in .planning/) rather than writing to it (#50).`,
        details: { declared, toolVersion: SCHEMA_VERSION },
      })
    );

  for (const rec of records) {
    const version = rec.doc?.schemaVersion;
    if (version === undefined || version === declared) continue;
    out.push(
      finding("artifact/schema-version-disagrees-with-manifest", SEVERITY.ERROR, {
        artifactId: rec.doc?.id ?? null,
        path: rec.relPath,
        message:
          `Carries schemaVersion ${JSON.stringify(version)} while the project declares ${declared}. One version ` +
          `for all content (#50) — a record left behind is one a migration missed, not one that opted out.`,
...
export function evaluateStageGate(ctx, stageId, opts = {}) {
  // ⚠️ `opts.lint` is a REUSE hatch, not a second source of truth: the handoff gate composes every
  // stage gate (#46), and re-linting the whole project once per stage would make the composition
  // cost nine full passes for one answer. Callers that pass it must pass the result of
  // `lintProject(ctx)` for the SAME ctx — anything else would let a stale lint decide a gate.
  const { findings, records } = opts.lint ?? lintProject(ctx);
