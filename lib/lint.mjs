...
export function evaluateStageGate(ctx, stageId, opts = {}) {
  ...
  const acknowledged = new Set(opts.acknowledged ?? []);
  const unmechanised = [];
  for (const c of criteria ?? []) {
    ...
    // ⚠️ A criterion nothing can check must NOT pass by default — that is the well-formed
    // false success this system exists to prevent. It needs a human decision on the record,
    // the same shape as #45's justified n/a and #59's acknowledged unvalidated step.
    if (!acknowledged.has(c.id)) {
      unmechanised.push(c.id);
      gateFindings.push(
        finding("gate/criterion-needs-human-judgement", SEVERITY.ERROR, {
          message: `"${c.describe}" cannot be checked mechanically and has not been acknowledged.`,
          details: { stageId, criterion: c.id, decidedBy: defs?.[stageId]?.decidedBy ?? null },
        })
      );
    }
  }
  ...
  return {
    ...
    unmechanisedCriteria: unmechanised,
