...
/**
 * ⚠️ **RE-EXPORTED, NOT REIMPLEMENTED.** The canonicaliser moved to `./declared-identity.mjs` so the setup
 * command can check `--endpoint-identity` before its dependencies are installed; everything that imported it from
 * here still does.
 */
export { EndpointIdentityError, canonicalizeEndpoint } from "./declared-identity.mjs";
...
/**
 * Whether a resolved `compat` holds only fields Kiln knows, each structured one in its known shape (TSK-0073).
 *
 * The same classification the compatibility key applies, for the canary child's copy of the configuration: an
 * unknown field or key is refused rather than carried, at every depth.
 *
 * @throws {RequestProfileError}
 */
export function assertKnownCompat(compat) {
  const known = new Set([...SCALAR_COMPAT_FIELDS, ...STRUCTURED_COMPAT_FIELDS, ...UNBOUNDED_COMPAT_FIELDS]);
  const unknown = Object.keys(compat ?? {}).filter((k) => !known.has(k));
  if (unknown.length) throw new RequestProfileError(`compat carries ${unknown.length} field(s) this projection does not know.`);
  for (const f of STRUCTURED_COMPAT_FIELDS) if (compat[f] !== undefined) assertKnownShape(compat[f], STRUCTURED_KEYS, `compat.${f}`);
}
 ...
 * `declaredIdentity` is required. Without one this refuses, there is no key, and setup and launch
 * refuse rather than send a check that could prove nothing (D22). That is the honest cost of configuration
 * Kiln cannot inspect safely; the alternative was a fingerprint.
 *
  ...
  if (categories.length && !declaredIdentity)
    throw new RequestProfileError(
      `This model's request depends on configuration Kiln cannot persist or digest safely ` +
        ...
        "non-secret label for this configuration; until one is declared, setup and launch refuse."
    );
  if (!categories.length && declaredIdentity)
    throw new RequestProfileError(
