...
export async function applyIgnoreBlock(plan, { choice = null, contentRoot = null, read = readFileSync, transaction = null } = {}) {
  if (transaction) {
    assertWritableUnder(transaction, plan);
    return runWithTransaction(transaction, "applyIgnoreBlock", () => write(plan, choice, contentRoot, read));
  }
  return write(plan, choice, contentRoot, read);
}

/**
 * The lease this write runs under, and the project it authorises.
 *
 * ⚠️ **THE PATH IS CHECKED AGAINST THE TRANSACTION'S ROOT, NOT THE CALLER'S ARGUMENT.** A live lease
 * proves a lock is held somewhere; it does not prove it is held over the file about to be appended to.
 * The plan carries a path the caller resolved, so comparing it to anything the caller also supplied
 * would be asking the caller to mark its own homework.
 */
function assertWritableUnder(transaction, plan) {
  ...
  const expected = join(state.roots.project, ".gitignore");
  if (pathIdentityKey(resolve(plan.path)) !== pathIdentityKey(expected))
    throw new IgnoreRefusal(
      "outside-transaction",
      `The transaction is open over ${state.roots.project}, and the planned .gitignore is ${plan.path}. ` +
        `Nothing was written.`,
      { path: plan.path, expected }
    );
}
