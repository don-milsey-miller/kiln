...
import { adoptPlannedWrite, runWithTransaction, transactionState } from "./setup-transaction.mjs";
 ...
 *
 * ⚠️ **UNDER A TRANSACTION WHEN THE CALLER HAS ONE, SO THE WRITE IS PART OF THE PLANNED RUN.** Setup
 * used to fix coverage before it planned its transaction: the lock was held, but the mutation sat
 * outside the plan's lifetime and outside the phase ledger, so an interrupted run left a change no
 * journal mentioned. `transaction` makes this module a collaborator like `createStateRoot` — the lease
 * is authenticated rather than duck-typed, the project it may write in is the transaction's to state,
 * and the work is registered in-flight so the lock cannot be released while it runs. It stays optional
 * because the initializer applies the block inside its own swap, where there is no transaction to join.
 *
 * ⚠️ **AND IT IS STILL THIS MODULE THAT WRITES, NOT `tx.merge`.** The block is appended, or created
 * exclusively, against a file re-read at the moment of writing; merge rebuilds a whole file from bytes
 * decided earlier, which is the one thing a `.gitignore` owner must never do.
 *
 * @param {object} plan  from `planIgnoreBlock`
 * @param {{choice?: string, contentRoot?: string, read?: Function, transaction?: object}} [opts]
 ...
 */
export async function applyIgnoreBlock(plan, { choice = null, contentRoot = null, read = readFileSync, transaction = null } = {}) {
  if (!transaction) return write(plan, choice, contentRoot, read);
  const target = plannedTarget(transaction, plan);
  return runWithTransaction(transaction, "applyIgnoreBlock", async () => {
    const result = await write(plan, choice, contentRoot, read);
    // ⚠️ THE PLAN IS TOLD WHAT THIS WROTE, and only when something was written. The entry's recorded identity
    // describes the file as it was before the append, so a journal flushed afterwards would publish an identity
    // that no longer matches. Adopting an UNCHANGED file would be different: there the bytes on disk are not
    // this module's work, and quietly accepting them is how someone else's edit becomes setup's record of it.
    if (result.changed) result.planned = adoptPlannedWrite(transaction, target);
    return result;
  });
}

/**
 * The planned target this write is, proved from the transaction's own plan.
 *
 * ⚠️ **A LEASE AND A DECLARED PHASE ARE NOT THE PLAN.** A live transaction proves a lock is held; it does not
 * give this file the three things every planned target gets before the first lasting write — containment inside
 * an authorised root, an identity recorded while a refusal is still free, and a probe that the directory can
 * actually be written. A write that skipped all three because its owner happened to be holding a transaction
 * would be exactly the unplanned mutation this collaborator exists to stop being.
 *
 * ⚠️ **AND THE ANSWER COMES FROM THE LEDGER, NOT FROM THE CALLER'S ARGUMENTS.** `plannedFiles` is the plan's,
 * canonical and unforgeable; comparing the caller's path against anything the caller also supplied would be
 * asking it to mark its own homework.
 */
function plannedTarget(transaction, plan) {
  const state = transactionState(transaction);
  if (!state || !state.active)
    throw new IgnoreRefusal(
      "no-lease",
      `Applying the .gitignore block was asked to run inside ${state ? "a transaction that has already finished" : "something that is not a setup transaction"}. ` +
        `Nothing was written.`,
      { path: plan.path }
    );

  const wanted = pathIdentityKey(canonicalPath(resolve(plan.path)));
  const target = state.plannedFiles.find((abs) => pathIdentityKey(abs) === wanted);
  if (!target)
    throw new IgnoreRefusal(
      "not-planned",
      `${plan.path} is not in this transaction's write plan, so it has had none of the checks a planned write ` +
        `gets: containment in an authorised root, an identity recorded before anything is written, and a probe ` +
        `that the directory can be written at all. The transaction is open over ${state.roots.project}. Nothing ` +
        `was written. Name the file in the transaction's files when the run intends to change it.`,
      { path: plan.path, projectRoot: state.roots.project, planned: [...state.plannedFiles] }
    );
  return target;
}

async function write(plan, choice, contentRoot, read) {
...
function staleReport(now, plan) {
  ...
  // ⚠️ **COVERAGE AND OWNERSHIP ARE SEPARATE QUESTIONS, and deriving the choice from coverage alone
  // answered the wrong one.** An edited or malformed block can hold every rule Kiln would have
  // written and still be somebody's own work — the operator arranged those lines themselves, or
  // added comments among them — and ACC-0046 requires a block like that to get an explicit choice
  // whether or not its contents happen to protect the paths. The old rule returned
  // `requiresChoice: false` beside `choices: [keep, rewrite]`, which is a report contradicting
  // itself: two options offered and none required.
  const requiresChoice = now.requiresChoice || uncovered.length > 0;

  // ⚠️ THE STATUS IS INHERITED ONLY FROM A CLASSIFICATION THAT WOULD HAVE WRITTEN NOTHING. A fresh
  // `create` carries the status `added`, and reporting that from a call which deliberately wrote
  // none would claim a block was added by the run that refused to add one. Written structurally
  // rather than as "is anything uncovered": those agree on every state reachable today — a `create`
  // or `append` always leaves something uncovered, and a fully-covered legacy block classifies as
  // `current` before `legacy` is tested — but they agree by coincidence, and the coincidence is one
  // reordering away from turning the status into a quiet lie.
  const wouldHaveWritten = now.action !== IGNORE_ACTION.NONE && now.action !== IGNORE_ACTION.REPORT;

  return {
    ...outcome(now),
    status: wouldHaveWritten ? GITIGNORE_STATUS.NEEDS_ATTENTION : now.status,
    changed: false,
    reported: true,
    requiresChoice,
    uncovered,
