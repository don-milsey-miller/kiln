...
function matchesExactly(block, rules) {
  return block.raw === spliceText(block, rules);
}
 ...
 * preserved" forbids — and it is a diff on a file the operator did not ask to have tidied. This is
 * also what makes `matchesExactly` and the splice the same function, so a block can never be
 * recognised by one rule and rewritten by another.
 */
 ...
 */
function classifyFile(path, recorded) {
...
async function perform(plan, choice) {
  for (let attempt = 0; ; attempt++) {
    ...
    const now = plan.repository === false ? plan : classifyFile(plan.path, plan.recorded ?? null);
    ...
    if (now.action === IGNORE_ACTION.REPORT) {
      // ⚠️ THE CHOICE IS ONLY HONOURED FOR THE REPORT THE CALLER WAS SHOWN. An operator who answered
      // "rewrite" to an edited block did not thereby authorise rewriting whatever is there now.
      if (plan.action !== IGNORE_ACTION.REPORT) return unchanged({ ...moved, reported: true });
      return report(now, choice, unchanged);
    }
    if (now.action === IGNORE_ACTION.MIGRATE) return { ...(await migrate(now)), ...moved };
