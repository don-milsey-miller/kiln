...
async function perform(plan, choice, read = readFileSync) {
  for (let attempt = 0; ; attempt++) {
    ...
    if (now.action === IGNORE_ACTION.REPORT) {
      // ⚠️ **THE CHOICE IS CONSENT TO REPLACE THE BYTES THE OPERATOR WAS SHOWN, NOT CONSENT TO
      // REPLACE A CATEGORY.** Comparing only that both classifications are `report` treats "an edited
      // block" as the thing consented to, so an answer given about one operator's block silently
      // authorised destroying a different one that replaced it — two `edited` reports look identical
      // through that check and are not the same file. The consent is matched to the exact text it was
      // given about; anything else is a new report, and the operator has to be asked again.
      if (plan.action !== IGNORE_ACTION.REPORT) return unchanged({ ...moved, reported: true });
      if (choice === IGNORE_CHOICE.REWRITE && now.existing !== plan.existing)
        return unchanged({
          ...moved,
          reported: true,
          note:
            `the file changed after the report this answer was given about, so the answer was not ` +
            `applied to it — nothing was written, and what is there now is reported instead`,
        });
