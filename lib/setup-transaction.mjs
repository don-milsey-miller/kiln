...
export const REFUSAL = {
  ...
  NOT_SELF_WRITTEN: "not-self-written",
  /**
 ...
 * different hat.
 *
 * ⚠️ **`roots` IS THE AUTHORISATION, AND IT COMES FROM HERE BECAUSE NOWHERE ELSE CAN BE TRUSTED TO
 * SAY IT.** A collaborator that creates directories needs to know which roots this transaction
 * actually authorised, and taking that from its own arguments is not authentication — it is asking
 * the caller to mark its own homework. `local-state.mjs` did exactly that: it proved the lease
 * against a `projectRoot` the caller passed and then created directories at a path the caller also
 * passed, so a live transaction for one project could create state in another, and a transaction
 * planned with no `stateRoot` could create one anyway. The roots are canonical, and `state` is
 * absent unless the spec explicitly named one — an authorised root nobody planned does not exist.
 *
 * ⚠️ **`plannedFiles` IS HERE FOR THE SAME REASON `roots` IS.** A collaborator that writes one file
 * itself — the `.gitignore` owner, which appends against a read taken at the moment of writing and so
 * cannot go through `merge` — still has to be a planned write, or it skips the containment check, the
 * plan-time identity and the writeability probe that every other target gets. Asking the caller which
 * file it was allowed to write is not a check, so the answer comes from the plan.
 *
 * @param {unknown} tx
 * @returns {{projectRoot: string, active: boolean, roots: {project: string, state?: string}, stateMode: string|null, plannedFiles: string[]}|null}
 */
export function transactionState(tx) {
  const record = tx && typeof tx === "object" ? LEDGER.get(tx) : undefined;
  if (!record) return null;
  // A fresh object each call, for the same reason the rest of this is a copy: a caller must not be
  // able to add a root to the ledger's map and authorise itself.
  return {
    projectRoot: record.projectRoot,
    active: record.active,
    roots: Object.freeze(Object.fromEntries(record.roots)),
    stateMode: record.stateMode,
    plannedFiles: Object.freeze(record.plannedFiles()),
  };
}

/**
 * Record, in the plan, the `.gitignore` owner's own append.
 *
 * ⚠️ **ONE FILE, NAMED HERE, AND NOTHING ELSE.** `.gitignore` is the only file setup changes that its
 * owner must write itself: the block is appended against a read taken at the moment of writing, because
 * rebuilding the file from bytes decided earlier destroys whatever reached it in between. Every other
 * target goes through `merge`, which re-reads and refuses a file that moved. The write is still planned
 * — containment, the plan-time identity and the writeability probe all applied — but the recorded
 * identity describes the file as it was BEFORE the append, so a journal flushed afterwards would publish
 * an identity that no longer matches. This updates the entry from the file itself.
 *
 * ⚠️ **AND IT IS RESTRICTED TO THAT FILE BECAUSE OTHERWISE IT LAUNDERS CONCURRENT EDITS.** Adoption
 * replaces the identity `merge` compares against. Offered for any planned target, it would take whatever
 * another process had just written to `.pi/settings.json`, record it as the plan's own, and let the next
 * `merge` sail past the concurrent-edit check that exists to refuse exactly that. A capability that can
 * only ever be pointed at the file whose owner cannot use `merge` cannot be pointed at the files that can.
 *
 * ⚠️ **IT IS NOT A WAY IN.** The target must already be in the plan, the transaction must still be live,
 * and the identity is re-read from disk rather than supplied: nothing here writes a file, and nothing
 * here lets a caller claim contents a file does not have.
 *
 * @param {object} tx  the transaction the collaborator is running inside
 * @param {string} abs  the file it wrote, as the plan resolved it
 */
export function adoptPlannedWrite(tx, abs) {
  const record = tx && typeof tx === "object" ? LEDGER.get(tx) : undefined;
  if (!record || !record.active)
    throw new SetupRefusal(
      REFUSAL.TRANSACTION_REVOKED,
      `A write was offered to ${record ? "a transaction that has already finished" : "something that is not a transaction this module issued"}, ` +
        `so there is no plan to record it in.`,
      { path: abs }
    );
  const entry = record.entryAt(abs);
  if (!entry)
    throw new SetupRefusal(
      REFUSAL.UNPLANNED_TARGET,
      `${abs} is not in this transaction's write plan, so the write cannot be recorded against it.`,
      { path: abs }
    );
  if (entry.abs !== join(record.roots.get("project"), IGNORE_FILE))
    throw new SetupRefusal(
      REFUSAL.NOT_SELF_WRITTEN,
      `${entry.key} is a target setup writes through merge, and merge records what it wrote. Only the ` +
        `project's ${IGNORE_FILE} is written by its own owner, so only that file's identity may be taken from ` +
        `the file afterwards. Adopting any other target would replace the identity merge compares against, ` +
        `which is how another process's edit becomes setup's own record of the file.`,
      { key: entry.key, path: entry.abs }
    );
  const current = identify(entry.abs);
  entry.state = current.state;
  entry.digest = current.digest;
  entry.text = current.text;
  return { target: entry.key, state: entry.state, digest: entry.digest ?? null };
}
...
/** The one file in a project that setup changes by appending, through a single owner rather than through `merge`. */
...
/**
 * The state POLICY this transaction authorises, alongside the root it authorises.
 *
 * ⚠️ **A ROOT WITHOUT ITS MODE IS HALF AN AUTHORISATION, and the missing half was the one that
 * decided whether escaping the project was allowed.** `local-state.mjs` reads a mode to answer
 * exactly that, and read it off the same caller-supplied object as the paths: with `<project>/.pi` a
 * junction, taking the legitimate project-local roots and changing `mode: "project"` to `"user"` —
 * one field, nothing else — turned the refusal into a directory outside the project. The root was
 * genuinely authorised the whole time; the policy governing it was not.
 *
 * ⚠️ **REQUIRED WHENEVER A STATE ROOT IS, AND FORBIDDEN OTHERWISE.** A mode that defaulted would be
 * a policy this module chose rather than the caller, which is the same defect as a default state
 * root. One without the other is a spec that has not decided.
 *
 * The value is opaque here on purpose: the vocabulary belongs to
 * `schemas/runtime/kiln-session.schema.json`, and restating it in this module would put a second
 * copy of an enum beside the persisted one. What this enforces is that a mode was named, and that
 * whoever acts on it gets the named one rather than their own.
 */
function authorizedStateMode(spec) {
  const mode = spec?.stateMode ?? null;
  if (!spec?.stateRoot) {
    if (mode !== null)
      throw new SetupRefusal(
        REFUSAL.PATH_ESCAPE,
        `This transaction names a stateMode (${JSON.stringify(mode)}) but no stateRoot. A policy for a ` +
          `root nobody planned governs nothing; pass both or neither.`,
        { stateMode: mode }
      );
    return null;
  }
  if (typeof mode !== "string" || mode.length === 0)
    throw new SetupRefusal(
      REFUSAL.PATH_ESCAPE,
      `This transaction authorises a state root but names no stateMode. The mode decides whether that ` +
        `root may lie outside the project, so a transaction that leaves it unsaid authorises a ` +
        `location without the policy that governs it.`,
...
export function planTransaction(spec) {
  ...
  const stateMode = authorizedStateMode(spec);
  ...
  return { roots, stateMode, projectRoot, lockPath, files, journalKey, journalValidate };
}
...
async function execute(plan, body, opts) {
   ...
   */
  /**
   * ⚠️ **THE PLAN IS REACHED THROUGH THE LEDGER, NOT THROUGH THE `tx` OBJECT'S SHAPE.** A collaborator asking
   * "did you plan this file?" has to get the answer from the thing that cannot be forged, for the same reason
   * `roots` comes from here: an object that merely looks like a transaction could otherwise answer yes.
   * Journal entries are excluded — the journal is the transaction's own file, and no collaborator writes it.
   */
  const mergeable = () => [...plan.files.values()].filter((f) => f.kind !== "journal");
  const record = {
    projectRoot: plan.projectRoot,
    roots: plan.roots,
    ...
    active: true,
    enrol: null,
    plannedFiles: () => mergeable().map((f) => f.abs),
    entryAt: (abs) => mergeable().find((f) => pathIdentityKey(f.abs) === pathIdentityKey(abs)) ?? null,
  };
