...
async function execute(plan, body, opts) {
  ...
  try {
    ...
    try {
      ...
      e.journalError = journalError;
    }
    throw e;
  }

  // ⚠️ THE JOURNAL IS REMOVED ON SUCCESS, so its PRESENCE is the signal that a run was interrupted.
  // A journal that lingered would make every later start look like a recovery — which is why a
  // failure to remove it is raised rather than swallowed, even though all the work succeeded.
