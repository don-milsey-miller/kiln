...
/**
 * Record, in the plan, a write that a planned target's OWN OWNER performed.
 *
 * ⚠️ **FOR THE ONE FILE `merge` CANNOT WRITE, AND FOR NOTHING ELSE.** `.gitignore` is appended to,
 * against a read taken at the moment of writing, because rebuilding it from bytes decided earlier
 * destroys whatever reached it in between. That write is still planned — containment, identity and
 * the writeability probe all applied — but the plan's recorded identity describes the file as it was
 * BEFORE it, so a journal flushed afterwards would publish an identity that no longer matches. This
 * updates the entry from the file itself.
 *
 * ⚠️ **IT IS NOT A WAY IN.** The target must already be in the plan, the transaction must still be
 * live, and the identity is re-read from disk rather than supplied by the caller: there is nothing
 * here a caller could use to write a file the plan does not contain, or to claim contents it does
 * not have.
 *
