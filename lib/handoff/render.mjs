...
export function renderPackage(input) {
  ...
  // --- identity: NORMALIZED WHOLE-PACKAGE HASH ------------------------------------------------
  // ⚠️ Every file is rendered with a fixed PLACEHOLDER where the snapshot goes, the whole tree —
  // including MANIFEST.json and README.md — is hashed, and only then are the placeholders replaced.
  // That removes the self-reference without excluding material files.
  //
  // ⚠️ The previous scheme hashed everything EXCEPT those two, and it was wrong in a way only
  // republishing showed: adding the `approval` block to MANIFEST.json changed the package and left
  // the snapshot at b218b4a525c6176b. A renderer change that alters ANY output now changes the
  // snapshot, and one that produces identical output correctly retains it — which is what makes
  // `toolVersion` provenance metadata rather than a load-bearing part of identity.
  const withPlaceholder = new Map(files);
  withPlaceholder.set("MANIFEST.json", canonicalJson(manifestBody(SNAPSHOT_PLACEHOLDER, input, activated, byType)));
  withPlaceholder.set("README.md", renderReadme(SNAPSHOT_PLACEHOLDER, input.toolVersion, byType));
  const contentHash = hashFiles(withPlaceholder);
  for (const [path, text] of withPlaceholder)
    files.set(path, typeof text === "string" ? text.split(SNAPSHOT_PLACEHOLDER).join(contentHash) : text);
  return files;
}

/** The placeholder every self-referential file carries while the package is hashed. */
export const SNAPSHOT_PLACEHOLDER = "0000000000000000";

/**
 * Recompute a package's snapshot from its own files, normalizing first.
 *
 * ⚠️ Verification must perform the SAME normalization — replace the recorded snapshot with the
 * placeholder — or it would be hashing bytes the renderer never hashed and would never agree.
 */
export function verifySnapshot(files) {
  const manifest = JSON.parse(files.get("MANIFEST.json"));
  const recorded = manifest.snapshot;
  const normalized = new Map();
  for (const [path, text] of files)
    normalized.set(path, typeof text === "string" ? text.split(recorded).join(SNAPSHOT_PLACEHOLDER) : text);
  const recomputed = hashFiles(normalized);
  return { recorded, recomputed, ok: recorded === recomputed };
}

function manifestBody(snapshot, input, activated, byType) {
  return {
    snapshot,
    ...
    note:
      "Generated by `npm run handoff`. `snapshot` is a NORMALIZED WHOLE-PACKAGE hash: every file, " +
      "including this one and README.md, is hashed with a fixed placeholder where the snapshot goes, " +
      "and the placeholders are replaced afterwards. So any change to any file changes it, and " +
      "verification must normalize the same way before recomputing. `toolVersion` is provenance, not " +
      "identity. There are deliberately no timestamps: a timestamp would make two identical plans look " +
      "different, which is the opposite of what an identifier is for.",
  };
}
