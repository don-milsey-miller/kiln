...
import { readBoundaryRefusals } from "./operator-boundary.mjs";
...
export function readProjectStatus(ctx, opts = {}) {
  ...
  // ⚠️ **THE REFUSAL AUDIT IS PART OF THE STATUS, NOT A SEPARATE TOOL (ACC-0070).** The criterion requires a
  // refused operator-boundary attempt to be observable in the recorded state rather than only in the
  // transcript, and a transcript is the model's own account. Reading it here is what makes it observable to
  // anyone who asks the project what happened.
  //
  // ⚠️ **OUTSIDE THE REFUSAL WRAPPER, FOR THE REASON ABOVE.** `readBoundaryRefusals` answers absent, empty,
  // recorded or invalid for every state of the file and throws only for a defect in this caller.
  return {
    ...base,
    intake: readStageIntake(base.stageOneDocument),
    boundaryRefusals: readBoundaryRefusals(ctx.contentRoot),
  };
}
