...
/**
 * Writes that are neither a typed creation nor a registry mutation, by the OPERATION each performs.
 *
 * ⚠️ **AN OPERATION IDENTIFIER IS NOT A WIRE NAME.** A stage definition declares what it may
 * change, and it says so in the same vocabulary the registry uses - `setLifecycle`, not
 * `kiln_set_lifecycle`. These three had only wire names, so a definition naming them would have had to
 * mix the two vocabularies. The tool names are DERIVED from this table rather than written out beside
 * it, so the pair cannot drift.
 */
export const OTHER_WRITE_OPERATIONS = Object.freeze({
  writeStageAttestation: "kiln_write_stage_attestation",
  writeStageDocument: "kiln_write_stage_document",
  setTypeActivation: TYPE_ACTIVATION_TOOL_NAME,
});

/** The same three, as the names they are offered under. Derived, so they are stated once. */
export const OTHER_WRITE_TOOL_NAMES = Object.freeze(Object.values(OTHER_WRITE_OPERATIONS));

/**
 * Every write that is not a creation, as an operation identifier - TSK-0071 (D54).
 *
 * ⚠️ **CREATION IS NOT HERE, AND THAT IS THE POINT.** What a stage may CREATE is `produces` in
 * its own definition, and `lib/stages.mjs` opens by saying why a second description of that would
 * disagree with the first. This vocabulary is what a stage may CHANGE.
 */
export const NON_CREATION_WRITE_OPERATIONS = Object.freeze([...Object.keys(MUTATION_TOOL_NAMES), ...Object.keys(OTHER_WRITE_OPERATIONS)].sort());
