...
export async function initializeProject(opts = {}) {
  ...
  /**
   * ⚠️ **THE DEFAULT IS STILL ONE RULE, AND A CALLER MAY NAME THE DIRECTORY INSTEAD (R9).** A project whose content
   * lives at `<project>/kiln-content` is a project Kiln can run — the launcher resolves it through
   * `PLANNING_CONTENT_DIR` — and setup owns making what it writes agree with that choice. What it may not be is a
   * directory outside the project: the scaffold is staged beside its destination and renamed into place, and the
   * committed settings hold a path relative to `.pi`, so a content root elsewhere is neither swappable nor
   * spellable in a file every clone reads.
   */
  const named = opts.contentRoot === undefined || opts.contentRoot === null ? null : canonicalPath(resolve(opts.contentRoot));
  if (named !== null && (pathIdentityKey(named) === pathIdentityKey(projectRoot) || !isAtOrInside(named, projectRoot)))
    return refusal(
      "invalid-content-root",
      REFUSAL_CLASS.INVALID_TARGET,
      `The content root must be a directory inside the project.
  content root: ${named}
  project root: ${projectRoot}`
    );
  const contentRoot = named ?? join(projectRoot, CONTENT_DIR_NAME);
  ...
  async function initializeUnderLock() {
    ...
    // ⚠️ **BESIDE THE DESTINATION, NOT IN THE PROJECT ROOT.** The swap is a rename, and a rename is only atomic
    // within one filesystem; staging beside the directory it will become keeps that true wherever the content
    // root was selected.
    const temp = join(dirname(contentRoot), `.${basename(contentRoot)}.${process.pid}.${randomBytes(4).toString("hex")}.tmp`);
