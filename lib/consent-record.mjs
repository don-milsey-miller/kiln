...
export async function obtainGrant(location, { grant, choice = null, ask, prompt, now }, opts = {}) {
  ...
  if (grant === GRANT.INSPECTION) {
    ...
    const r = await reconcileConsent(location, choice, opts);
    ({ cleared, persisted } = r);
    standing = r.standings[grant];
    if (standing === undefined) throw new TypeError(`${JSON.stringify(grant)} is not a grant`);
  }
