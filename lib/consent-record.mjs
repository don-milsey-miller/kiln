...
/** A credential variable's NAME, which is all a consent record may hold of one. */
const CREDENTIAL_VAR = /^[A-Z][A-Z0-9_]*$/;
...
function assertChoice(choice) {
  ...
  if (model?.credentialVar != null && (typeof model.credentialVar !== "string" || !CREDENTIAL_VAR.test(model.credentialVar)))
    throw new TypeError("A model choice's credentialVar is the NAME of an environment variable");
...
export function staleGrants(record, choice) {
  ...
  // ⚠️ A DIFFERENT CREDENTIAL VARIABLE IS A DIFFERENT GRANT (TSK-0072): the operator confirmed use through a named
  // route, and a new name has not been confirmed. Compared only when the caller knows the name; launch does not,
  // and reads it from the grant instead.
  const routeChanged = use && choice.model !== null && "credentialVar" in choice.model && (use.credentialVar ?? null) !== (choice.model.credentialVar ?? null);
  if (use && (choice.model === null || use.provider !== choice.model.provider || use.model !== choice.model.model || routeChanged))
...
export async function recordGrant(location, { grant, granted, choice = null, now = () => new Date() }, opts = {}) {
  ...
  if (grant === GRANT.MODEL_USE) {
    ...
    if (choice.model.credentialVar != null) entry.credentialVar = choice.model.credentialVar;
...
/**
 * The credential variable this host's model-use grant for exactly `model` was given through, or `null`.
 *
 * ⚠️ **WHATEVER THE ANSWER WAS.** A declined grant keeps its variable too, so asking again with the same name finds
 * the same decision. A caller that will USE the variable checks the grant's standing first, as launch does.
 */
export function declaredCredentialVar(location, model, opts = {}) {
  const use = readConsent(location, opts).record?.modelUse;
  if (!use || use.provider !== model?.provider || use.model !== model?.model) return null;
