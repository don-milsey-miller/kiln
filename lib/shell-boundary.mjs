...
export function shellBoundaryConfig(root) {
  return {
    ...
    // ⚠️ WHO MAY IMPORT WHICH ADAPTER. Unlisted adapters admit the reader alone, so this widens the
    // rule only where a reviewer can see it widened. The change stream is not a content adapter — it
    // hands out a notification service that reads nothing — which is why the justification for
    // "reader only" does not reach it.
    adapterConsumers: {
      [join(root, "app", "server", "change-stream.js")]: [join(root, "app", "events", "route.js")],
      // ⚠️ TWO CONSUMERS, AND THE ADAPTER WAS SPLIT OUT SO THAT COULD BE TRUE CHEAPLY. `paths.js`
      // answers "where does content live" and holds nothing that reads it, so admitting the events
      // route costs no read surface — the reason `content.js` admits the reader alone does not reach
      // a path calculation. The route needs it because its watcher and the reader must resolve the
      // same directory; they did not until 2026-08-31, and in a consumer layout they disagreed.
      [join(root, "app", "server", "paths.js")]: [
        join(root, "app", "_read", "planning.js"),
        join(root, "app", "events", "route.js"),
      ],
      // ⚠️ ONE CONSUMER, ONE NAME, admitted on the same argument that `paths.js` earned: it holds
      // no read. `healthIdentity` opens no content, takes no lock and caches nothing — two
      // environment variables and the tool's own package version — so admitting the health route
      // widens no read surface. The route needs it because a readiness claim is a claim about the
      // process that answered, and only that process can make it.
      [join(root, "app", "server", "identity.js")]: [join(root, "app", "health", "kiln", "route.js")],
      // ⚠️ THE ONLY WRITE ADAPTER, admitted by the Server Action alone. DEC-0021 reviewed this
      // one individually because it takes the content lock; naming its single consumer here is
      // what stops the second caller being added without that review happening again.
      [join(root, "app", "server", "review.js")]: [join(root, "app", "_write", "review-action.js")],
    },
  };
}
 ...
 */
export function analyseReadBoundary({ appDir, allowedDir, readerFile, exclude = [], adapterConsumers = {} }) {
  const excluded = new Set(exclude.map((p) => resolve(p)));
  ...
  for (const file of scanned) {
    ...
    for (const { spec, line } of specifiersIn(src)) {
      ...
      // (b) EACH ADAPTER DECLARES WHO MAY IMPORT IT, defaulting to the reader alone.
      //
      // ⚠️ The original rule was "only the reader may import app/server/*", justified by "or the read
      // escapes connection()". That justification covers the CONTENT adapters and nothing else — and
      // the first module that needed a non-content adapter (a change stream, which watches rather
      // than reads) would have had to be exempted. An exemption list is the thing this project keeps
      // refusing; a per-adapter consumer list is the same information stated positively, where a
      // reviewer reads it, and it still fails closed: an adapter with no declaration admits only the
      // reader.
      const target = resolveRelative(file, spec);
      if (target && inside(target, allowedDir) && !isAdapter) {
        const permitted = adapterConsumers[resolve(target)] ?? [reader];
        if (!permitted.some((p) => resolve(p) === resolve(file)))
          add(
            "adapter-outside-reader",
            file,
            line,
            `imports ${spec} — that adapter admits only ${permitted.map((p) => p.split(sep).pop()).join(", ")}`
          );
      }
    }
