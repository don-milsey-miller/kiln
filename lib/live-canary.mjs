...
import { OBSERVED_KEY_FIELDS } from "./compatibility-record.mjs";
...
/**
 * The separate approval for the one diagnostic request, worded as the proposal's section 6.4 requires: it names
 * the provider and the exact model, says what the request cannot do, and says the provider may charge.
 */
export function liveCheckPrompt({ displayName, model }) {
  return [
    "Live model check",
    "",
    `Kiln will send one small diagnostic request to ${displayName} using ${model}. The`,
...
function readLiveReport(raw, provider, model) {
  ...
  if (!isPlainObject(report) || Object.keys(report).sort().join(",") !== "activeTools,calls,ceiling,model,observed,registeredTools,requests") throw invalid();
  // The bounded projection: the four key fields or a key refusal, and endpoint parts only.
  const observed = report.observed;
  if (!isPlainObject(observed)) throw invalid();
  const observedKeys = Object.keys(observed).sort().join(",");
  if (observedKeys !== "keyError" && observedKeys !== [...OBSERVED_KEY_FIELDS].sort().join(",")) throw invalid();
  if (observedKeys === "keyError" && typeof observed.keyError !== "string") throw invalid();
  if (!Array.isArray(report.requests)) throw invalid();
  for (const r of report.requests)
    if (r !== null && (!isPlainObject(r) || Object.keys(r).sort().join(",") !== "hostname,pathname,port,scheme")) throw invalid();
...
export function judgeLiveReport(report, challenge, { provider, model, thinkingLevel }) {
  ...
  return Object.freeze({ provider, model, thinkingLevel, passed: true, challengeEchoed: true, ceiling: report.ceiling, observed: report.observed, requests: report.requests });
}
...
export async function runLiveCanary(request = {}) {
  const { thinkingLevel, randomBytes = nodeRandomBytes, declared = {} } = request;
  ...
  return runCanaryChild(request, {
    ...
    workDir: true,
    // ⚠️ THE DECLARED IDENTITIES ARE NON-SECRET LABELS BY CONTRACT, so they may travel by argv; nothing else is added.
    childArgs: ({ sdk, provider, model, workDir }) => [LIVE_CANARY_CHILD_PATH, sdk.url, provider, model, thinkingLevel, challenge, workDir, JSON.stringify(declared)],
