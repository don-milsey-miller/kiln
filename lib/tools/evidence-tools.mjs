...
const ACCEPTANCE_CRITERION_FIELDS = new Set(["title", "statement", "evaluates", "verifies", "outcome", "evidencedBy", "notes", "tags"]);

/** #144. Defaults to `not-evaluated`: a criterion that exists and has never been run is a real state,
 *  and it is what distinguishes an IMPLEMENTED component from an ACCEPTED one (QST-0015). */
export const createAcceptanceCriterion = (input, opts) =>
  createArtifact("acceptance-criterion", ACCEPTANCE_CRITERION_FIELDS, { outcome: "not-evaluated", ...input }, opts);

const TASK_FIELDS = new Set(["title", "statement", "role", "implements", "fulfils", "acceptedBy", "dependsOn", "notes", "tags"]);

/** #144. `role` is a string by decision (#18): roles are derived by collecting these values. */
export const createTask = (input, opts) => createArtifact("task", TASK_FIELDS, input, opts);

const COMPONENT_FIELDS = new Set(["title", "responsibility", "satisfies", "implementedBy", "notes", "tags"]);

/**
 * #140's component. Deliberately narrow: the PM approved a named design unit, a responsibility and
 * `satisfies`, and said further links wait for authentic stage 7-8 demand. A field added now for an
 * acceptance criterion nobody has written is the catalogue justifying itself again (#106).
 */
export const createComponent = (input, opts) => createArtifact("component", COMPONENT_FIELDS, input, opts);
 ...
 *
 * ⚠️ **WHICH FIELDS THOSE ARE IS ASKED OF THE SCHEMAS, NOT OF A LIST HERE (QST-0032, DEC-0031).**
 * It used to be a hand-written array of seventeen names, and the schemas declared TWENTY-FIVE trace
 * fields — so `acceptedBy`, `answeredBy`, `blocks`, `evaluates`, `fulfils`, `raisedBy`, `satisfies`
 * and `verifies` were all writable through this function, bypassing the very guards #101 exists to
 * protect. `resolveQuestion` refuses to mark a question answered without an answer or a link; this
 * function would happily write `answeredBy` on its own, and did, for a whole cycle.
 *
 * A field is a trace field exactly when its schema declares `x-traceTarget`, which is the same
 * source `linkTrace` validates against — so the two cannot drift. Asked PER TYPE rather than as a
 * union, because a name that is a link on one type need not be one on another. Adding a trace field
 * to a schema now extends this guard with no code change here at all.
 *
...
export async function reviseArtifact(type, id, changes, opts = {}) {
  ...
  const isTraceField = (k) => Array.isArray(eff.properties?.[k]?.["x-traceTarget"]);
  for (const k of Object.keys(changes ?? {})) {
    if (forbidden.includes(k))
      throw new ValidationError(`\`${k}\` is not revisable — it is identity or lifecycle state (#82).`, []);
    if (isTraceField(k))
      throw new ValidationError(
        `\`${k}\` is a trace field on \`${type}\` — its schema declares x-traceTarget. Links have ` +
          `their own operations so their guards cannot be bypassed (#101): use linkTrace/unlinkTrace, ` +
          `or the bespoke operation where one exists.`,
        []
      );
  }
  ...
  return withLock(join(contentRoot, LOCK_FILE), async () => {
    ...
    // never from a hardcoded field list — which is the same rule the trace-field guard above now
    // follows, and did not until DEC-0031. `eff` is the one resolved at the top of this function.
