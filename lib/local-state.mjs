...
function assertStateRootLeased(transaction, roots, what) {
  ...
  // ⚠️ THE AUTHENTICATED MODE, NOT `roots.mode`. They are equal by the check above, and using the
  // transaction's copy is what keeps that true if the two ever drift apart again.
  if (state.stateMode === STATE_MODE.PROJECT && !isAtOrInside(authorised, state.projectRoot))
    throw new LocalStateRefusal(
      STATE_REFUSAL.ESCAPES_ROOT,
