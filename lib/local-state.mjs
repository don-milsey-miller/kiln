 ...
 */
export function createStateRoot(roots, { transaction, projectRoot }) {
  assertLease(transaction, projectRoot, "creating a state root");

  for (const dir of [roots.root, roots.sessions, roots.runtime]) {
    assertContained(dir, roots.within, dir === roots.root ? "the state root" : `${dir}`);
    ...
    assertContained(dir, roots.within, dir);
  }
  return roots;
}

/**
 * The authenticated, live lease this project's mutations require.
 *
 * ⚠️ **AUTHENTICATED, NOT DUCK-TYPED, AND CHECKED AGAINST THIS PROJECT** — the same three questions
 * `initialize-project.mjs` asks, for the same reason. A shape describes data; it cannot establish a
 * capability, and a transaction held on a different project excludes nobody here.
 */
function assertLease(transaction, projectRoot, what) {
  ...
  if (projectRoot && pathIdentityKey(state.projectRoot) !== pathIdentityKey(canonicalPath(resolve(projectRoot))))
    throw new LocalStateRefusal(
      STATE_REFUSAL.NO_LEASE,
      `${what} was attempted through a transaction holding a different project.\n` +
        `  transaction: ${state.projectRoot}\n  this project: ${resolve(projectRoot)}\n` +
        `A lock on another project is not exclusion here. Nothing was written.`,
      { what }
    );
  return state;
}
...
export function openStateRoot({ projectRoot, mode, projectId = null, recorded = null, platform, env, home, transaction = null }) {
  ...
  return { ok: true, roots: transaction ? createStateRoot(roots, { transaction, projectRoot }) : roots, covers };
}
 ...
 *
 * @param {{transaction: object, projectRoot: string, randomBytes: Function, validators?: object}} opts
 */
export async function ensureProjectId({ transaction, projectRoot, randomBytes, validators, recordVersion = 1 }) {
  assertLease(transaction, projectRoot, `writing ${PROJECT_RECORD}`);
