 ...
 * The mode is what says whether being outside the project is intended, so the mode is what is
 * checked. External state is deliberately outside and is exempt.
 */
function assertStateRootLeased(transaction, roots, what) {
  ...
  if (roots.mode === STATE_MODE.PROJECT && !isAtOrInside(authorised, state.projectRoot))
    throw new LocalStateRefusal(
      STATE_REFUSAL.ESCAPES_ROOT,
