...
/**
 * Refuse a state mode this module does not know, wherever one is about to decide something.
 *
 * ⚠️ **A DEFAULT-DENY, AND THE THIRD PLACE IT WAS NEEDED.** `createStateRoot` had this check added
 * after an unrecognised mode inherited the permissive branch there; `coverageState` did not get it,
 * and a mode misspelled the SAME WAY in both the request and the layout passed the binding check and
 * fell through to the project branch — returning `covered: true` under a mode nobody recognises, and
 * echoing that mode back to the caller as though it meant something. The supervisor route happens to
 * be protected because `stateRootFor` refuses first; an exported function is not sound because of
 * where its callers happen to check.
 *
 * ⚠️ **THE SHAPE MATTERS MORE THAN THE PLACES.** Written as "if it is the strict one, be strict",
 * every unknown value lands on the permissive branch; written as a vocabulary check up front, an
 * unknown value lands nowhere at all, which is the only safe answer for a policy that decides where
 * data may be written.
 */
function assertKnownMode(mode, what) {
  if (!Object.values(STATE_MODE).includes(mode))
    throw new LocalStateRefusal(
      STATE_REFUSAL.UNKNOWN_MODE,
      `${what} was attempted under the state mode ${JSON.stringify(mode)}, which is not one this ` +
        `module knows. The two are ${Object.values(STATE_MODE).map((m) => JSON.stringify(m)).join(" and ")}. ` +
        `An unrecognised mode cannot be assumed to be either one, and the policy that decides where ` +
        `state may be written is not a place to guess.`,
      { what, mode, known: Object.values(STATE_MODE) }
    );
  return mode;
}
...
function assertLayout(roots, projectRoot, mode) {
  // ⚠️ VALIDATED BEFORE IT IS COMPARED. Binding the two values closes a disagreement between them
  // and says nothing about whether either is a mode: the same typo in both passes the binding.
  assertKnownMode(mode, "Checking a state layout");
...
function assertStateRootLeased(transaction, roots, what) {
  ...
  assertKnownMode(state.stateMode, what);
...
export function coverageState({ projectRoot, mode, roots = null, recorded = null }) {
  // ⚠️ AT THE TOP, BEFORE ANY BRANCH. The branches below are "external" and "everything else", so an
  // unrecognised mode would otherwise be answered by the second one — a coverage verdict, and a mode
  // echoed back to the caller, for a policy this module cannot name.
  assertKnownMode(mode, "Deciding coverage");
...
export function projectRecordState(projectRoot, { validators } = {}) {
  ...
  try {
    ...
    // ⚠️ THE CODE, NEVER THE MESSAGE. An OS error quotes the path it failed on, and this file's
    // content is not this module's to repeat into a refusal that gets printed.
    return { kind: RECORD.INVALID, path, detail: `cannot be read (${e?.code ?? "unknown error"})` };
  }
...
function classifyRecordText(text, validators) {
  ...
  try {
    ...
    // ⚠️ V8's parser quotes the input around the failure, and this record may hold anything.
    void e;
    return { kind: RECORD.INVALID, detail: "is not JSON" };
  }
...
/**
 * The non-secret identities this project declares for configuration Kiln cannot derive or digest safely (D22).
 *
 * ⚠️ **COMMITTED, BECAUSE THE KEY IS RECOMPUTED AT EVERY SETUP AND EVERY LAUNCH.** A declaration that lived
 * only in the command line that first supplied it would leave the next start recomputing a different key and
 * refusing a record it had just written. Read from the same validated record as the project id, so a damaged
 * file is a recovery decision here too rather than a silently empty declaration.
 *
 * @returns {{endpointIdentity?: object, requestIdentity?: string}} shaped as the compatibility key's `declared`
 */
export function committedDeclarations(projectRoot, { validators } = {}) {
  const state = projectRecordState(projectRoot, { validators });
  if (state.kind === RECORD.ABSENT) return {};
  if (state.kind === RECORD.INVALID) throw recordRefusal(state);
  const declared = state.record.declaredIdentities ?? {};
  return {
    ...(declared.endpoint ? { endpointIdentity: declared.endpoint } : {}),
    ...(declared.request ? { requestIdentity: declared.request } : {}),
  };
}
...
function recordRefusal(state) {
  return new LocalStateRefusal(
    ...
    `${PROJECT_RECORD} exists and ${state.detail}. It holds this project's committed identity, which keys ` +
    ...
    // ⚠️ THE RELATIVE IDENTIFIER, IN THE DETAIL TOO. A refusal is printed, logged and sometimes
    // serialised, and the record's absolute path names the operator's machine while saying nothing
    // the relative one does not: this file is always `.pi/kiln.json` under the project root the
    // caller already knows. `projectRecordState` still returns the absolute path for callers that
    // need to act on the file.
    { path: PROJECT_RECORD, detail: state.detail }
  );
}
