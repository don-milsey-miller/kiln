 ...
 */
function assertLayout(roots, projectRoot) {
  ...
  for (const [name, dir] of [["state root", roots.root], ...children]) {
    ...
    if (!isAtOrInside(c, project))
      throw new LocalStateRefusal(
        STATE_REFUSAL.ESCAPES_ROOT,
        `Project-local ${name} resolves to ${c}, which is outside the project ${project}. Nothing ` +
          `was started. The ignore block protects paths inside this repository; a directory that ` +
          `spells like one and resolves elsewhere is protected by nothing.`,
        { name, resolved: c, projectRoot: project }
      );
  }
...
export function coverageState({ projectRoot, mode, roots = null, recorded = null }) {
  if (mode === STATE_MODE.USER) {
    ...
    const { root } = assertLayout(roots, projectRoot);
  ...
  assertLayout(layoutRoots, projectRoot);
