...
/**
 * Check `--provider`, `--model` and `--thinking` against what discovery listed.
 *
...
/** Ask for a thinking level. There is no default, so an empty answer is no choice. */
async function chooseThinking({ ask }) {
  const answer = await ask(`Thinking level? (${THINKING_LEVELS.join(", ")}) `);
  const said = typeof answer === "string" ? answer.trim() : null;
  return THINKING_LEVELS.includes(said) ? said : null;
}
 ...
 * @param {() => Date} [opts.now]
 */
export async function selectModel({ transaction, location, inspection, ask, print = () => {}, requested = {}, settings, now, validators }) {
  ...
  if (!wantsChange && current !== null) {
    ...
    if (listed) {
      const thinkingLevel = requested.thinking ?? current.thinkingLevel;
      if (requested.thinking !== undefined) validateRequested(choices, { provider: current.provider, model: current.model, thinking: requested.thinking });
      const { standing } = await reconcileGrant(location, GRANT.MODEL_USE, { model: current }, opts);
      if (standing === STANDING.GRANTED && thinkingLevel === current.thinkingLevel) return result(SELECTION_OUTCOME.REUSED, { selection: { ...listed, thinkingLevel } });
      if (standing === STANDING.DECLINED && requested.thinking === undefined) return result(SELECTION_OUTCOME.DECLINED, { selection: { ...listed, thinkingLevel } });
      const level = thinkingLevel ?? (await needThinking({ ask }));
      if (level === null) return result(SELECTION_OUTCOME.CANCELLED);
      return confirmAndPersist({ transaction, location, choice: { ...listed, thinkingLevel: level }, current, ask, settings, now, opts, existing: true });
    }
  ...
  if (wantsChange) choice = validateRequested(choices, requested);
    ...
    if (typeof ask !== "function")
      throw new ModelSelectionRefusal(
        ...
        "A model has to be chosen and confirmed, and this run cannot ask. Pass --provider and --model, and run setup interactively to confirm."
      );
    choice = await chooseFrom(choices, { ask, print });
    if (choice === null) return result(SELECTION_OUTCOME.CANCELLED);
    if (requested.thinking !== undefined) choice = { ...choice, thinkingLevel: validateRequested(choices, { ...choice, thinking: requested.thinking }).thinkingLevel };
  }
  if (!choice.thinkingLevel) {
    const level = await needThinking({ ask });
    if (level === null) return result(SELECTION_OUTCOME.CANCELLED);
    choice = { ...choice, thinkingLevel: level };
  }
  return confirmAndPersist({ transaction, location, choice, current, ask, settings, now, opts, existing: false });
}

async function needThinking({ ask }) {
  if (typeof ask !== "function")
  ...
  return chooseThinking({ ask });
}

async function confirmAndPersist({ transaction, location, choice, current, ask, settings, now, opts, existing }) {
  if (typeof ask !== "function")
    throw new ModelSelectionRefusal(
      SELECTION_REFUSAL.NEEDS_CONFIRMATION,
      `${choice.displayName} ${choice.model} has to be confirmed before Kiln uses it, and this run cannot ask. Run setup interactively.`,
  ...
  // ⚠️ ONLY AN EXPLICIT YES.
  const answer = await ask(confirmationPrompt(selection));
  if (answer !== true) {
