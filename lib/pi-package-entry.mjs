...
const SIDE = Object.freeze({ LITERAL: "literal", PORTABLE: "portable", PINNED: "pinned-package" });
...
/**
 * Prove the portable entry resolves to THIS CHECKOUT'S package directory.
 *
 * ⚠️ **SETUP REGISTERS THE PACKAGE ITSELF, SO NOTHING ELSE HAS WRITTEN AN ENTRY TO COMPARE AGAINST (D27).**
 * `provePortablePackageEntry` answers "is what `pi install -l` wrote the same directory as the portable
 * spelling?", which is the question when Pi did the registering. When Kiln writes the entry, the question is
 * the one a committed path actually has to answer: does `../.planning/pi-package`, resolved from
 * `.pi/settings.json`, reach the pinned package in the checkout that is running? A checkout somewhere other
 * than `<project>/.planning` would commit an entry that points at nothing, or at another copy of Kiln.
 *
 * ⚠️ **BOTH SIDES ARE DIRECTORIES THAT EXIST, NEVER TWO SPELLINGS OF NOTHING**, and neither side is
 * constructed from the other: the portable spelling comes from this module and the package root from the
 * caller's checkout, and they are compared after canonicalisation.
 *
 * @param {object} options
 * @param {string} options.settingsDir  the directory `.pi/settings.json` lives in
 * @param {string} options.packageRoot  this checkout's `pi-package` directory
 * @returns {{packageEntry: string, packageEntryEquivalents: string[]}}
 */
export function provePortableEntryTarget({ settingsDir, packageRoot }) {
  const portable = canonicalDirectory(resolve(settingsDir, PORTABLE_PACKAGE_ENTRY), SIDE.PORTABLE);
  const pinned = canonicalDirectory(resolve(packageRoot), SIDE.PINNED);

  if (portable !== pinned)
    throw new PackageEntryRefusal(
      PACKAGE_ENTRY_REFUSAL.NOT_EQUIVALENT,
      `\`${PORTABLE_PACKAGE_ENTRY}\` resolves from ${SETTINGS_PATH} to a different directory from the package ` +
        `this checkout is running, so committing it would point the project at something nobody showed was the ` +
        `pinned package. Kiln registers the package by that spelling and no other, which requires this checkout ` +
        `to be the project's \`.planning\` directory. Nothing was written.`,
      { side: SIDE.PORTABLE, expected: PORTABLE_PACKAGE_ENTRY }
    );

  // ⚠️ NO EQUIVALENTS. There is no other spelling in the file to replace: Kiln wrote the only entry it owns.
  return { packageEntry: PORTABLE_PACKAGE_ENTRY, packageEntryEquivalents: [] };
}
