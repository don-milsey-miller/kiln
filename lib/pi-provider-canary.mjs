...
import { RequestProfileError, STRUCTURED_COMPAT_FIELDS, UNBOUNDED_COMPAT_FIELDS, assertKnownCompat } from "./runtime-records.mjs";
...
/**
 * ⚠️ **`samplingParams` CROSSES BECAUSE THE REQUEST IS WHAT IS BEING PROVED.** Pi documents that it overrides
 * named request fields, so a child that did not have it would send a different request from the one the operator's
 * configuration makes — and a proof of a request nobody makes is worse than no proof. It is not a credential
 * route: `apiKey`, `headers` and `authHeader` are, and they stay refused below. What keeps it out of the key is the
 * declared identity (D22), not keeping it out of the child.
 */
const ACCEPTED_MODEL_KEYS = Object.freeze(["id", "name", "contextWindow", "maxTokens", "reasoning", "input", "cost", "samplingParams", "compat"]);

/**
 * ⚠️ **`compat` CROSSES FOR THE SAME REASON (TSK-0073), AND ONLY IN ITS KNOWN SHAPE.** It changes the request Pi
 * builds — the system role, the token-limit field, the streaming options — so a child without it proved a different
 * request, and the compatibility key, which includes it, refused the proof as `canary-inputs-differ`. It carries
 * only fields the compatibility key knows, each structured one in its known shape; anything else is refused.
 *
 * ⚠️ **AND NO CREDENTIAL-NAMED KEY CROSSES AT ANY DEPTH.** An entry, its `samplingParams` or its `compat` holding a key
 * named for a credential is refused before any child is created, whatever the value; names are compared whole, after
 * lowercasing and dropping separators, so `maxTokens` is not a token.
 */
const CREDENTIAL_KEY_NAMES = new Set([
  "apikey", "authheader", "authorization", "proxyauthorization", "header", "headers", "bearer",
  "token", "accesstoken", "authtoken", "sessiontoken", "refreshtoken", "idtoken", "securitytoken",
  "secret", "clientsecret", "password", "passwd", "cookie", "setcookie", "credential", "credentials",
  "xapikey", "xgoogapikey", "anthropicapikey", "openaiapikey", "xauthtoken", "xaccesstoken", "xsessiontoken", "xamzsecuritytoken",
]);
/**
 * ⚠️ **WHERE A STRING MAY CROSS, SPELLED OUT, BECAUSE A KEY'S NAME CANNOT VOUCH FOR ITS VALUE.** An innocuous key can
 * hold a secret, and no test of a string's shape can say it does not. So a string value reaches the child only where
 * the request needs it and its content is bounded: the provider's `baseUrl` (with no user, password, query or fragment)
 * and `api`; each model's `id`, `name` and `input` modalities; the compat fields whose values Pi restricts to the literals
 * below; and a chat-template variable's `$var`, which Pi restricts to two. Everywhere else — `samplingParams`, a
 * chat-template argument's value, a structured routing field, a compat string Pi leaves open — a string is refused, and
 * so is any structure a field is not declared to hold. Numbers, booleans and null carry configuration, not credentials.
 * A model whose request needs one of the refused values cannot be proved by the live check (TSK-0073).
 */
const COMPAT_LITERALS = Object.freeze({
  maxTokensField: ["max_completion_tokens", "max_tokens"],
  cacheControlFormat: ["anthropic"],
  deferredToolsMode: ["kimi"],
  sessionAffinityFormat: ["openai", "openai-nosession", "openrouter"],
  thinkingFormat: ["openai", "openrouter", "together", "baseten", "deepseek", "zai", "qwen", "chat-template", "qwen-chat-template", "string-thinking", "ant-ling"],
});
const CHAT_TEMPLATE_VARS = Object.freeze(["thinking.enabled", "thinking.effort"]);
const isBoundedScalar = (v) => v === null || typeof v === "number" || typeof v === "boolean";
/** How many values in one model entry fall outside the contract above. */
const unboundedValueCount = (entry) => {
  let n = 0;
  const bad = (ok) => (n += ok ? 0 : 1);
  if (entry.name !== undefined) bad(typeof entry.name === "string");
  for (const k of ["contextWindow", "maxTokens"]) if (entry[k] !== undefined) bad(typeof entry[k] === "number");
  if (entry.reasoning !== undefined) bad(typeof entry.reasoning === "boolean");
  if (entry.input !== undefined) bad(Array.isArray(entry.input) && entry.input.every((m) => m === "text" || m === "image"));
  if (entry.cost !== undefined) {
    const numbers = (o) => isPlainObject(o) && Object.entries(o).every(([k, v]) => (k === "tiers" ? Array.isArray(v) && v.every(numbers) : typeof v === "number"));
    bad(numbers(entry.cost));
  }
  if (entry.samplingParams !== undefined) bad(isPlainObject(entry.samplingParams) && Object.values(entry.samplingParams).every(isBoundedScalar));
  for (const [k, v] of Object.entries(entry.compat ?? {})) {
    if (UNBOUNDED_COMPAT_FIELDS.includes(k)) {
      const variable = (x) => isPlainObject(x) && CHAT_TEMPLATE_VARS.includes(x.$var) && Object.keys(x).every((p) => p === "$var" || (p === "omitWhenOff" && typeof x.omitWhenOff === "boolean"));
      bad(isPlainObject(v) && Object.values(v).every((x) => isBoundedScalar(x) || variable(x)));
    } else if (STRUCTURED_COMPAT_FIELDS.includes(k)) {
      const noStrings = (x) => (Array.isArray(x) ? x.every(noStrings) : isPlainObject(x) ? Object.values(x).every(noStrings) : isBoundedScalar(x));
      bad(noStrings(v));
    } else bad(typeof v === "boolean" || (typeof v === "string" && (COMPAT_LITERALS[k] ?? []).includes(v)));
  }
  return n;
};
/** A base URL that carries a user, a password, a query or a fragment could carry a credential. */
const baseUrlCarriesMore = (url) => {
  try {
    const u = new URL(url);
...
const credentialKeyCount = (value) => {
  if (value === null || typeof value !== "object") return 0;
  if (Array.isArray(value)) return value.reduce((n, v) => n + credentialKeyCount(v), 0);
  return Object.entries(value).reduce(
    (n, [k, v]) => n + (CREDENTIAL_KEY_NAMES.has(k.toLowerCase().replace(/[^a-z0-9]/g, "")) ? 1 : 0) + credentialKeyCount(v),
    0
  );
};
...
function customConfigFor(config, isCustom, provider, model) {
  ...
  if (baseUrlCarriesMore(config.baseUrl)) refuse("credential-bearing", { where: "baseUrl" });
  ...
  for (const entry of config.models) {
    ...
    if (entry.compat !== undefined) {
      if (!isPlainObject(entry.compat)) refuse("bad-shape");
      try {
        assertKnownCompat(entry.compat);
      } catch (e) {
        if (!(e instanceof RequestProfileError)) throw e;
        refuse("unknown-property", { unknownCount: 1 });
      }
    }
    const credentials = credentialKeyCount(entry);
    if (credentials > 0) refuse("credential-bearing", { credentialKeyCount: credentials });
    const unbounded = unboundedValueCount(entry);
    if (unbounded > 0) refuse("unbounded-value", { unboundedValueCount: unbounded });
  }
...
export async function runProviderCanary(request = {}) {
  return runCanaryChild(request, {
    ...
    childArgs: ({ sdk, provider, model }) => [CANARY_CHILD_PATH, sdk.url, provider, model],
    readReport,
    finish: ({ report, contract, provider, model }) => {
...
/**
 * Run one canary child in a disposable, credential-scoped root, and hand its report to `spec.finish`.
 *
 * ⚠️ **SHARED BY BOTH CANARIES, SO THEIR ISOLATION CANNOT DRIFT.** The provider canary (TSK-0040) and the live
 * tool-call canary (TSK-0041) differ only in which child runs, what it is told, and how its report is read.
 * Everything that keeps the operator's real agent directory and unrelated credentials out of the child —
 * the contract check, the private root, the one selected stored credential, the `$NAME` reference for a
 * custom provider, the scoped environment and the confirmed removal — is this one function.
 *
 * @param {object} request  as `runProviderCanary`
 * @param {{childPath: string, childArgs: (ctx: object) => string[], readReport: (raw: string, provider: string, model: string) => object,
 *          finish: (ctx: object) => any, workDir?: boolean}} spec
 */
export async function runCanaryChild(request = {}, spec) {
  const {
    provider,
    model,
    custom = null,
  ...
  try {
    ...
    // ⚠️ AN EMPTY WORKING DIRECTORY, so a child that runs a session has no project, no context file and no
    // planning content anywhere it could look.
    const workDir = spec.workDir ? join(root, "work") : null;
    if (workDir) mkdirSync(workDir, { mode: 0o700 });
    ...
    const raw = await runChild(spawnImpl, spec.childArgs({ sdk, provider, model, workDir, request }), env, timeoutMs, provider, model);
    const report = spec.readReport(raw, provider, model);
    return spec.finish({ report, contract, provider, model, request });
  } catch (e) {
    primary = e;
    throw e;
  } finally {
