...
import { openSync, closeSync, writeSync, readFileSync, renameSync, unlinkSync, existsSync, statSync } from "node:fs";
 ...
 * take their lock.
 *
 * @returns {{broken: true, owner: object} | {broken: false, reason: "absent"|"unreadable"|"other-host"|"alive"}}
 */
export function breakDeadLock(lockPath) {
  ...
  /**
   * ⚠️ **THE FILE IS CLAIMED BY RENAME BEFORE IT IS JUDGED, BECAUSE READ-THEN-UNLINK IS TWO WRITERS.** Two
   * recoveries starting together both read the dead owner; the first unlinks it and acquires the lock, and the
   * second then unlinks THAT lock — the live one — and acquires it as well. Both believe they hold the project.
   * A rename cannot be shared: exactly one of them moves the file, and the loser finds it gone and is simply a
   * process with no lock to break, which is the correct answer for it.
   */
  const claim = `${lockPath}.recovering.${process.pid}.${randomUUID()}`;
  try {
    renameSync(lockPath, claim);
  } catch {
    // Somebody else claimed it, or the holder released it. Either way this process broke nothing.
    return { broken: false, reason: "claimed-elsewhere" };
  }

  // ⚠️ RE-JUDGED ON THE CLAIMED FILE, because the glimpse above was of a file anybody could have replaced in the
  // meantime: the lock that was dead a moment ago may be a live holder's by now.
  const owner = readOwner(claim);
  const dead = owner && typeof owner.pid === "number" && owner.hostname === hostname() && !ownerAlive(owner);
  if (dead) {
    try {
      unlinkSync(claim);
    } catch {
      /* the claim is ours and named after this process; a failure to remove it is not a lock anybody waits on */
    }
    return { broken: true, owner };
  }

  /**
   * ⚠️ **AND PUT BACK WHAT WAS NOT OURS TO TAKE, WITHOUT OVERWRITING A NEWER HOLDER.** The exclusive create is
   * what makes the restoration safe: if somebody acquired the lock while this was holding the claim, the create
   * fails and the claim is simply discarded — that new holder is the one process holding the project, which is
   * what the lock is for.
   */
  try {
    const fd = openSync(lockPath, "wx");
    try {
      writeSync(fd, readFileSync(claim));
    } finally {
      closeSync(fd);
    }
  } catch {
    /* a newer holder exists; leave it alone */
  }
  try {
    unlinkSync(claim);
  } catch {}
  return { broken: false, reason: owner ? "alive" : "unreadable" };
}
