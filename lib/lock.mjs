 ...
 *
 * @returns the value `judge` returned, or `null` when the gate could not be taken.
 */
function withBreakGate(lockPath, judge) {
  const gate = `${lockPath}.breaking`;
  let fd = null;
  try {
    fd = openSync(gate, "wx");
  } catch (e) {
    if (!CONTENTION_CODES.has(e.code) || !reclaimAbandonedGate(gate)) return null;
    try {
      fd = openSync(gate, "wx");
    } catch {
      return null;
 ...
 *
 * ⚠️ **AGE AND DEATH, AND THE REMOVAL ITSELF IS A RENAME.** A gate is held for microseconds, so one older than
 * {@link GATE_ABANDONED_MS} whose owner is gone was left by a process that no longer exists. The rename is what
 * keeps two processes from both clearing it and both proceeding: exactly one moves the file.
 */
function reclaimAbandonedGate(gate) {
  let age;
  try {
    age = Date.now() - statSync(gate).mtimeMs;
  } catch {
    return true; // it went away by itself: the gate is free, try to take it
  }
  if (age < GATE_ABANDONED_MS) return false;
  // ⚠️ AN UNREADABLE GATE THIS OLD IS ABANDONED TOO, unlike an unreadable LOCK, which is left alone. A gate is
  // written the instant it is created, so an empty one is what a process killed between those two calls leaves;
  // treating it as somebody's would let a single kill block every later recovery for good.
  const owner = readOwner(gate);
  if (owner && ownerAlive(owner)) return false;
  const claim = `${gate}.abandoned.${process.pid}.${randomUUID()}`;
  try {
    renameSync(gate, claim);
  } catch {
    return false; // somebody else cleared it; they are the one proceeding
  }
  try {
    unlinkSync(claim);
  } catch {}
...
export function breakDeadLock(lockPath) {
  ...
  // No gate means another process is breaking this lock right now: it is theirs to clear, not this one's.
  return verdict ?? { broken: false, reason: "claimed-elsewhere" };
}
