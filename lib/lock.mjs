...
/**
 * Is this gate one whose holder died inside it, rather than one somebody is holding?
 *
 * A gate is taken across a handful of synchronous calls with nothing awaited between them, so age alone does not
 * decide it: the holder must also be gone. An unreadable gate this old counts, unlike an unreadable LOCK, which
 * is always left alone — a gate is written the instant it is created, so an empty one is what a process killed
 * between those two calls leaves, and treating it as somebody's would let one kill block every later recovery.
 */
function gateAbandoned(gate) {
  let age;
  try {
    age = Date.now() - statSync(gate).mtimeMs;
  } catch {
    return false; // it is not there at all; there is nothing to reclaim
  }
...
function reclaimAbandonedGate(lockPath) {
  ...
  if (!gateAbandoned(gate)) return "claimed-elsewhere";
