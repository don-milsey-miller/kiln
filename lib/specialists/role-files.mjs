...
export function planRoleDefinitions(toolRoot) {
  ...
  for (const item of wanted) {
    ...
    // ⚠️ **WHAT THE ENTRY IS, BEFORE WHERE IT LEADS.** An earlier version resolved the path first, so a
    // symlink pointing outside the directory was reported as a path escape rather than as a link. Both
    // sentences are true, but the link is the more precise one and it is the one that holds wherever the
    // link points: a link INSIDE the directory is still a link this generator must not read or write
    // through. `lstat` never follows, so the question is answered before anything is resolved.
    //
    // ⚠️ The ordering defect passed locally and failed in all four CI cells, because the test could not
    // create a symlink on the development host and returned early. The test now says so instead.
    let stat = null;
    try {
      stat = lstatSync(full);
    } catch (error) {
    ...
    // Not a link, so resolving it cannot follow one. This catches a directory reached through something
    // odd above the file itself.
