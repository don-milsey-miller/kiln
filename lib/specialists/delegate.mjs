...
import { CHILD_REPORT_TYPE, judgeChildReport, reportedToolsOf } from "./child-report.mjs";
...
export async function delegateToSpecialist(request, deps) {
  ...
  // ⚠️ THE CHILD'S REPORT IS JUDGED BEFORE IT IS USED, and cross-checked against what was requested and
  // against Pi's own assistant events. A missing, duplicated, malformed or contradictory report yields
  // no signatures at all, so the contract's capability check refuses.
  const reportVerdict = judgeChildReport(run.reports, { provider, model, thinkingLevel }, run.nativeSelection);
  ...
  const observation = {
    ...
    childReportAccepted: reportVerdict.accepted,
    childReportReason: reportVerdict.reason,
    reportedActiveTools: reportVerdict.accepted ? reportVerdict.report.activeTools : [],
...
async function runChild(spec, deps) {
  ...
  return { exit, timedOut, aborted, teardown, treeStopped, attestation, reports, nativeSelection: nativeSelectionFrom(stdout), stdout, stderr, output: answerFrom(stdout) };
}
 ...
 *
 * ⚠️ **NOT THE RAW EVENT STREAM.** An earlier version returned stdout whole, which carries Pi's
 * `agent_end` event echoing the user message - and therefore the framed task, nonce and digest.
 * A retained real-child run caught the frame arriving in the delegation's own result.
 *
 * ⚠️ The stream is still kept on the run record for a developer; only what a CALLER receives is
 * narrowed, and it is narrowed to the thing the caller asked for.
 */
function answerFrom(stdout) {
  let answer = null;
  for (const raw of String(stdout).split(String.fromCharCode(10))) {
    if (!raw.trim()) continue;
    let event;
    try {
    ...
    const text = parts.filter((p) => p?.type === "text" && typeof p.text === "string").map((p) => p.text).join(String.fromCharCode(10)).trim();
    if (text.length > 0) answer = text;
  }
  return answer;
}

function nativeSelectionFrom(stdout) {
  let provider = null;
  let model = null;
  for (const raw of String(stdout).split(String.fromCharCode(10))) {
    if (!raw.trim()) continue;
