...
import { createToolName, mutationToolName } from "../tool-wire-names.mjs";
import { TAVILY } from "../research/tavily-adapter.mjs";
import { AUTH_SOURCE } from "../pi-provider-credentials.mjs";
import { ROLES, requiredCapabilitiesFor } from "./roster.mjs";

/** Roles, from #26's roster. */
/**
 * Every specialist role Kiln defines, re-exported from the roster.
 *
 * ⚠️ **THE ROSTER MOVED, THE NAME DID NOT.** `lib/specialists/roster.mjs` imports nothing, so
 * `lib/stages.mjs` can read it while a stage definition loads without pulling this module's tool
 * registry - and `ajv` - into the stage-skill CLI. Callers keep importing `ROLES` from here.
 */
export { ROLES };
...
/** What each role cannot work without. ONE table, declared in `roster.mjs` and read here. */
const REQUIRED_CAPABILITIES = Object.fromEntries(ROLES.map((role) => [role, requiredCapabilitiesFor(role)]));
...
export function contractFor(role) {
  ...
  /**
   * ⚠️ **THE FINAL TOOL LIST, RESOLVED HERE AND NOWHERE ELSE (TSK-0051, D45).** A role definition
   * declares the names a child is offered, and deriving them a second time in the generator would be a
   * second answer to a question this module already answers. Capabilities first, then the creates, then
   * the mutations, each group sorted, so the order is a property of the contract rather than of the
   * order somebody happened to write a boundary in.
   *
   * ⚠️ **ONLY WHAT THE CONTRACT DERIVES.** No read-only tool is added: a specialist receives a
   * task-scoped payload, and handing it `kiln_project_status` would show it the whole project and
   * weaken that boundary. A role needing one adds the requirement here, and the definitions regenerate.
   *
   * ⚠️ **BUILT ONLY WHEN THE CONTRACT IS SOUND.** An unimplemented type or an unknown mutation is
   * already reported below; constructing a name for one would put a tool nothing registers into a
   * child's allowlist, where the intersection with the real registry would silently drop it.
   */
  const sound = unimplemented.length === 0 && unknownMutations.length === 0;
  const tools = sound
    ? Object.freeze([
        ...[...REQUIRED_CAPABILITIES[role]].sort(),
        ...boundary.create.map(createToolName).sort(),
        ...boundary.mutate.map(mutationToolName).sort(),
      ])
    : Object.freeze([]);
 ...
 *
 * ⚠️ **THE WIRE VALUE `task-not-delivered` IS UNCHANGED (TSK-0052).** Only the key moved, from
 * `NO_STDIN`. Anything that switches on the code keeps working, and the code was always about the task
...
export const CHILD_REFUSED = {
  ...
  // ⚠️ ADDED BY TSK-0054 (D47, D48). Every literal above is unchanged; these three name facts the
  // five above cannot express. A nonzero exit and an unreadable stream are not "did not finish", and
  // a child answering under a selection nobody asked for is not a child missing a tool - a caller
  // told `capability-missing` for that would look for the wrong fix.
 ...
 *
 * ⚠️ **`taskBindingObserved` IS NOT ABOUT STDIN, AND ITS OLD NAME SAID IT WAS.** The child is spawned
 * with stdin CLOSED, deliberately, so a field called `stdinDelivered` asked whether something arrived
 * down a channel the design shuts. It read as a contradiction and invited a later implementer to reopen
 * stdin to satisfy it. What is required instead is that the exact delegated task was OBSERVED reaching
 * the child's model turn, through provider-side capture of the child's outbound request: evidence from
 * outside the child rather than the child vouching for itself.
 *
 * ⚠️ **THIS VERIFIER CONSUMES THE OBSERVATION; IT DOES NOT MAKE IT.** A boolean, because producing
 * it during a real specialist launch belongs to the delegation runtime, not to a pure function.
 *
 * @param {object} contract
 * @param {{taskBindingObserved: boolean, timedOut: boolean, reportedTools: object, output: any}} run
 */
export function verifyChild(contract, run) {
  const refuse = (reason, detail) => ({ accepted: false, reason, detail, role: contract.role });

  if (!run?.taskBindingObserved)
    return refuse(
      CHILD_REFUSED.NO_TASK_BINDING,
      "No task binding was observed for this child, so its output cannot be accepted as an answer to the delegated task."
    );
