  ...
  validation_run: {
    ...
    input: {
      ...
      properties: {
        ...
        // ⚠️ The shape a caller must declare, not merely "an array". `expectedOutputs` is read after
        // execution and each entry is a workspace-relative path; `checkJob` refuses anything that
        // leaves the workspace, and a signature that said `array` left the tool's contract describing
        // less than the controller enforces.
        expectedOutputs: {
          type: "array",
          items: {
            oneOf: [
