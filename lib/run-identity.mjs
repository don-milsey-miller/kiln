...
/**
 * The error codes this endpoint is allowed to report, as a closed set.
 *
 * ⚠️ **CLOSED, BECAUSE A SUPERVISOR THAT ECHOES THE CODE IS ECHOING A REMOTE STRING.** Reporting a
 * code from this set names a condition; reporting whatever arrived would put an unrelated service's
 * text into Kiln's output. Anything outside the set is described, not quoted.
 */
export const HEALTH_ERRORS = Object.freeze([NO_IDENTITY, NO_BUILD]);

/** True when a body is shaped like THIS endpoint's — compared against constants, so nothing leaks. */
export function isKilnHealthBody(body) {
  return Boolean(body) && typeof body === "object" && body.service === SERVICE && body.protocol === HEALTH_PROTOCOL;
}
 ...
 */
/**
 * A fresh run identifier: 32 lowercase hex characters, the same shape as the project id.
 *
 * ⚠️ **RANDOM AND EPHEMERAL, DERIVED FROM NOTHING.** It exists to distinguish THIS invocation from a
 * previous Kiln still holding the port, so anything derived — a pid, a timestamp, the project — is a
 * value the impostor could also produce.
 */
export function generateRunId(randomBytes) {
  return randomBytes(16).toString("hex");
}

/**
 * Compare a health response against what this invocation expects.
 *
 * ⚠️ **IDENTITY AND COMPATIBILITY ARE SEPARATED IN THE RESULT, BECAUSE THEY CARRY DIFFERENT WEIGHT.**
 * Service, protocol, run id and project id are what make the responder *this* one; `build` is
 * compared exactly and a mismatch refuses, but a match credits nothing while every build of this
 * package reports `0.0.0`. A field that cannot differ cannot discriminate, and folding it into the
 * identity count would inflate a four-fact handshake into a five-fact one that is not stronger.
 *
 * ⚠️ **IT RETURNS FIELD NAMES, NEVER THE REMOTE VALUES.** Whatever answered the port is untrusted:
 * an unrelated service can put a filesystem path, a token, or anything else into a JSON field, and
 * a comparison that hands those back is a comparison that puts them in the supervisor's diagnostics
 * and its error detail. Naming which field disagreed is everything a caller needs to act; carrying
 * the value is how a probe of a stranger becomes a disclosure about them.
 *
 * @param {{runId: string, projectId: string, build: string|null}} expected
 * @param {unknown} body
 * @returns {{ok: boolean, mismatches: string[], identityFields: string[], buildCompared: boolean}}
 */
export function matchHealth(expected, body) {
  const mismatches = [];
  const check = (field, want, got) => {
    if (want !== got) mismatches.push(field);
  };
  const doc = body && typeof body === "object" ? body : {};

  const identityFields = ["service", "protocol", "runId", "projectId"];
  check("service", SERVICE, doc.service);
  check("protocol", HEALTH_PROTOCOL, doc.protocol);
  check("runId", expected.runId, doc.runId);
  check("projectId", expected.projectId, doc.projectId);

  // ⚠️ Compared, and a mismatch refuses — a server built from different code answering this port is
  // worth stopping for. It is simply not counted as identity.
  const buildCompared = expected.build !== null && expected.build !== undefined;
  if (buildCompared) check("build", expected.build, doc.build);

  return { ok: mismatches.length === 0, mismatches, identityFields, buildCompared };
}
