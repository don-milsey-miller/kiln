...
// ⚠️ THE INTAKE SECTION IS DEFINED WHERE IT IS PARSED, not transcribed here. The writer refuses a document
// whose anchors it does not recognise, so a second copy of those headings in this module would be a
// starter document that the only tool allowed to write into it rejects.
import { intakeSection } from "./stage-documents.mjs";
 ...
 *  addable at most once — see `project-gitignore.mjs`, which owns every edit to that file. */
 ...
 *
 * ⚠️ **`migrated` AND `needs-attention` ARE RECORDED AFTER INITIALIZATION, unlike the first three.**
 * A fresh scaffold can only have added a block, observed an existing rule, or found no repository.
 * The other two are what a LATER run found — an older Kiln's block brought up to date, or a block
 * the operator removed or edited — and they are written back into the record by
 * `recordIgnoreStep`. They are listed here because drift detection rebuilds the reference scaffold
 * from this field, so a value it did not know would report the record as corrupt.
 */
