...
export const CONTRACT_REFUSAL = Object.freeze({
  ...
  /** Pi's `models.json` does not read the provider's key from the variable declared for it on this host. */
  ROUTE_MISMATCH: "declared-route-mismatch",
  /** The declared variable could not be kept with this host's model-use grant, so no launch could find it. */
  NOT_REMEMBERED: "declaration-not-remembered",
});
...
export const PROVIDER_CREDENTIALS = Object.freeze(
  Object.fromEntries(
    [
      ...
      // ⚠️ **THE API-KEY BRANCH ONLY, AND IT IS ONE NAME.** Vertex's other documented path is
      // Application Default Credentials, which is an ambient chain and therefore excluded.
      // `GOOGLE_CLOUD_PROJECT` and `GOOGLE_CLOUD_LOCATION` were listed here as optional names and are
      // gone: they configure the ADC branch, so carrying them made this entry describe two routes
      // while claiming to support one. A contract that names variables belonging to an excluded
      // branch is the widening this table exists to refuse, in the mildest possible disguise.
      simple("google-vertex", "GOOGLE_CLOUD_API_KEY"),
...
/**
 * The complete accepted shape. Anything else is refused.
 *
 * ⚠️ **SILENTLY IGNORING AN EXTRA PROPERTY IS HOW A DECLARATION MEANS SOMETHING IT DOES NOT.** A
 * `models.json` provider block legitimately carries `baseUrl`, `api`, `headers` and `authHeader`, so
 * an operator copying one in has every reason to expect them to matter. Dropping them without a word
 * lets a declaration that reads as "authenticate with this header" load as "authenticate with this
 * variable name", and the difference surfaces as a child that cannot reach its provider. A `key`
 * beside `apiKey`, or an `env` object of values, would be worse: both look like they were accepted.
 */
const ACCEPTED_DECLARATION_KEYS = Object.freeze(["id", "apiKey", "optional"]);
...
export function validateCustomDeclaration(declaration) {
  ...
  // ⚠️ **THE COUNT, NEVER THE KEYS AND NEVER THE VALUES.** An unexpected property may be a `key`
  // holding a credential, or a header naming an internal host; echoing either into a refusal that
  // reaches a log is the leak this whole module is arranged to prevent. The accepted shape is stated
  // instead, which is what an operator needs in order to fix it.
  const unknown = Object.keys(declaration).filter((k) => !ACCEPTED_DECLARATION_KEYS.includes(k));
  if (unknown.length > 0)
    refuse(
      DECLARATION_PROBLEM.UNKNOWN_PROPERTY,
      `${id} declares ${unknown.length} propert${unknown.length === 1 ? "y" : "ies"} this table does not ` +
        `accept. The accepted shape is exactly ${ACCEPTED_DECLARATION_KEYS.join(", ")} — a credential ` +
        `contract holds NAMES, so nothing that could carry a value or an endpoint is read here. The ` +
        `offending properties are not named, because one of them may be the credential.`,
      { id, unknownCount: unknown.length }
    );
...
export function resolveProviderCredentials(providerId, opts = {}) {
  ...
  // ⚠️ **OWN PROPERTIES ONLY, IN BOTH TABLES.** Both are plain objects, so a bracket lookup also finds what
  // `Object.prototype` holds: `toString` resolved to a function, which is truthy, and was refused as a
  // DECLINED provider with the function in `detail.why` and its source printed in the message. That it
  // refused at all was an accident of the declined check running first; the same lookup in the built-in
  // table would have returned the function as a contract. An id is a key in these tables or it is not, and
  // `Object.hasOwn` is the question that means that - not a list of names to refuse, since a declared custom
  // provider is entitled to any id.
  const declined = Object.hasOwn(UNSUPPORTED_PROVIDERS, providerId) ? UNSUPPORTED_PROVIDERS[providerId] : undefined;
  if (declined !== undefined) throw unsupported(`${providerId} has no supported credential contract.\n${declined}`, { why: declined });

  const known = Object.hasOwn(PROVIDER_CREDENTIALS, providerId) ? PROVIDER_CREDENTIALS[providerId] : undefined;
