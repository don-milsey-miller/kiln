...
/**
 * ⚠️ **EXPORTED BECAUSE THE PROJECT ROOT NEEDS THE SAME BOUNDARY, NOT A SECOND ONE.** The setup
 * transaction writes outside `planning-content/` — `.pi/`, `.gitignore`, the local state root — and
 * a junction at `<project>/.pi` is the identical attack on a different root. Re-deriving "resolve to
 * the deepest existing ancestor, then compare canonical prefixes" over there would be two
 * implementations of one rule, which is the disagreement #70 exists to prevent.
 *
 * @param {string} p
 * @returns {string} the canonical path, resolvable even when the tail does not exist yet
 */
export function canonicalPath(p) {
  return canonicalise(p);
}

/** True when `target` IS `root` or lies beneath it, both already canonicalised. */
export function isAtOrInside(target, root) {
  return samePathPrefix(target, root);
}

/**
 * A canonical path reduced to the form two paths must share to be THE SAME FILE.
 *
 * ⚠️ **EXPORTED SO "IS THIS THE SAME FILE?" AND "IS THIS INSIDE?" CANNOT DISAGREE.** Both questions
 * turn on the same platform rule, and the setup transaction needs the first one to index its write
 * plan by location: two entries spelled differently can name one physical file, and two authorized
 * roots can overlap. A separate case rule over there would answer one question with `A` and the
 * other with `a` on exactly the filesystem where it matters.
 */
...
function samePathPrefix(child, parent) {
  const c = pathIdentityKey(child);
  const p = pathIdentityKey(parent);
