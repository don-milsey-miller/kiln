...
function render(stage) {
  ...
  // ⚠️ ONE PHRASE, NOT A COMPOSED LIST. A stage that declares its question rule no longer defers the choice of
  // QUESTION; it still defers the choice of ACTIVITY, which is a different field and still nobody's.
  const deferred = stage.questionSelection ? "how to choose the next activity" : "how to choose the next question or activity";
  lines.push(
    "",
    "## Not in this skill",
    "",
    `This stage's purpose, its method, ${deferred}, its allowed delegations, its mutation and approval boundaries and its completion summary are not stated here, because its canonical definition does not declare them yet.`
  );
