...
export async function stopTree(
  ...
  const readTable = async () => {
    ...
    return readProcessTable({ run: psRun, platform, timeoutMs: readMs });
  };
  ...
  const ask = (hard, verdicts) => {
    ...
    for (const d of watched) {
      ...
      if (platform === "win32") {
        spawnKill("taskkill", ["/pid", String(d.pid), "/F"]);
        continue;
      }
      try {
        killImpl(d.pid, signal);
      } catch (e) {
  ...
  let ours = watched;
  const settled = () => exited(child) && !ours.some(alive);
