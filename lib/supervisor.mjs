...
export async function runSupervisor(deps) {
  ...
  // ⚠️ **AN UNRESOLVED SESSION FAILS CLOSED.** Choosing among the sessions actually available is the next
  // slice; until it exists the outcome is a refusal naming what is there, never a silent fresh session in
  // place of the one the operator came back for. `recoverable` says whether a choice could resolve it.
