...
export async function shutdown({
  ...
  const launcherControl = await stopLauncher(launcher, { graceMs, hardMs, log });
  ...
  const launcherTree = await stopTree(launcher, { ...opts, group: true, knownDescendants: launcherDescendants });
...
export const runFilePath = (runtimeDir, runId) => join(runtimeDir, `${RUN_FILE_PREFIX}${runId}${RUN_FILE_SUFFIX}`);

export function writeRunFile(runtimeDir, { runId, projectId, pid = process.pid, host = HOST, port, now = () => new Date() }) {
...
export function leftoverRunFiles(runtimeDir) {
  ...
  return names.filter((n) => n.startsWith(RUN_FILE_PREFIX) && n.endsWith(RUN_FILE_SUFFIX)).sort();
}
...
export async function runSupervisor(deps) {
  ...
  const performShutdown = (signal = null) => {
    ...
    shutdownStarted = (async () => {
      ...
      // anything to them.
      await launcherTree.stop();
      await agentTree?.stop();
  ...
  try {
    ...
    const session = withSessionDir(agent, stateRoots.sessions, { ...env, PORT: String(port) });
    ...
    const runFile = writeRunFile(stateRoots.runtime, { runId, projectId: project.projectId, host, port });
    if (runFile) ownedFiles.push(runFile);
    else log(`${stateRoots.runtime} does not exist, so this run leaves no run file there; re-run setup to restore it`);
    ...
    stopWatch.dispose();
    // ⚠️ IDEMPOTENT: `performShutdown` has already sampled and stopped both trackers on every path
    // that reached it. This covers the paths that did not — a refusal before the shutdown — and
    // `stop()` on an already-stopped tracker clears a timer that is already null.
    await launcherTree.stop();
    await agentTree?.stop();
