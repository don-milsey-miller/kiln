...
import { SESSION, SESSION_PROBLEM, STORAGE, planSession, recordSession } from "./session-record.mjs";
 ...
 *
 * ⚠️ **THE ID IS PASSED ON BOTH PATHS, AND THAT SYMMETRY IS THE DESIGN.** On a first run Kiln mints the
 * id, passes it, and records it; on a later run it passes the recorded one. `--session-id` creates the
 * session when it is missing and opens it when it is not, so Kiln never has to read Pi's choice back out
 * of a file it does not own — which is what made the identity knowable in the first place.
 */
export function withSessionPolicy(agent, sessionId, env) {
  if (typeof sessionId !== "string" || sessionId.trim().length === 0)
    throw new TypeError(`withSessionPolicy needs the session id to pass to Pi, got ${JSON.stringify(sessionId)}`);

  const args = agent.args ?? [];
  const conflict = args.findIndex((a) => {
    const text = String(a);
    return SESSION_SELECTORS.some((flag) => text === flag || text.startsWith(`${flag}=`));
  });
  if (conflict !== -1)
    throw new SupervisorRefusal(
      REFUSAL.SESSION_SELECTOR_CONFLICT,
      `The agent's argument list already names ${args[conflict]} at position ${conflict}, and the ` +
        `supervisor must choose the session: two selectors leave it to whichever Pi prefers, and a run ` +
        `that resumed something other than the recorded session is the failure this refuses to risk.`,
      { args, sessionId }
    );

  return { args: [...args, SESSION_ID_FLAG, sessionId], env: { ...env } };
}
...
export const PROCESS_TABLE_COMMAND = Object.freeze({
  ...
  // wherever PowerShell is, which on Windows is everywhere. The creation time is a UTC FILETIME.
  win32: [
    "powershell",
    [
      "-NoProfile",
      "-NonInteractive",
      "-Command",
      "Get-CimInstance Win32_Process | ForEach-Object { \"$($_.ProcessId) $($_.ParentProcessId) $(if ($_.CreationDate) { $_.CreationDate.ToFileTimeUtc() } else { '-' })\" }",
    ],
  ],
});
...
export async function runSupervisor(deps) {
  ...
  try {
    ...
    // ⚠️ **WHICH SESSION, DECIDED AND RECORDED BEFORE PI IS STARTED (ACC-0103).** "The agent started"
    // is not "the session resumed": a brand-new unrelated session starts just as successfully, and the
    // two are indistinguishable to an operator until the context is missing. So the id is settled here,
    // written down here, and passed to Pi as `--session-id` — the same value on both paths.
    //
    // ⚠️ **THE IDS COME FROM PI'S OWN LISTER (F121).** Session files are `<timestamp>_<uuid>.jsonl` and
    // the id lives in the header, a different value from the uuid in the name; deriving ids from
    // filenames reported every real recorded session as one that no longer exists.
    ...
    // ⚠️ **AN UNRESOLVED SESSION FAILS CLOSED, and this is the slice where that is all it does.** The
    // recovery UI — presenting the sessions actually available and asking — is the next slice; until it
    // exists the honest outcome is a refusal naming what is there, never a silent fresh session in
    // place of the one the operator came back for.
    if (plan.action === SESSION.ASK)
      throw new SupervisorRefusal(
        ...
        { problem: plan.problem, storage: plan.available?.state ?? null, sessions: plan.available?.sessions?.length ?? 0 }
      );
    ...
    const chosen = plan.action === SESSION.RESUME ? plan.sessionId : generateSessionId(randomBytes);
