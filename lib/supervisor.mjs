 ...
 */
export async function stopLauncher(child, { graceMs = 8000, hardMs = 3000, kill, log = () => {}, timeline = null } = {}) {
  const record = { sentStop: false, endRequested: false, exitObserved: false, exitObservedMs: null, escalated: false, stdinError: null };
  ...
  const grace = tl.begin("grace-wait", { tree: "launcher-control", ms: graceMs });
  const until = Date.now() + graceMs;
  let graceGone = gone();
  while (!graceGone && Date.now() < until) {
    await sleep(50);
    graceGone = gone();
  }
  ...
  if (!gone()) {
    ...
    const hardWait = tl.begin("hard-wait", { tree: "launcher-control", ms: hardMs });
    const hard = Date.now() + hardMs;
    let hardGone = gone();
    while (!hardGone && Date.now() < hard) {
      await sleep(50);
      hardGone = gone();
    }
...
export async function shutdown({
  ...
  const tl = timeline ?? createShutdownTimeline();
  const opts = { platform, run, psRun, kill, log, timeline: tl };
  ...
  const launcherControl = await stopLauncher(launcher, { ...phase(), log, timeline: tl });
  ...
  let portFree = null;
  if (port !== null) {
    const probe = tl.begin("port-probe", { port });
    portFree = (await probePort(port, { host, createServerImpl })).free;
    tl.end(probe, { free: portFree });
  }
...
export async function stopTree(
  ...
  const taskkill = (pid, args) => timed("taskkill", { tree, pid, args }, () => spawnKill("taskkill", args), (r) => ({ status: r?.status ?? null }));
  const signalPid = (pid, sig) => timed("signal", { tree, pid, signal: sig }, () => killImpl(pid, sig), () => ({ error: null }));
  ...
  const readTable = async () => {
    ...
    const entry = tl.begin("identity-read", { tree, timeoutMs: readMs });
    const table = await readProcessTable({ run: psRun, platform, timeoutMs: readMs });
  ...
  const ask = (hard, verdicts) => {
    ...
    record.identity.leaderSignalled = true;
    if (platform === "win32") {
      taskkill(child.pid, ["/pid", String(child.pid), "/T", "/F"]);
      return;
    }
    try {
      signalPid(group ? -child.pid : child.pid, signal);
    } catch (e) {
  ...
  const graceEntry = tl.begin("grace-wait", { tree, ms: graceMs });
  const until = Date.now() + graceMs;
  let graceSettled = settled();
  while (!graceSettled && Date.now() < until) {
    await sleep(50);
    graceSettled = settled();
  }
  ...
  if (!settled()) {
    ...
    if (!exited(child) || ours.length) {
      ...
      ask(true, verdicts);
      const hardEntry = tl.begin("hard-wait", { tree, ms: hardMs });
      const hard = Date.now() + hardMs;
      let hardSettled = settled();
      while (!hardSettled && Date.now() < hard) {
        await sleep(50);
        hardSettled = settled();
      }
...
function identityVerdict(d, table, platform) {
  ...
  if (table.error)
    return { state: "unverifiable", reason: table.error === "process-table-timeout" ? "process-table-timeout" : "process-table-unavailable" };
...
export async function runSupervisor(deps) {
  ...
  try {
    ...
    const session = withSessionDir(agent, stateRoots.sessions, agentEnv);
