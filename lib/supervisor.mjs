...
import { PROJECT_ROOT_ENV, researchPermission } from "./research/permission.mjs";
import {
  HEALTH_ERRORS,
  HEALTH_PATH,
  isKilnHealthBody,
...
export async function runSupervisor(deps) {
  ...
  for (const name of Object.keys(baseEnv)) if (name.toUpperCase() === KEYBOARD_STOP_ENV || name.toUpperCase() === PROJECT_ROOT_ENV) delete baseEnv[name];
  ...
  const research = researchPermission({ projectRoot: root });
