...
export async function stopJobTree(child, job, { graceMs = 8000, hardMs = 3000, timeline = null, deadline = null, tree = "agent", waitFloorMs = null } = {}) {
  const record = {
    ...
    timing: { leaderExitObservedMs: null, treeStopObservedMs: null },
    treeStopped: false,
    job: { members: null, terminated: null, remaining: null, released: null },
  };
  const tl = timeline ?? createShutdownTimeline();
  const clock = deadline ?? createShutdownDeadline({ at: Date.now() + graceMs + hardMs });
  ...
  record.treeStopped =
    record.exitObserved && record.descendantsEnumerated && remaining !== null && remaining.length === 0 && record.unfinished.length === 0;
