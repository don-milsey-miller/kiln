 ...
 *
 * ⚠️ **THE SUPERVISOR CHECKS AND NEVER CREATES.** Creating the state root is setup's work, and it
 * happens inside the one authenticated transaction that owns the project lock; a start command that
 * created directories would be a second writer with no exclusion behind it. If the root is missing
 * here, the honest answer is that setup has not run — not to quietly make one.
 *
...
export function resolveRunState({ projectRoot, stateMode = STATE_MODE.PROJECT, projectId, platform, env, home }) {
  ...
  const covers = coverageState({ projectRoot, mode: stateMode });
...
export async function runSupervisor(deps) {
  ...
  try {
    ...
    const agentChild = spawn(agent.command, session.args, {
      cwd: agent.cwd ?? root,
      env: session.env,
      stdio: "inherit",
      shell: false,
