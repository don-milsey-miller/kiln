...
export async function descendantsOfAsync(pid, { run, platform = process.platform } = {}) {
  ...
  const exec =
    ...
    ((c, a) =>
      new Promise((resolve) => {
        execFile(c, a, { encoding: "utf-8", maxBuffer: 8 * 1024 * 1024 }, (error, stdout) =>
...
export function trackDescendants(
  ...
  const sample = async () => {
    ...
    inFlight = descendantsOfAsync(child.pid, { run: psRun, platform }).then((found) => {
      ...
      // An answer that cannot be trusted is recorded as an enumeration NOT MADE, not as an absence.
      if (exited(child)) {
        enumerated = false;
        inFlight = null;
        return;
