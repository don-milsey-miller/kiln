...
import { JobHostRefusal, failedJobAgent, jobAgentProcess } from "./windows-job.mjs";
import { KEYBOARD_STOP, KEYBOARD_STOP_ENV, watchKeyboardStop } from "./keyboard-stop.mjs";
import { PROJECT_ROOT_ENV, STATE_MODE_ENV, researchPermission } from "./research/permission.mjs";
import {
  HEALTH_ERRORS,
  HEALTH_PATH,
  isKilnHealthBody,
...
/**
 * The prompt a new Kiln session opens with (TSK-0058, ACC-0103).
 *
 * ⚠️ **PI'S FIRST POSITIONAL MESSAGE, AND ONLY IN A TERMINAL.** Measured on the pinned 0.84.4: an interactive Pi
 * sends its first positional argument as the first turn, through the same prompt call a typed line takes, so the
 * template is expanded. Without a terminal Pi runs in print mode and joins any piped input and that argument into
 * one message with no separator, so the operator's text and the command arrive as one unexpanded line. The caller
 * says whether Pi will have a terminal.
 */
export const START_PROMPT = "/kiln-start";
...
export async function stopLauncher(
  child,
  { graceMs = SHUTDOWN_BUDGET.graceMs, hardMs = SHUTDOWN_BUDGET.hardMs, kill, log = () => {}, timeline = null, deadline = null, waitFloorMs = null } = {}
) {
...
export async function shutdown({
  ...
  signal = null,
  // True when the run was stopped by Ctrl+C read at the keyboard inside an interactive session (lib/keyboard-stop.mjs).
  keyboard = false,
  port = null,
  host = HOST,
  graceMs = SHUTDOWN_BUDGET.graceMs,
  hardMs = SHUTDOWN_BUDGET.hardMs,
  ...
  timeline = null,
  log = () => {},
  // F130 mechanism 2, prototype: the job that holds the agent, when the agent was started in one.
  agentJob = null,
  // And the launcher's.
  launcherJob = null,
} = {}) {
  ...
  // ⚠️ **THE KEYBOARD STOP IS THE THIRD TRIGGER, AND IT CARRIES NO SIGNAL (F130).** An interactive Pi reads
  // Ctrl+C as a key, so no signal occurs; `keyboard` says the stop was the key, and `signal` stays null.
  if (keyboard && signal) throw new TypeError("a shutdown has one trigger: a keyboard stop carries no signal");
  const trigger = keyboard ? "keyboard" : signal ? "signal" : "agent-exit";
  ...
  // ⚠️ **THE TWO TREES STOP SIDE BY SIDE, AND ONE PROCESS-TABLE READ AT A TIME SERVES BOTH (F130, O1).** They used to
  // stop one after the other, each with its own identity reads, so the agent's grace period and reads came out of the
  // budget before the launcher's tree was asked anything. On a loaded Windows host, where a table costs up to 2.3
  // seconds, that left the launcher's read about 1.5 seconds: it was abandoned about 70ms before it answered, and its
  // descendant stayed unverified. Stopped together, the trees' grace periods end on the same deadline, their reads
  // fall together, and the broker answers every read requested while one is running with that one.
  ...
  const tables = createTableBroker({ run: psRun, platform });
  const opts = { platform, run, psRun, kill, log, timeline: tl, deadline: clock, identityFloorMs: treeFloorMs, waitFloorMs, tables };
  ...
  const stoppingAgent = agentJob
    ? stopJobTree(agent, agentJob, { ...phase(), timeline: tl, deadline: clock, tree: "agent", waitFloorMs })
    : stopTree(agent, { ...opts, ...phase(), tree: "agent", knownDescendants: agentDescendants });

  // ⚠️ THE LAUNCHER GETS ITS OWN CONTROL CHANNEL FIRST — a stop message and a closed pipe are what
  // it was built to answer.
  // ⚠️ **AND THAT STEP LEAVES THE LAUNCHER'S TREE ITS VERIFICATION.** The control channel's wait runs before the
  // launcher's tree is looked at, so the tree's read, termination and wait are held back from it and released once it
  // is done. The agent's allowance was taken before this reserve and is not shortened by it, so both trees' grace
  // periods still end on the same deadline and their reads still fall together.
  ...
  const [agentTree, launcherTree] = await Promise.all([
    stoppingAgent,
    launcherJob
      ? stopJobTree(launcher, launcherJob, { ...phase(), timeline: tl, deadline: clock, tree: "launcher", waitFloorMs })
      : stopTree(launcher, { ...opts, ...phase(), tree: "launcher", group: true, knownDescendants: launcherDescendants }),
  ]);
...
/**
 * ACC-0081's one shutdown deadline, 8,000 ms from the trigger, as its grace and hard periods.
 *
 * ⚠️ **ONE CONFIGURATION, FOR THE COMMAND AND FOR ITS EVIDENCE.** The command ran on this module's own defaults, 8,000 ms of
 * grace plus 3,000 ms hard, and recorded an 11,000 ms budget, while the evidence fixture passed 5,000 plus 3,000 and
 * recorded 8,000 (EVD-0150). Both now take their periods from here, so the evidence measures the budget that ships.
 * `stopTree`, `stopJobTree` and `stopLauncher` default to it as well; in a run they never use their defaults, because
 * `shutdown()` hands each of them its share of the one deadline.
 */
export const SHUTDOWN_BUDGET = Object.freeze({ graceMs: 5000, hardMs: 3000 });
...
export function watchForStop(onStop, { signals = STOP_SIGNALS, target = process } = {}) {
  ...
  // `request(stop)` is the same first-one-wins path for a stop that is not a signal: the keyboard stop
  // (`lib/keyboard-stop.mjs`), recorded under its own name and never as a signal that did not occur.
  const state = { received: null, completion, error: null, dispose: () => {}, request: (stop) => fire(stop) };
...
/** A creation key as an ISO time, for a diagnostic line only: FILETIME digits on Windows, epoch ms on POSIX. */
function creationIso(created) {
  if (created == null) return "unknown";
  const n = BigInt(created);
  const ms = n > 10n ** 16n ? n / 10000n - 11644473600000n : n;
...
/**
 * One process-table read at a time for a whole shutdown, shared by every tree that asks while it runs (F130, O1).
 *
 * ⚠️ **A READ ALREADY RUNNING IS JOINED, NEVER REPEATED; A FINISHED ONE IS NEVER REUSED.** On a loaded Windows host a
 * table costs 0.5 to 2.3 seconds to answer, and two trees each reading their own inside one 8,000ms deadline left the
 * second tree's read too little time: it was abandoned about 70ms before it would have answered, and its descendant
 * stayed unverified. The trees now stop side by side, so their reads fall together and one answers both. Each
 * identity is still checked against a table taken after both trees were asked to stop, immediately before the kill
 * it authorises, which is the check F116 requires; a table that has already answered is history and is not handed out.
 *
 * @param {{run?: Function, platform?: string}} [options]
 */
export function createTableBroker({ run, platform = process.platform } = {}) {
  let inFlight = null;
  return {
    async read(timeoutMs) {
      if (inFlight) return { ...(await inFlight), joined: true };
      inFlight = readProcessTable({ run, platform, timeoutMs });
      try {
...
export async function stopTree(
  child,
  {
    ...
    deadline = null,
    tree = null,
    // O1 (F130): the whole shutdown's shared reader. A direct caller without one reads on its own, as before.
    tables = null,
    log = () => {},
  } = {}
) {
  ...
  const readTable = async () => {
    ...
    const table = tables ? await tables.read(allowed.ms) : await readProcessTable({ run: psRun, platform, timeoutMs: allowed.ms });
    const outcome = {
      error: table.error ?? null,
      resolved: table.error !== "process-table-timeout",
      rows: table.rows ? table.rows.size : null,
      ...(table.joined ? { joined: true } : {}),
    };
...
/**
 * Stop a tree that a Windows job holds — F130 mechanism 2 (TSK-0058), PROTOTYPE, used only in job mode.
 *
 * ⚠️ **MEMBERSHIP, NOT PARENTAGE, AND NO PID IS EVER SIGNALLED.** The job holds the agent and everything it started
 * from creation, so its own list answers "what is left of this tree" however long ago the agent exited, and ending
 * the job ends exactly its members: no pid can have been reused into it. The steps are the teardown's own: wait for
 * the leader within a grace allowance, list, end the job if anything is in it, wait for it to empty within a hard
 * allowance, list again, release the host. Every step draws from the one deadline; a step that cannot be paid for,
 * or a list the host does not answer, is named, and the tree is then not reported stopped.
 *
 * @param {object} child  the job-mode agent: its pid and exit
 * @param {{list: Function, terminate: Function, release: Function}} job
 */
export async function stopJobTree(child, job, { graceMs = SHUTDOWN_BUDGET.graceMs, hardMs = SHUTDOWN_BUDGET.hardMs, timeline = null, deadline = null, tree = "agent", waitFloorMs = null } = {}) {
  const record = {
    requested: false,
    exitObserved: false,
    escalated: false,
    method: "job object: list its members, end the job, list again",
    error: null,
    descendants: null,
    descendantIdentities: null,
    descendantsEnumerated: null,
    ...
    descendantsSurviving: null,
    descendantsUnverified: null,
    identity: { leaderSignalled: false, reads: 0, withheld: [] },
    unfinished: [],
    timing: { leaderExitObservedMs: null, treeStopObservedMs: null },
    treeStopped: false,
    job: { members: null, terminated: null, remaining: null, released: null, hostExited: null },
  };
  const tl = timeline ?? createShutdownTimeline();
  const clock = deadline ?? createShutdownDeadline({ at: Date.now() + graceMs + hardMs });
  const REQUEST_MS = 2000;
  const REQUEST_MIN_MS = 50;
  const unfinished = (operation, allowed) => {
    record.unfinished.push({ operation, reason: allowed.reason, remainingMs: allowed.remainingMs });
    tl.observe("deadline-refusal", { tree, operation, reason: allowed.reason });
  };
  const ask = async (kind, call) => {
    const allowed = clock.allow(REQUEST_MS, { minMs: REQUEST_MIN_MS });
    if (!allowed.granted) {
      unfinished(kind, allowed);
      return null;
    }
    const entry = tl.begin(kind, { tree, timeoutMs: allowed.ms });
    const answer = await call(allowed.ms);
    tl.end(entry, { ok: answer?.ok === true, members: Array.isArray(answer?.pids) ? answer.pids.length : null, error: answer?.error ?? null });
    return answer;
  };
  const settledLeader = () => exited(child);

  // The leader first: an interrupt reaches it too, and a leader that goes on its own needs nothing ended.
  if (!settledLeader()) {
    const graceAllowed = clock.allow(graceMs, { reserveMs: 3 * REQUEST_MIN_MS + Math.min(hardMs, SHUTDOWN_MIN_PHASE_MS) + WAIT_SLACK_MS, floorMs: waitFloorMs ?? undefined });
    if (graceAllowed.granted) {
      const entry = tl.begin("grace-wait", { tree, ms: graceAllowed.ms });
      tl.end(entry, { settled: await waitUntil(settledLeader, graceAllowed.ms) });
    }
  }
  if (settledLeader()) record.timing.leaderExitObservedMs = tl.now();

  const first = await ask("job-list", (ms) => job.list(ms));
  const members = first?.ok === true && Array.isArray(first.pids) ? first.pids.map(Number) : null;
  record.descendantsEnumerated = members !== null;
  record.job.members = members;
  record.descendants = (members ?? []).filter((pid) => pid !== child.pid);
  record.descendantIdentities = record.descendants.map((pid) => ({ pid, created: null }));

  let remaining = members;
  if (members && members.length) {
    record.requested = true;
    record.escalated = true;
    const ended = await ask("job-terminate", (ms) => job.terminate(ms));
    record.job.terminated = ended?.ok === true;
    remaining = ended?.ok === true && Array.isArray(ended.pids) ? ended.pids.map(Number) : null;
    // What ending the job leaves is read again until it is empty, within the hard allowance.
    const hardAllowed = clock.allow(hardMs, { minMs: 1, reserveMs: 2 * REQUEST_MIN_MS + WAIT_SLACK_MS });
    if (hardAllowed.granted) {
      const entry = tl.begin("hard-wait", { tree, ms: hardAllowed.ms });
      const until = Date.now() + hardAllowed.ms;
      while ((remaining === null || remaining.length > 0) && Date.now() < until) {
        await sleep(50);
        const again = await job.list(Math.max(REQUEST_MIN_MS, until - Date.now()));
        remaining = again?.ok === true && Array.isArray(again.pids) ? again.pids.map(Number) : null;
      }
      tl.end(entry, { settled: remaining !== null && remaining.length === 0 });
    } else if (remaining === null || remaining.length) unfinished("hard-wait", hardAllowed);
  }
  record.job.remaining = remaining;

  // The host's report of the leader's exit reaches the supervisor's view of it a poll later.
  await waitUntil(settledLeader, Math.max(0, Math.min(500, clock.remainingMs() - REQUEST_MIN_MS)));
  const released = await ask("job-release", (ms) => job.release(ms));
  record.job.released = released?.ok === true;
  // ⚠️ THE HOST IS THIS RUN'S CHILD TOO, so its exit is observed, within what the deadline has left, and a host that
  // does not go leaves the tree unstopped. A job without a host process (a test's) has nothing to wait for.
  const host = job.host;
  const tracked = Boolean(host) && typeof host.exitCode !== "undefined";
  const hostGone = () => !tracked || host.exitCode !== null || host.signalCode !== null;
  if (tracked) {
    // PowerShell can take more than a second to exit on a loaded host, so the wait takes what the deadline has left.
    const waitAllowed = clock.allow(3000, { minMs: 1 });
    if (waitAllowed.granted) {
      const entry = tl.begin("host-exit-wait", { tree, ms: waitAllowed.ms });
      tl.end(entry, { exited: await waitUntil(hostGone, waitAllowed.ms) });
    } else if (!hostGone()) unfinished("host-exit-wait", waitAllowed);
  }
  record.job.hostExited = hostGone();

  record.exitObserved = settledLeader();
  if (record.exitObserved && record.timing.leaderExitObservedMs === null) record.timing.leaderExitObservedMs = tl.now();
  record.descendantsSurviving = remaining === null ? [] : remaining.filter((pid) => pid !== child.pid);
  record.descendantsUnverified = remaining === null ? [...record.descendants] : [];
  record.treeStopped =
    record.exitObserved && record.descendantsEnumerated && remaining !== null && remaining.length === 0 && record.job.hostExited && record.unfinished.length === 0;
  if (record.treeStopped) record.timing.treeStopObservedMs = tl.now();
  if (!timeline) {
    tl.finalize();
    record.timeline = tl.snapshot();
  }
...
export async function runSupervisor(deps) {
  const {
    ...
    agentDir,
    // Whether a new session is opened with START_PROMPT: true only when Pi will have a terminal on both ends.
    startPrompt = false,
    // F130 mechanism 2, PROTOTYPE, opt-in: starts the agent inside a Windows job (`startInJob`). Absent, the agent is
    // spawned as before. Present, a host that cannot start it refuses the launch; nothing falls back to a plain spawn.
    agentJob = null,
    // The same, for the launcher's tree. Its control pipe passes through the host to the launcher unchanged.
    launcherJob = null,
    // Whether one Ctrl+C read by an interactive Pi stops the run (lib/keyboard-stop.mjs): true only when Pi has a
    // terminal. Outside Kiln, and in a piped run, the variable is not set and Pi's own Ctrl+C is unchanged.
    keyboardStop = false,
  } = deps;
  ...
  // An inherited keyboard-stop path names some other run's notice: no child gets one this run did not create. The same for
  // an inherited project root: the agent is told this run's, below.
  for (const name of Object.keys(baseEnv))
    if ([KEYBOARD_STOP_ENV, PROJECT_ROOT_ENV, STATE_MODE_ENV].includes(name.toUpperCase())) delete baseEnv[name];
  // ⚠️ **NO TAVILY KEY WHERE RESEARCH IS NOT PERMITTED (F4, ACC-0120).** Research runs only when the project chose Tavily
  // and this computer granted it. Where that does not hold at launch, the key reaches neither child, so nothing in either
  // tree can spend it; the research tools refuse on their own as well.
  const research = researchPermission({ projectRoot: root, stateMode });
  if (!research.permitted) {
    const withheld = Object.keys(baseEnv).filter((name) => name.toUpperCase() === "TAVILY_API_KEY");
    for (const name of withheld) delete baseEnv[name];
    if (withheld.length) log(`web research is not permitted here (${research.reason}); TAVILY_API_KEY is withheld from Pi and the shell`);
  }
  ...
  // ⚠️ **NOT RUN WHEN BOTH TREES START IN JOBS (ACC-0081).** Their shutdown lists and terminates each tree through its job
  // and reads no process table, so proving the reader first would be launch time spent on nothing the run uses.
  const bothInJobs = Boolean(agentJob && launcherJob);
  const preflight = bothInJobs
    ? { ran: false, ok: true, verified: false, reason: "both-trees-in-jobs", ms: 0, rows: null }
    : await primeProcessTable({ psRun: deps.psRun, platform: deps.platform });
  if (!preflight.ok)
    throw new SupervisorRefusal(
  ...
  // ⚠️ F130, PROTOTYPE: IN JOB MODE THE LAUNCHER, TOO, IS CREATED INSIDE A JOB BEFORE IT RUNS. Its control pipe is the
  // host's stdin, which the launcher inherits, so `stop` and the closed pipe reach it exactly as before. A host that
  // cannot start it fails the launch through the launcher's own spawn-failure path; nothing falls back.
  let launcherChild;
  let launcherJobHandle = null;
  let launcherJobDir = null;
  if (launcherJob) {
    launcherJobDir = mkdtempSync(join(tmpdir(), "kiln-launcher-job-"));
    try {
      launcherJobHandle = await launcherJob({ command: launcher.command, args: launcher.args, controlDir: launcherJobDir, env: childEnv, cwd: launcher.cwd ?? root, stdio: ["pipe", "inherit", "inherit"] });
      launcherChild = jobAgentProcess(launcherJobHandle);
      log(`launcher started inside a job by its host in ${launcherJobHandle.startedMs} ms`);
    } catch (e) {
      launcherChild = failedJobAgent(e instanceof JobHostRefusal ? e : new JobHostRefusal("host-error", String(e?.message ?? e)));
    }
  } else
    launcherChild = spawn(launcher.command, launcher.args, {
  ...
  // In job mode the job is the launcher's tree, so no process-table tracker is needed for it.
  const launcherTree = launcherJob ? null : trackDescendants(launcherChild, { psRun: deps.psRun, platform: deps.platform, tree: "launcher" });
  ...
  const ownedFiles = [];
  let agentChild = null;
  let agentSpawnedAt = null;
  // Job mode: the host's handle and its private control directory, removed when the run ends.
  let agentJobHandle = null;
  let agentJobDir = null;
  let agentTree = null;
  ...
  // The keyboard stop's private directory, its notice file and the watch on it (lib/keyboard-stop.mjs).
  let keyboardStopDir = null;
  let keyboardWatch = null;
  let keyboardNotice = null;
  ...
  const performShutdown = (signal = null, startedAtMs = null) => {
    ...
    // ⚠️ **AND FOR A KEYBOARD STOP, THAT MOMENT IS THE KEY, NOT THE NOTICE.** The operator started waiting when they
    // pressed Ctrl+C; however late the notice was seen, that delay is spent from the same eight seconds. A key time
    // later than now is a clock this process cannot trust, and counts from now.
    const deadline = Math.min(startedAtMs ?? Date.now(), Date.now()) + graceMs + hardMs;
    ...
    shutdownStarted = (async () => {
      ...
      return shutdown({
        ...
        agentJob: agentJobHandle,
        launcherDescendants: launcherTree?.snapshot({ origin }) ?? null,
        launcherJob: launcherJobHandle,
        ownedFiles,
        signal: signal === KEYBOARD_STOP ? null : signal,
        keyboard: signal === KEYBOARD_STOP,
        port,
        host,
        graceMs,
        hardMs,
  ...
  const stopWatch = watchForStop((signal) => performShutdown(signal, signal === KEYBOARD_STOP ? (keyboardNotice?.keyAtMs ?? null) : null), { target: deps.signalTarget });
  ...
  try {
    ...
    // ⚠️ THE KEYBOARD STOP'S NOTICE FILE IS NAMED TO THE AGENT ALONE, and only when Pi is interactive.
    if (keyboardStop) keyboardStopDir = mkdtempSync(join(tmpdir(), "kiln-keyboard-stop-"));
    const keyboardStopFile = keyboardStopDir ? join(keyboardStopDir, "stop.json") : null;
    const agentEnv = {
      ...baseEnv,
      PORT: String(port),
      ...(mode.selfHost === true ? { [SELF_HOST_MARKER]: SELF_HOST_VALIDATED } : {}),
      ...(keyboardStopFile ? { [KEYBOARD_STOP_ENV]: keyboardStopFile } : {}),
      // The project research and consent are read for (F4): named, never guessed from Pi's working directory.
      [PROJECT_ROOT_ENV]: root,
      [STATE_MODE_ENV]: stateMode,
    };
    ...
    // ⚠️ **THE START PROMPT FOLLOWS THE RECORD WRITTEN UNDER THE LOCK, NOT THE PLAN MADE BEFORE IT.** A run that
    // planned a first session can lose the race and adopt another run's session, and that session must not be
    // opened with a second start. A resume is never sent one.
    const starting = recorded.action === SESSION.START;
    const agentArgs = starting && startPrompt ? [...session.args, START_PROMPT] : session.args;
    ...
    if (starting)
      log(startPrompt ? `new session: Pi opens it with ${START_PROMPT}` : `new session: no ${START_PROMPT}, because Pi's input or output is not a terminal`);
    ...
    if (agentJob) {
      // ⚠️ F130 MECHANISM 2, PROTOTYPE: THE AGENT IS CREATED INSIDE A JOB BEFORE IT RUNS, and its tree is the job's.
      // A host that cannot do that refuses the launch through the spawn-failure path; there is no plain-spawn fallback.
      agentJobDir = mkdtempSync(join(tmpdir(), "kiln-agent-job-"));
      try {
        agentJobHandle = await agentJob({ command: agent.command, args: agentArgs, controlDir: agentJobDir, env: session.env, cwd: agent.cwd ?? root, stdio: "inherit" });
        agentChild = jobAgentProcess(agentJobHandle);
        log(`agent started inside a job by its host in ${agentJobHandle.startedMs} ms`);
      } catch (e) {
        agentChild = failedJobAgent(e instanceof JobHostRefusal ? e : new JobHostRefusal("host-error", String(e?.message ?? e)));
      }
    } else
      agentChild = spawn(agent.command, agentArgs, {
        cwd: agent.cwd ?? root,
        env: session.env,
        stdio: "inherit",
        shell: false,
    ...
    // F130 mechanism 2, diagnostic only: when the agent was spawned, beside the creation time a late table reports.
    agentSpawnedAt = new Date().toISOString();
    // In job mode the job is the agent's tree, so no process-table tracker races the agent's exit.
    agentTree = agentJob ? null : trackDescendants(agentChild, { psRun: deps.psRun, platform: deps.platform, tree: "agent" });
    // ⚠️ **THE KEYBOARD STOP STARTS THE SHUTDOWN WHEN ITS NOTICE IS SEEN, NOT WHEN PI EXITS.** Pi's own shutdown waits
    // until it is idle, so the eight-second deadline is started here, from the notice, through the same first-one-wins
    // path a signal takes.
    if (keyboardStopFile)
      keyboardWatch = watchKeyboardStop(keyboardStopFile, (notice) => {
        keyboardNotice = notice;
        stopWatch.request(KEYBOARD_STOP);
      });
    ...
    // ⚠️ **A KEY PRESSED JUST BEFORE PI WENT IS STILL THE KEYBOARD STOP.** The extension writes the notice before it asks
    // Pi to go, so when Pi's exit is observed before the watch has seen the file, the file is already there: it is
    // looked for now, synchronously, before anything is recorded about what ended the run.
    if (ended.how === "agent-exit" && !ended.exit.spawnError && keyboardWatch?.check() && stopWatch.received === KEYBOARD_STOP) ended = { how: "signal" };
    ...
    if (ended.how === "signal")
      log(
        stopWatch.received === KEYBOARD_STOP
          ? `stopped by Ctrl+C at the keyboard${keyboardNotice?.keyAtMs != null ? `, seen ${Math.max(0, keyboardNotice.noticedAtMs - keyboardNotice.keyAtMs)} ms after the key (${keyboardNotice.via})` : ` (${keyboardNotice?.via ?? "notice"})`}`
          : `interrupted by ${stopWatch.received}`
      );
    ...
    // ⚠️ F130 MECHANISM 2: WHY A TREE'S DESCENDANTS WENT UNOBSERVED, said where the refusal is printed. The count of
    // each kind of look, and for each look how long it took and whether the leader was alive at its start and end,
    // so a failed run in a test log says whether its looks raced the exit, failed or never came back.
    for (const [name, t] of [["agent", shutdownRecord.agent], ["launcher", shutdownRecord.launcherTree]])
      if (t?.descendantsEnumerated === false)
        log(
          `${name} tree looks: ${JSON.stringify(t.descendantLooks ?? null)}${name === "agent" && agentSpawnedAt ? `, spawned ${agentSpawnedAt}` : ""}; ` +
            (t.descendantQueries ?? [])
              .map(
                (q) =>
                  `${q.id} ${q.classification}${q.discardReason ? ` (${q.discardReason})` : ""} ` +
                  `${q.durationMs === null ? "open" : `${Math.round(q.durationMs)}ms`} alive ${q.leaderAliveAtStart}->${q.leaderAliveAtEnd}` +
                  ` root ${q.rootRow ? `row created ${creationIso(q.rootRow.created)}` : "row absent"} (${q.rootVerification})` +
                  `${q.error ? ` error ${q.error}` : ""}`
              )
              .join("; ")
        );
    // F130, job mode: an incomplete teardown says what each job did, since a job's tree names no looks.
    if (!shutdownRecord.complete)
      for (const [name, t] of [["agent", shutdownRecord.agent], ["launcher", shutdownRecord.launcherTree]])
        if (t?.job) log(`${name} job: stopped ${t.treeStopped}, exit observed ${t.exitObserved}, ${JSON.stringify(t.job)}, unfinished ${JSON.stringify(t.unfinished)}`);
    if (!shutdownRecord.complete)
      throw new SupervisorRefusal(
    ...
    if (launcherJobDir) rmSync(launcherJobDir, { recursive: true, force: true, maxRetries: 17, retryDelay: 100 });
    if (agentJobDir) rmSync(agentJobDir, { recursive: true, force: true, maxRetries: 17, retryDelay: 100 });
    keyboardWatch?.dispose();
    if (keyboardStopDir) rmSync(keyboardStopDir, { recursive: true, force: true, maxRetries: 17, retryDelay: 100 });
  }
}
