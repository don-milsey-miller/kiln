...
export async function stopLauncher(
  ...
  if (!graceAllowed.granted) tl.observe("deadline-refusal", { tree: "launcher-control", operation: "grace-wait", reason: graceAllowed.reason });
    ...
    const until = Date.now() + graceAllowed.ms;
    while (!graceGone && Date.now() < until) {
      await sleep(Math.max(1, Math.min(50, until - Date.now())));
      graceGone = gone();
    }
  ...
  if (!gone()) {
    ...
    if (!killAllowed.granted) {
      ...
      if (!hardAllowed.granted) {
        ...
        const hard = Date.now() + hardAllowed.ms;
        let hardGone = gone();
        while (!hardGone && Date.now() < hard) {
          await sleep(Math.max(1, Math.min(50, hard - Date.now())));
          hardGone = gone();
        }
