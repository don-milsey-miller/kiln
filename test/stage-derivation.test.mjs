...
test("the current stage moves when an attestation moves, and only then", () => {
  ...
  // Today's real state, edited when the plan legitimately moves. Every gate is attested through, so
  // nothing is current: stage 5's `data-model-approved` was the last to clear, when the five
  // runtime records got schemas (DEC-0033).
  //
  // ⚠️ `null` IS THE ANSWER, NOT AN ABSENT ONE. "No blocking stage" and "the derivation failed" are
  // different things, which is why the mutations below have to move it off `null` for this test to
